...
const Offcanvases = ({ count }) => {
  ...
  const getFlashcardStudylist = () => {
    axiosInstance.get(`${ipaddress2}/FlashcardStudylistSection/${user.user_id}/`)
      .then((r) => { setflashcardStudylists(r.data.data) })
      .catch(() => { console.log("FlashcardStudylistSection Error") })
  }
  // To fetch all the document based study lists
  const getStudylist = () => {
    axiosInstance.get(`${ipaddress}/GetStudyListCategories/${user.user_id}/`)
      .then((r) => { setStudylists(r.data) })
      .catch(() => { console.log("GetStudyListCategories Error") })
  }

  useEffect(() => {
    axiosInstance.get(`${ipaddress}/CoursesView/${user.user_id}/`)
      .then((r) => { setJoinedCourses(r.data.joined_courses); })
  ...
  const getallStudylist = () => {
    axiosInstance.get(`${ipaddress}/GetStudyListCategories/${user.user_id}/`)
      .then((r) => { setallstudylists(r.data) })
      .catch((err) => { console.log(" GetStudyListCategories error", err) })
  }
              ...
              joinedCourses.map((x, index) => {
                return (
                  <li data-bs-dismiss="offcanvas" key={index} className='mt-2 py-3 border-bottom px-2 px-lg-3'>
                    ...
                    }} className='text-decoration-none text-primary' style={{ fontSize: '16px',textTransform:'capitalize', cursor: 'pointer', fontWeight: 450, lineHeight: '22px', letterSpacing: '0.32px' }}>{x.course_name}
          ...
          <div>
                ...
                <ul className='p-0 mt-3 px-2' style={{ listStyleType: 'none' }}>
                  {joinedpublicgroups && (
                    joinedpublicgroups.map((x, index) => {
                      return (
                        <li data-bs-dismiss="offcanvas" key={index} className='mt-1 py-3 border-bottom px-2 px-lg-3'>
                          <span data-bs-dismiss="offcanvas" onClick={() => {
                            removepublicgroupunseenmessages(x.group_id, x.group_name)
                          }} className='text-decoration-none text-primary' style={{ fontSize: '16px',textTransform:'capitalize', cursor:'pointer', fontWeight: 450, lineHeight: 'normal', letterSpacing: '0.32px' }}>{x.group_name} <span className={`text-white rounded-circle px-1 ${x.unseen_count > 0 ? '' : 'd-none'}`} style={{ backgroundColor: '#FF845D', height: '20px', width: '20px', fontSize: '13px' }}>{x.unseen_count}</span></span>
            ...
            <div className={groups === "privategroups" ? 'd-block' : 'd-none'}>
              <div className='d-flex justify-content-between px-2 px-lg-5 mt-3'>
                <span data-bs-dismiss="offcanvas" onClick={() => { navigate('/groups/privategroup') }}
                    ...
                    joinedGroups.map((x, index) => {
                      return (
                        <li data-bs-dismiss="offcanvas" key={index} className='mt-1 py-3 border-bottom px-2 px-lg-3'>
                          ...
                          }} className='text-decoration-none text-primary'>{x.group_name} <span className={`text-white rounded-circle px-1 ${x.unseen_messages_count > 0 ? '' : 'd-none'}`} style={{ backgroundColor: '#FF845D',textTransform:'capitalize', height: '20px', width: '20px', fontSize: '13px' }}>{x.unseen_messages_count}</span></span>
          ...
          <div>
            ...
            <div>
              <ul className={`p-0 px-2 ${studylisttype === "document" ? '' : 'd-none'}`} style={{ listStyleType: 'none', height: '80%', overflowY: studylists.length > 5 ? 'scroll' : 'none' }}>
                {studylists && (
                  studylists.map((x) => (
                    <li key={x.id} data-bs-dismiss="offcanvas" className='mb-2 py-3 border-bottom px-2 px-lg-3'>
                      <Link to={`/studylist/${x.id}`} className='text-decoration-none text-primary' style={{ fontSize: '16px',textTransform:'capitalize', cursor: 'pointer', letterSpacing: '0.32px', lineHeight: 'normal', fontWeight: 450 }}>{x.study_list_name}</Link>
                      <div className='mt-1'>
                        <span>
                          ...
                          <span className='ms-1 text-secondary text-decoration-none' style={{ fontSize: '13px' }}>{x.document_count} Documents</span>
                        </span>
                      </div>
                    </li>
                  ))
                    ...
                    return (
                      <>
                        <li data-bs-dismiss="offcanvas" className='mb-2 py-3 border-bottom px-2 px-lg-3'>
                          <Link to={`/flashcard_studylist/${x.id}`} className='text-decoration-none text-primary' style={{ fontSize: '17px', cursor: 'pointer',textTransform:'capitalize' }}>{x.study_list_name}</Link>
                          <div className='mt-1'>
