...
import { openTour, closeTour } from "../features/tourSlice";
...
const Navbar = ({ setLanguage, count2, count, setindex1, setCount }) => {
  ...
  const handleLogout = () => {
    axiosInstance.post(`${ipaddress}/User_Logout/${user?.user_id}/`, {
  ...
  const pushnotificationcontrol = () => {
    axiosInstance.get(`${ipaddress}/PushNotificationsControl/${user?.user_id}/`)
      ...
      }).catch(() => { console.log("Push notification error") })
  }
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`${ipaddress}/displaynotifications/${user?.user_id}/`)
      ...
      }).catch(() => { console.log("Notification Fetching Error") })
    ...
    axiosInstance.get(`${ipaddress}/ParticularUserGroup/${user?.user_id}/`)
      ...
      }).catch(() => { console.log("Group details fetching error") });
  ...
  const acceptRequest = (user_id, group_id) => {
    apiClient.put(`${ipaddress}/groupadminacceptance/${user_id}/${group_id}/`)
      .then((r) => {
        setCount1(count + 1);
        toast.success('Request accepted', { autoClose: 2000 });
      })
      .catch(() => { console.log("Admin accept request error") })
  }

  // -----------------------------------------DELETE NOTIFICATION-----------------------------------------------------------
  const deleteNotification = (id) => {
    axiosInstance.delete(`${ipaddress}/deletenotification/${id}/`)
      .then((r) => { setCount1(count1 + 1) })
      .catch(() => { console.log("Notification Delete Error") })
  }
  ...
  const rejectRequest = (user_id, id) => {
    axiosInstance.delete(`${ipaddress}/groupadminacceptance/${user_id}/${id}/`)
      .then((r) => { setCount1(count1 + 1) })
      .catch(() => { console.log("Request Reject Error Error") })
  }
  // -------------------------------------------Language Translation for website---------------------------------------------
  const trasnslateGerman = () => { setLanguage("ge") }
  const trasnslateEnglish = () => { setLanguage("en") }
    ...
    <div className="container px-2">
      <nav className="navbar navbar-expand-lg navbar-expand-md py-3 d-none d-lg-block">
        <div className="d-flex w-100 justify-content-between">
          ...
          <ul className="navbar-nav gap-2 d-flex align-items-center justify-content-end" style={{ width: '40%' }}>
            <li className="nav-item d-flex gap-3">
              <span className={`${lang === "en" ? 'text-decoration-underline fw-bold' : ''}`} onClick={() => { translate("en") }} style={{ color: lang === "en" ? '#2C2C2C' : '#B6BBBC', cursor: 'pointer', fontSize: '14px' }} >EN</span>
              <span className={`${lang === "ge" ? 'text-decoration-underline fw-bold' : ''}`} onClick={() => { translate("ge") }} style={{ color: lang === "ge" ? '#2C2C2C' : '#B6BBBC', cursor: 'pointer', fontSize: '14px' }} >DE</span>
            </li>
            {/* -----------------------------------------------------Rewards icon-------------------------------------------------- */}
            <li className="nav-item d-flex align-items-center">
              <Link to='/rewards' className="nav-link d-flex align-items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
                  <circle cx="17.5" cy="17.5" r="17.25" fill="#F4CD00" stroke="#2A3941" strokeWidth="0.5" />
            ...
            {['subjects', 'filterflashcard', 'showpdf'].includes(pageLocation) ?
              <OverlayTrigger placement="left" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Start Tour")}>
                <li className="nav-item d-none d-lg-block" onClick={() => { openTourGuide(); setnavbar_dropdown_visible(false); }}>
                  <Link className="dropdown-item p-2" style={{ fontFamily: " 'DM Sans', sans-serif", fontSize: '16px', lineHeight: '28px', color: '#2A3941' }}>
                    <svg viewBox="0 0 1024 1024" width={25} height={25} xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path fill="#000000" d="M640 608h-64V416h64v192zm0 160v160a32 32 0 0 1-32 32H416a32 32 0 0 1-32-32V768h64v128h128V768h64zM384 608V416h64v192h-64zm256-352h-64V128H448v128h-64V96a32 32 0 0 1 32-32h192a32 32 0 0 1 32 32v160z"></path><path fill="#000000" d="m220.8 256-71.232 80 71.168 80H768V256H220.8zm-14.4-64H800a32 32 0 0 1 32 32v224a32 32 0 0 1-32 32H206.4a32 32 0 0 1-23.936-10.752l-99.584-112a32 32 0 0 1 0-42.496l99.584-112A32 32 0 0 1 206.4 192zm678.784 496-71.104 80H266.816V608h547.2l71.168 80zm-56.768-144H234.88a32 32 0 0 0-32 32v224a32 32 0 0 0 32 32h593.6a32 32 0 0 0 23.936-10.752l99.584-112a32 32 0 0 0 0-42.496l-99.584-112A32 32 0 0 0 828.48 544z"></path></g></svg>
                  </Link>
                </li>
              </OverlayTrigger> : ''
            }
            ...
            <li className="nav-item d-flex align-items-center" onClick={() => { setnavbar_dropdown_visible(!navbar_dropdown_visible) }} style={{ position: 'relative', cursor: 'pointer' }}>
              <a className="nav-link p-0 py-1">
                <img src={userdetails.profile_pic} width={50} className={userdetails.profile_pic == null ? 'd-none' : 'd-inline rounded-circle'} style={{ height: '45px', width: '45px' }} alt="navbar" />
              ...
              <ul className={`border-0 shadow px-2 bg-white p-3 px-4 rounded ${navbar_dropdown_visible ? '' : 'd-none'}`} style={{ position: 'absolute', top: '60px', listStyleType: 'none', left: '-220px', zIndex: 9999 }}>
                ...
                <li className=""><Link to={'/reset-password'} className="dropdown-item mb-3" style={{ fontFamily: " 'DM Sans', sans-serif", fontSize: '16px', lineHeight: '28px', color: '#2A3941' }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 24 24" fill="none">
                    <path d="M12 1C8.68629 1 6 3.68629 6 7V10H4C3.44772 10 3 10.4477 3 11V21C3 21.5523 3.44772 22 4 22H20C20.5523 22 21 21.5523 21 21V11C21 10.4477 20.5523 10 20 10H18V7C18 3.68629 15.3137 1 12 1ZM8 10V7C8 4.79086 9.79086 3 12 3C14.2091 3 16 4.79086 16 7V10H8ZM12 14C12.5523 14 13 14.4477 13 15V17C13 17.5523 12.5523 18 12 18C11.4477 18 11 17.5523 11 17V15C11 14.4477 11.4477 14 12 14Z" fill="#2A3941" />
                  </svg>
                  <span className="ms-4">Change Password</span></Link>
                </li>
      ...
      <nav className="navbar navbar2 navbar-expand bg-white py-2 fixed-top border-bottom d-block d-lg-none">
        <div className="container-fluid">
          <button className="border-0" type="button" data-bs-toggle="offcanvas" data-bs-target="#website_offcanvas" aria-controls="website_offcanvas">
            <img src={require('../img/menus.png')} className="bg-white" alt="navbar" height={30} />
          </button>

          <ul className="navbar-nav">
            <li className="nav-item d-flex align-items-center">
              <div className="mx-1">
                <i className="fa fa-search" style={{ fontSize: '18px' }} data-bs-toggle="offcanvas" data-bs-target="#searchoffcanvas" aria-controls="searchoffcanvas" bis_skin_checked="1" aria-hidden="true"></i>
              </div>
            </li>
            <li className="nav-item d-flex align-items-center">
              <Link to='/rewards' className="nav-link mx-1 text-uppercase">
            ...
            <li className="nav-item dropdown">
              <a className="nav-link mx-1 d-flex align-items-center" type="button" data-bs-toggle="offcanvas" data-bs-target="#notification" aria-controls="notification">
                  ...
                  notification2.map((x) => {
                    return (
                      <li className="d-flex justify-content-between align-items-center px-2 py-2">
                        ...
                        <button className="btn btn-sm btn-transparent ms-2" onClick={() => { deleteNotification(x.id) }}><i className="fa-solid fa-xmark"></i></button>
                      </li>
                    )
                  })
                )}
                  ...
                  pushNotification.map((z) => {
                    return (
                      <li className="d-flex justify-content-between align-items-center px-2 py-2">
                        ...
                        <button className="btn btn-sm btn-transparent ms-2" onClick={() => { deletepushNotification(z.id) }}><i className="fa-solid fa-xmark"></i></button>
                      </li>
                    )
                  })
                )}
      ...
      <div className="offcanvas offcanvas-start d-sm-block d-lg-none d-xl-none" data-bs-backdrop="static" tabIndex="-1" id="website_offcanvas" aria-labelledby="website_offcanvasLabel">
        <div className="offcanvas-header d-flex justify-content-between align-items-center">
          <img src={require('../img/landing_page/Group 385.png')} width={120} alt="navbar" />
          ...
          <div className="pb-5">
            <ul className="nav flex-column gap-3 text-start ps-0 pb-4">
              ...
              <li className="nav-item">
                <a data-bs-toggle="offcanvas" data-bs-target="#courseOffcanvas" aria-controls="courseOffcanvas" className="nav-link" type="button">
                  <span className="nav-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="26" viewBox="0 0 35 35" fill="none">
                  ...
                  <span className="ms-4 fw-medium text-secondary">Course</span>
                </a>
              </li>
              <li className="nav-item">
                <Link data-bs-toggle="offcanvas" data-bs-target="#groupOffcanvas" aria-controls="groupOffcanvas" className="nav-link" type="button">
                  <span className="nav-icon">
                  ...
                  <span className="ms-4  fw-medium text-secondary">Group</span>
                </Link>
              </li>
              <li className="nav-item">
                <Link data-bs-toggle="offcanvas" data-bs-target="#studylistcanvas" aria-controls="studylistcanvas" className="nav-link">
                  <span className="nav-icon">
                  ...
                  <span className="ms-4  fw-medium text-secondary">Study List</span>
                </Link>
              </li>
              {/**<li className="nav-item">
              ...
              <li data-bs-dismiss="offcanvas" className="nav-item">
                <Link to={'/help'} className="nav-link">
                  ...
                  <span className="ms-4  fw-medium text-secondary">Need Help</span>
                </Link>
              </li>
            </ul>
            <ul className="border-top pt-4 nav flex-column gap-3 text-start ps-0" style={{ listStyleType: 'none' }}>
              ...
              <li data-bs-dismiss="offcanvas" className="nav-item">
                <p onClick={() => { handleLogout() }} style={{ cursor: 'pointer' }} className="nav-link">
                  <span className="nav-icon">
              ...
              <div className="bg-light w-100 py-2 mt-5">
                  ...
                  <div className="pe-2 d-flex justify-content-between py-2 py-lg-0 gap-2 flex-column flex-lg-row align-items-center h-100">
                    <div className="d-flex align-items-center gap-2">
                      <div className="d-flex gap-2 align-items-center">
                        ...
                        <select className="border px-2 py-1" onChange={(e) => translate(e.target.value)}>
                    ...
                    <ul className="nav d-flex gap-4">
                      <li><Link className="text-secondary" to={'https://www.iubenda.com/terms-and-conditions/43142553'} target="_blank" style={{ textDecoration: 'none', color: 'inherit' }}>Terms of Use</Link></li>
                      <li><Link className="text-secondary" to={'https://www.iubenda.com/privacy-policy/43142553'} target="_blank" style={{ textDecoration: 'none', color: 'inherit' }}>Privacy</Link></li>
                      <li><Link className="text-secondary" to={'/our_team'} target="_blank" style={{ textDecoration: 'none', color: 'inherit' }}>About Us</Link></li>
                      <li><Link className="text-secondary" to={'https://www.iubenda.com/privacy-policy/43142553/cookie-policy'} target="_blank" style={{ textDecoration: 'none', color: 'inherit' }}>Privacy Settings</Link></li>
                    </ul>
                  </div>
                </div>
              </div>
      ...
      <div className="offcanvas offcanvas-end border-0 shadow" data-bs-scroll="true" data-bs-backdrop="false" tabIndex="-1" id="notification" aria-labelledby="notificationLabel" style={{ marginTop: '74px' }}>
        ...
        <div className="offcanvas-body p-0 px-2">
          <div className={notification1 <= 0 && notification2 <= 0 && pushNotification.length <= 0 ? "d-none" : "d-block text-end mt-1"}>
            <span onClick={() => { clearall() }} style={{ color: '#5d5fe3', fontSize: '13px', textDecoration: 'underline', cursor: 'pointer' }}>Clear All</span>
          </div>
          ...
          <ul className={notification1 <= 0 && notification2 <= 0 && pushNotification.length <= 0 ? "d-none" : "p-0"} style={{ listStyleType: 'none' }} aria-labelledby="notificationDropdown">
            {notification1 && (
              notification1.map((x, i) => {
                return (
                  <li key={i} className="d-flex justify-content-between align-items-center px-1 py-3 border-bottom">
                  ...
                  return (
                    <li className="d-flex justify-content-between align-items-center px-1 py-3 border-bottom">
                      <div className="d-flex align-items-center">
                        ...
                        <img src={x.profile_pic.profile_pic} width={28} height={28} className={x.profile_pic.profile_pic == null ? 'd-none' : 'd-inline rounded-circle'} alt="navbar" />
                        <div className="ms-2 d-flex">
                  ...
                  return (
                    <li className="d-flex justify-content-between align-items-center px-1 py-3 border-bottom">
                      <div className="d-flex">
                        ...
                        <img src={z.profile_pic} width={28} height={28} className={z.profile_pic == null ? 'd-none' : 'd-inline rounded-circle'} alt="navbar" />
                        <div className="ms-2">
