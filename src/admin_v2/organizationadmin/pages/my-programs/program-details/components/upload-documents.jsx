...
const DocumentUpload = () => {
    ...
    const onFileChange = async (e) => {
        ...
        const fileReader = new FileReader();  // Step 1: Read the uploaded file
        fileReader.onload = async () => {
            try {
                const fileData = fileReader.result; // Step 2: Load the PDF with pdf-lib
                const pdfDoc = await PDFDocument.load(fileData);
                const keywords = pdfDoc.getKeywords(); // Step 3: Retrieve the keywords metadata // Returns a string or undefined
                if (keywords) {
                    const formint = new FormData();
                    formint.append('pdf_keyword', keywords);
                    try {
                        const result = await axiosInstance.post(`${ipaddress}/GetPDFChecksum/`, formint);
                        const resultData = result?.data;
                        if (resultData?.status === true && resultData?.message === "success") {
                            toast.error("Request File in Our System!", toastConfig);
                        } else {
                    ...
                    document.getElementById('uploadform').style.display = 'none';
                    document.getElementById('details').style.display = 'block';
                    console.log("No keywords metadata found in the PDF.");
                }
            } catch (err) {
                console.log("Error processing uploaded PDF:", err);
            }
        };
        fileReader.readAsArrayBuffer(file); // Read the file as binary data
    };
    ...
    const upload = async (e) => {
        ...
        if (fileSizeInMB <= 100) {
            ...
            try {
                const response = await axiosInstance.post(`${ipaddress}/uploadDocuments/${user.user_id}/`, formData, {
                    headers: {
    ...
    const [allSemestersData, setAllSemestersData] = useState([]); // Raw for filtering
    ...
    useEffect(() => {
        setUserdata(orgData?.user)
        getSubjectAndSemester()
    }, [])
    ...
    const getSubjectAndSemester = async () => {
        try {
            setloading(true);
            const response = await axiosInstance.get(`/userProgramDetails/${orgData?.user?.id}/`);
            if (response?.data?.data) {
                const allCourses = [];
                const rawSemesters = [];

                response.data.data.forEach(program => {
                    program.semester.forEach(sem => {
                        // Store semester with all course info
                        rawSemesters.push({
                            semester_id: sem.start_sem_id,
                            sem_name: sem.samester_name,
                            courses: sem.courses
                        });
                ...
                const uniqueCourses = Array.from(
                    new Map(allCourses.map(item => [item.courses_id, item])).values()
                );

                // Set states
                setJoinedCourses(uniqueCourses);
                setAllSemestersData(rawSemesters);
            }
        } catch (err) {
            console.log("Error in getSubjectAndSemester", err);
        } finally {
            setloading(false);
        }
    };
    ...
    const handleCourseChange = (e) => {
        ...
        setCategoryIdid(selectedCourseId);
        setpreview_btn_status1(true);

        // Filter semesters that include this course
        const filteredSemesters = allSemestersData.filter(sem =>
            sem.courses.some(course => course.courses_id === selectedCourseId)
        );

        // Remove course list and keep only id and name
        const formattedSemesters = filteredSemesters.map(sem => ({
            semester_id: sem.semester_id,
            sem_name: sem.sem_name
        }));

        // Remove duplicate semesters (just in case)
        const uniqueFiltered = Array.from(
            new Map(formattedSemesters.map(item => [item.semester_id, item])).values()
        );

        setSemesterdata(uniqueFiltered);
    };


    return (
        <div className="upload-documents">
            <div className="container pb-5" style={{ position: 'relative', backgroundColor: '#F3F0FF', height: '150px' }}>

                <div id='uploadimg' className='bg-light rounded-circle p-3 border' style={{ position: 'absolute', top: '100px' }}>
                    <img src={require('../../../../../../img/images_icons/send-img2.png')} style={{ width: '60px' }} alt="upload" />
                </div>
            </div>
            <div className="container mt-2 pt-4">
                ...
                <section className="row justify-content-center mt-4" id='uploadform'>
                    ...
                    <div className="col-lg-9 col-md-10 rounded text-center mx-auto pt-4 pb-3 bg-light">
  ...
  <div className='text-end'>
    ...
    <button className='btn btn-sm btn-primary text-white ms-3' onClick={upload}>Upload</button>
                ...
                {/* -------------------------------------------------DETAILS FORM----------------------------------------------------- */}
                <div className='py-3' id='details' style={{ display: 'none' }}>
                    <div className='d-flex justify-content-evenly py-2 py-lg-3 pb-2 px-lg-5'>
                        <span className='fw-bold'>Upload Document</span>
                        ...
                        <div className={`mb-4 col-md-6 ${course_id > 0 ? 'd-none' : ''}`}>
                            ...
                            >
                                ...
                                {joinedCourses.map((x, index) => (
                                    <option key={index} value={x.courses_id}>{x.course_name}</option>
                                ))}
                            </select>
                        </div>
                ...
                <section className="row justify-content-center mt-4" id='finalstep' style={{ display: 'none' }}>
                    ...
                    <div className="col-lg-9 col-md-10 rounded text-center mx-auto pt-2 pb-3 bg-white shadow-sm">
                        ...
                        <div className='py-5 rounded px-4 mt-4' style={{ border: '1px dashed #5D5FE3', backgroundColor: '#CFF4D2' }}>
                            <img className='animate__animated animate__bounceIn' src={require('../../../../../../img/checkmark-removebg-preview.png')} width={50} alt="upload" />
