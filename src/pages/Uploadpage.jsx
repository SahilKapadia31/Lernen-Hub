...
const Uploadpage = (props) => {
  ...
  const [nav_paths, setNavPaths] = useState(location?.state?.backNavPath || [
  ...
  const courseData = (e) => {
    ...
    useEffect(()=>{
      setNavPaths([...nav_paths,  { name: "Upload Documentboard", path: "" }]);
    },[])
  


  // -------------------------------------------Set path----------------------------------------------
  setEncryptedData("name3", JSON.stringify('Upload Document'), 180)
  setEncryptedData("path3", JSON.stringify(`/uploadpage/${course_id}/${course_name}`), 180)
  ...
  const upload = async (e) => {
    ...
    if (fileSizeInMB <= 100) {
      ...
      try {
        const response = await apiClient.post(`${ipaddress}/uploadDocuments/${user.user_id}/`, formData);
