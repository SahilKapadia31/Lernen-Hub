...
const tourConfig = [
  {
    selector: "#Subject__Upload",
    content: `Add new document`,
  },
  {
    selector: "#Subject__Tabs",
    content: `Browse discussions, documents, and flashcards`,
    position: "bottom",
  },
  {
    selector: "#Subject__CreatePost",
...
const Subjects = (props) => {
  ...
  setEncryptedData("name2", JSON.stringify(`${course_name}`), 180);
  setEncryptedData("path2", JSON.stringify(`/subjects/${course_id}/${course_name}`), 180);
  removeData('name3');
  removeData('path3');
  ...
  const fetchdocuments = () => {
    ...
    axiosInstance.get(`${ipaddress}/courseDocuments/${course_id}/${userdata.user_id}/`)
      .then((r) => {
        ...
        setsubloading(false);
      }).catch((err) => { console.log("Course Documents fetching Error", err) })
  }
  ...
  const fetchcoursedetails = () => {
    axiosInstance.get(`${ipaddress}/CourseDetailsView/${course_id}/${userdata.user_id}/`)
      .then((r) => {
        ...
        setloading(false);
      }).catch(() => { console.log("Course Details fetching Error") })
  }
  ...
  const getallflashsetsundersubject = () => {
    ...
    axiosInstance.get(`${ipaddress2}/flashcardsetsRetrive/${course_id}/${userdata.user_id}/`)
      ...
      }).catch((err) => { console.log("Course Flashsets fetching Error", err) })
  }
  ...
  const getparticularusersflashsets = () => {
    ...
    axiosInstance.get(`${ipaddress2}/myflashset/${userdata.user_id}/${course_id}/`)
      .then((r) => {
        ...
        setsubloading(false);
        setsearchflashsetname("");
      }).catch(() => { console.log("Particular user's Flashsets fetching Error") })
  }
  ...
  const getdiscussion = () => {
    axiosInstance.get(`${ipaddress}/ocoursediscussion/${user.user_id}/${course_id}/`)
      .then((r) => {
        ...
        setloading(false);
      }).catch(() => { console.log("Course Discussion data fetching Error") })
  }
  useEffect(() => {
    setSearchcomment("");
    getdiscussion();
  ...
  const leaveCourse = () => {
    axiosInstance.post(`${ipaddress}/ExitGroup/${user.user_id}/${course_id}/`, { 'reason': exitgroup_reason })
      .then((r) => { navigate('/dashboard/page') })
      .catch(() => { console.log("ExitGroup Error") })
  }
  ...
  function handleReplyLike1(discussion_id, discid, index) {
    axiosInstance.delete(`${ipaddress}/DiscussionRepliesLikes/${userdata.user_id}/${discussion_id}/`)
      .then((r) => { getreplies(discid, index); })
  ...
  function handleReplyLike(discussion_id, discid, index) {
    axiosInstance.post(`${ipaddress}/DiscussionRepliesLikes/${userdata.user_id}/${discussion_id}/`)
      .then((r) => { getreplies(discid, index) })
      .catch((err) => { console.log("User Reply like error", err) })
  }
  ...
  const sendEditedData = (e) => {
    ...
    axiosInstance.put(`${ipaddress}/CourseDiscussionEdit/${userdata.user_id}/${course_id}/${discussionId}/`, formData)
      .then((r) => {
        ...
        setindex1(-1);
      }).catch(() => { console.log("Post Editing Error") })
  };
  ...
  const getreplies = (discussion_id, index) => {
    axiosInstance.get(`${ipaddress}/coursecommentsreplies/${discussion_id}/${userdata.user_id}/`)
      .then((r) => {
        ...
        setCount(count + 1);
      }).catch(() => { console.log("coursecommentsreplies Error") })
  }
  ...
  const getreplies_for_reply = (particular_reply_id) => {
    axiosInstance.get(`${ipaddress}/CourseDiscussionRepliesReply/${userdata.user_id}/${particular_reply_id}/`)
      ...
      }).catch(() => { console.log("CourseDiscussionRepliesReply Error") })
  }
  ...
  const deleteReply = (disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/CourseDiscussionReplydelete/${disc_reply_id}/`)
      .then((r) => {
        ...
        getreplies(discid);
      }).catch(() => { console.log("Reply Delete Error") })
  }
  ...
  const deleteReply_for_reply = (reply_reply_id, disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/CourseDiscussionRepliesReply/${userdata.user_id}/${reply_reply_id}/`)
      ...
      }).catch((err) => { console.log("Replies reply Delete Error", err) })
  }
  // -------------------------------------------------SEARCH COMMENTS-------------------------------------------------------
  ...
  const filterfunction = (value) => {
    if (value === "select_filter") {
      ...
      axiosInstance.get(value).then((r) => {
  ...
  const filterfunction2 = (category, value) => {
    axiosInstance.post(`${ipaddress}/DocumentsBasedOnCategory/${userdata.user_id}/${category}/${course_id}/`, { 'sort_by': value })
      .then((r) => {
        setFiltereddocuments(r.data.documents);
        document.getElementById('original').style.display = 'none';
        document.getElementById('filterlike').style.display = 'block';
      }).catch((err) => { console.log("DocumentsBasedOnCategory Error", err) })
  }
  ...
  const filterflashsets = (value) => {
    if (value === "likes") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFilters/${userdata.user_id}/${course_id}/`, { 'like': true, 'datetime': false })
        .then((r) => {
          setSearchedFlashsets(r.data);
          document.getElementById('flashsetoriginal').style.display = 'none';
        }).catch((err) => { console.log("FlashsetsFilters Error", err) })
    }
    if (value === "date") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFilters/${userdata.user_id}/${course_id}/`, { 'like': false, 'datetime': true })
        .then((r) => {
          setSearchedFlashsets(r.data);
          document.getElementById('flashsetoriginal').style.display = 'none';
        }).catch((err) => { console.log("FlashsetsFilters Error", err) })
  ...
  const filterpublicflashsets = (value) => {
    if (value === "likes") {
      axiosInstance.post(`${ipaddress2}/PublicFlashsetsFilters/${userdata.user_id}/${course_id}/`, { 'like': true, 'datetime': false })
        .then((r) => {
          setSearchedFlashsets(r.data);
          document.getElementById('flashsetoriginal').style.display = 'none';
        }).catch((err) => { console.log("PublicFlashsetsFilters Error", err) })
    }
    if (value === "date") {
      axiosInstance.post(`${ipaddress2}/PublicFlashsetsFilters/${userdata.user_id}/${course_id}/`, { 'like': false, 'datetime': true })
        .then((r) => {
          setSearchedFlashsets(r.data);
          document.getElementById('flashsetoriginal').style.display = 'none';
        }).catch((err) => { console.log("PublicFlashsetsFilters Error", err) })
  ...
  const searchDocuments = (value) => {
    if (value.length > 0) {
      axiosInstance.post(`${ipaddress}/SearchCourseFlashcardsAndDocumentsUnderCourse/${userdata.user_id}/${course_id}/${value}/`)
        ...
        }).catch((err) => { console.log("SearchCourseFlashcardsAndDocumentsUnderCourse Error", err) })
    } else {
  ...
  const searchFlashset = (value) => {
    if (value.length > 0) {
      axiosInstance.get(`${ipaddress2}/SearchFlashsetsCourse/${userdata.user_id}/${course_id}/${value}/`)
        ...
        }).catch(() => { console.log("Search flashset error") })
    } else {
      setSearchedFlashsets([]);
      document.getElementById('flashsetoriginal').style.display = 'block';
    }
  ...
  const searchpublicFlashset = (value) => {
    if (value.length > 0) {
      axiosInstance.get(`${ipaddress2}/SearchPublicFlashsets/${userdata.user_id}/${course_id}/${value}/`)
        .then((r) => {
          setSearchedFlashsets(r.data);
          if (r.data.length > 0) { document.getElementById('flashsetoriginal').style.display = 'none'; }
        }).catch(() => { console.log("Search flashset error") })
    } else {
      setSearchedFlashsets([]);
      document.getElementById('flashsetoriginal').style.display = 'block';
    ...
    axiosInstance.post(`${ipaddress}/coursediscussionpinningview/${userdata.user_id}/${course_id}/`, formdata)
      .then((r) => {
        if (r.data.message === 'Message already pinned') {
          toast.success('Comment Alraedy Followed', { autoClose: 1000 });
          setindex1(-1);
        } else {
          setCount(count + 1);
    ...
    axiosInstance.post(`${ipaddress}/CourseReport/${userdata.user_id}/${course_id}/`, report)
      .then((r) => {
        if (r.data === "Already reported") {
          toast.success('You already reported', { autoClose: 2000 });
          setreportdropdownstate(false);
        }
        toast.success('Successfully reported', { autoClose: 2000 });
        setreportdropdownstate(false);
        fetchcoursedetails();
      }).catch((err) => { console.log("CourseReport Error", err) })
  }
  ...
  const unsave = (doc_id) => {
    axiosInstance.delete(`${ipaddress}/UserStudyListView/${userdata.user_id}/${doc_id}/`)
      .then((r) => { fetchdocuments() })
      .catch((err) => { console.log("UserStudyListView Error", err) })
  }
  ...
  const unpin = (discid) => {
    ...
    axiosInstance.delete(`${ipaddress}/Unpiningcoursecomments/${userdata.user_id}/${course_id}/${discid}/`)
      ...
      }).catch(() => { console.log("discussion id") })
  }
  ...
  useEffect(() => {
    sessionStorage.setItem("selectedSubTab", 'public')
    closeTourGuide();
  }, [])
  ...
  let selectedSubTab = sessionStorage.getItem("selectedSubTab")
  useEffect(() => {
    setTablist("discussion");
    if (selectedTab) {
      setTablist(selectedTab);
      if (selectedTab == "documents") {
        fetchdocuments()
      } else if (selectedTab == "flashcards") {
        setflashsetstatus(selectedSubTab || flashsetstatus);
        getallflashsetsundersubject();
      }
    }else{
      setTablist("discussion");
        ...
        <div className='subject'>
          <div className="d-flex">
            ...
            <div className='w-100' >
              ...
              <div onClick={() => { setcourse_visible(false); setgroup_visible(false); setnavbar_dropdown_visible(false); setstudylist_visible(false) }} className='container'>
                <Navpath type={"group"} navPaths={nav_paths} />
                <div onClick={() => { setindex1(-1) }} className="mt-1 d-flex flex-lg-row justify-content-between px-4 py-4 position-relative" style={{ backgroundColor: '#F3F0FF' }}>
                  <div>
                    <p onClick={() => { setreportdropdownstate(false) }} className='mb-3 d-flex align-items-center' style={{ color: '#2A3941', fontSize: '20px', fontWeight: 500, lineHeight: 'normal', letterSpacing: '0.4px', fontSize: '14px' }}>
                      ...
                      <span className='ms-2'>{courseDetails.course}</span>
                    </p>
                    <div onClick={() => { setreportdropdownstate(false) }} className='d-flex gap-4 mb-3 align-items-center' style={{ color: '#2A3941' }}>
                      <span className='d-flex align-items-center'>
                        ...
                        <span className='ms-2' style={{ fontSize: '14px' }}>{courseDetails.count} <span className='d-md-inline d-none'>Students</span></span>
                      </span>
                      <span className={`d-flex align-items-center`}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                        ...
                        <span className='ms-1' style={{ fontSize: '14px' }}>{courseDetails.docuemnts_count} <span className='d-md-inline d-none'>Documents</span></span>
                      </span>
                      <span className='d-flex  align-items-center'>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                        ...
                        <span className='ms-2' style={{ fontSize: '14px' }}>{courseDetails.public_flashsets_count} <span className='d-md-inline d-none'>Flashsets</span></span>
                      </span>
                    </div>
                    <div onClick={() => { setreportdropdownstate(false) }} className='d-flex gap-3 find-more-upload-btn-div'>
                      <Link to='/courses' className='btn btn-find-more border px-3 py-2 fw-medium text-decoration-none d-flex align-items-center text-white' style={{ height: '44px', backgroundColor: '#5D5FE3', fontSize: '15px' }}>
                        ...
                        <span className='ms-2' style={{ fontSize: '14px' }}>{translate_value.common_words.join_course}</span>
                      </Link>
                      <Link id="Subject__Upload" state={{backNavPath : nav_paths}} to={`/uploadpage/${course_id}/${course_name}`} className='btn btn-upload px-4 py-2 d-none d-lg-block text-decoration-none fw-medium d-flex align-items-center' style={{ height: '44px', color: '#5D5FE3', fontSize: '15px', border: '1px solid #5D5FE3' }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                        ...
                        <span className='ms-2  fw-bold' style={{ fontSize: '14px' }}>{translate_value.common_words.upload}</span>
                      </Link>
                    </div>
                  </div>
                  <div className='d-flex justify-content-between mb-lg-3 align-self-start'>
                    {/* --------------------------------------------Path Direction flow--------------------------------------------------- */}
                    <div className='d-flex gap-3 align-items-center'>
                      <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Exit the Subject")}>
                        <button className='p-1 border-0 bg-transparent' data-bs-toggle="modal" data-bs-target="#leavecourse">
                          <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="#5D60E1" className="bi bi-box-arrow-left" viewBox="0 0 16 16">
                      ...
                      <div className="position-relative">
                        <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Report the Subject")}>
                          <span className={``} style={{ cursor: 'pointer', position: 'relative', color: courseDetails.course_report_status ? 'blue' : '#5D60E1' }} onClick={() => { setreportdropdownstate(!reportdropdownstate) }}>
                        ...
                        <ul className={`${reportdropdownstate ? '' : 'd-none'} rounded bg-white border-0 py-2 px-2 report-dropdown1`} style={{ width: '180px', listStyleType: 'none', position: 'absolute', top: 0, right: '30px' }}>
                          <li className={`${courseDetails.course_report_status ? 'd-none' : ''}`}>
                            <a data-bs-toggle="modal" data-bs-target="#reportsubjectmodal" className="dropdown-item d-flex align-items-center" href="#">
                              ...
                              <span className='ms-2'>Report Irrelevant</span>
                            </a>
                          </li>
                          <li className={`mt-2 ${courseDetails.course_report_status ? 'd-none' : ''}`}>
                            <a data-bs-toggle="modal" data-bs-target="#reportsubjectmodal" className="dropdown-item d-flex align-items-center" href="#">
                              ...
                              <span className='ms-2'>Report Invalid</span>
                            </a>
                          </li>
                          <li className={`${courseDetails.course_report_status ? '' : 'd-none'}`} style={{ color: '#ff845d' }}>
                            <a className="dropdown-item d-flex align-items-center" href="#">
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-bug" viewBox="0 0 16 16">
                                <path d="M4.355.522a.5.5 0 0 1 .623.333l.291.956A5 5 0 0 1 8 1c1.007 0 1.946.298 2.731.811l.29-.956a.5.5 0 1 1 .957.29l-.41 1.352A5 5 0 0 1 13 6h.5a.5.5 0 0 0 .5-.5V5a.5.5 0 0 1 1 0v.5A1.5 1.5 0 0 1 13.5 7H13v1h1.5a.5.5 0 0 1 0 1H13v1h.5a1.5 1.5 0 0 1 1.5 1.5v.5a.5.5 0 1 1-1 0v-.5a.5.5 0 0 0-.5-.5H13a5 5 0 0 1-10 0h-.5a.5.5 0 0 0-.5.5v.5a.5.5 0 1 1-1 0v-.5A1.5 1.5 0 0 1 2.5 10H3V9H1.5a.5.5 0 0 1 0-1H3V7h-.5A1.5 1.5 0 0 1 1 5.5V5a.5.5 0 0 1 1 0v.5a.5.5 0 0 0 .5.5H3c0-1.364.547-2.601 1.432-3.503l-.41-1.352a.5.5 0 0 1 .333-.623M4 7v4a4 4 0 0 0 3.5 3.97V7zm4.5 0v7.97A4 4 0 0 0 12 11V7zM12 6a4 4 0 0 0-1.334-2.982A3.98 3.98 0 0 0 8 2a3.98 3.98 0 0 0-2.667 1.018A4 4 0 0 0 4 6z" />
                              </svg>
                              <span className='ms-2'>Already Reported</span>
                            </a>
                          </li>
                        </ul>
                      </div>
              ...
              <div onClick={() => { setcourse_visible(false); setgroup_visible(false); setnavbar_dropdown_visible(false); setstudylist_visible(false) }} className='container main-division'>
                <div>
                  {/* ----------------------------------------------------DISCUSSION AND DOCUMENT LAYOUT--------------------------------- */}
                  <div className="m-0 mt-3 mb-5 row justify-content-center">
                    <div className="col-lg-9 ps-lg-0 comments-div">
                      <div id="Subject__Tabs" onClick={() => { setindex1(-1); setreportdropdownstate(false); }} className='px-3 py-3 py-mb-1 rounded shadow-sm border'>
                        <ul className="nav nav-underline gap-2 w-100 w-md-50 mb-3" style={{ backgroundColor: 'transparent' }} id="myTab" role="tablist">
                          <li className="nav-item" role="presentation">
                            <button className={`nav-link fw-normal p-2 ${tablist === "discussion" ? 'active' : ''}`} id="home-tab" data-bs-toggle="tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="home-tab-pane" aria-selected="true" style={{ color: tablist === "discussion" ? '#5D5FE3' : '#8E9696', fontSize: '16px', backgroundColor: 'transparent' }}
                              onClick={() => { setTablist("discussion") }}>Discussion ({discussions.length})</button>
                          </li><li className="nav-item" role="presentation">
                            <button className={`nav-link p-2 fw-normal ${tablist === "documents" ? 'active' : ''}`} id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane" aria-selected="false" style={{ color: tablist === "documents" ? '#5D5FE3' : '#8E9696', fontSize: '16px', backgroundColor: 'transparent' }}
                              onClick={() => { setTablist("documents"); fetchdocuments(); }}>Documents ({courseDetails.docuemnts_count})</button>
                          </li>
                          <li className="nav-item" role="presentation">
                            <button className={`nav-link p-2 fw-normal ${tablist === "flashcards" ? 'active' : ''}`} id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false" style={{ color: tablist === "flashcards" ? '#5D5FE3' : '#8E9696', fontSize: '16px', backgroundColor: 'transparent' }}
                              onClick={() => { setTablist("flashcards"); getallflashsetsundersubject(); }}>Flashsets ({courseDetails.public_flashsets_count})</button>
                          </li>
                        </ul>
                        <div className='w-100 ps-0 mt-md-0'>
                          {/* ------------------------------------------------Document Search bar-------------------------------------------- */}
                          <div className={`input-group my-3 bg-white rounded border ${tablist === "documents" ? '' : 'd-none'}`}>
                            ...
                            <input type="text" className="form-control bg-transparent border-0 ps-0 sub-search" placeholder={translate_value.common_words.search}
                              onChange={(e) => { searchDocuments(e.target.value) }} aria-label="Username" aria-describedby="basic-addon1" style={{ fontSize: '14px' }} />
                          </div>
                          {/* --------------------------------------------------Flashcards Search bar------------------------------------------- */}
                          <div className={`input-group bg-white rounded border ${tablist === "flashcards" && flashsetstatus === "myflashcards" ? '' : 'd-none'}`}>
                            <span className="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                              <path d="M20.2965 20.9936L13.774 14.4712C13.2532 14.9145 12.6542 15.2577 11.9771 15.5008C11.3001 15.7439 10.6196 15.8654 9.93586 15.8654C8.26746 15.8654 6.85544 15.2879 5.6998 14.1329C4.54414 12.9778 3.96631 11.5666 3.96631 9.89908C3.96631 8.23158 4.54382 6.81924 5.69883 5.66205C6.85385 4.50486 8.26511 3.92627 9.93261 3.92627C11.6001 3.92627 13.0125 4.50409 14.1696 5.65973C15.3268 6.81538 15.9054 8.22741 15.9054 9.89583C15.9054 10.6196 15.7772 11.3201 15.5208 11.9972C15.2644 12.6743 14.9279 13.2532 14.5112 13.734L21.0336 20.2564L20.2965 20.9936ZM9.93586 14.8237C11.3181 14.8237 12.485 14.348 13.4365 13.3964C14.388 12.4449 14.8638 11.278 14.8638 9.89583C14.8638 8.51362 14.388 7.34676 13.4365 6.39523C12.485 5.4437 11.3181 4.96794 9.93586 4.96794C8.55366 4.96794 7.38679 5.4437 6.43527 6.39523C5.48376 7.34676 5.008 8.51362 5.008 9.89583C5.008 11.278 5.48376 12.4449 6.43527 13.3964C7.38679 14.348 8.55366 14.8237 9.93586 14.8237Z" fill="#8E9696" />
                            </svg></span>
                            <input type="text" value={searchflashsetname} className="form-control border-0 ps-0 sub-search bg-transparent" placeholder={translate_value.common_words.flashset}
                              onChange={(e) => { setsearchflashsetname(e.target.value); searchFlashset(e.target.value); }} aria-label="Username" aria-describedby="basic-addon1" style={{ fontSize: '14px' }} />
                          </div>

                          <div className={`input-group bg-white my-3 rounded border ${tablist === "flashcards" && flashsetstatus === "public" ? '' : 'd-none'}`}>
                            <span className="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                              <path d="M20.2965 20.9936L13.774 14.4712C13.2532 14.9145 12.6542 15.2577 11.9771 15.5008C11.3001 15.7439 10.6196 15.8654 9.93586 15.8654C8.26746 15.8654 6.85544 15.2879 5.6998 14.1329C4.54414 12.9778 3.96631 11.5666 3.96631 9.89908C3.96631 8.23158 4.54382 6.81924 5.69883 5.66205C6.85385 4.50486 8.26511 3.92627 9.93261 3.92627C11.6001 3.92627 13.0125 4.50409 14.1696 5.65973C15.3268 6.81538 15.9054 8.22741 15.9054 9.89583C15.9054 10.6196 15.7772 11.3201 15.5208 11.9972C15.2644 12.6743 14.9279 13.2532 14.5112 13.734L21.0336 20.2564L20.2965 20.9936ZM9.93586 14.8237C11.3181 14.8237 12.485 14.348 13.4365 13.3964C14.388 12.4449 14.8638 11.278 14.8638 9.89583C14.8638 8.51362 14.388 7.34676 13.4365 6.39523C12.485 5.4437 11.3181 4.96794 9.93586 4.96794C8.55366 4.96794 7.38679 5.4437 6.43527 6.39523C5.48376 7.34676 5.008 8.51362 5.008 9.89583C5.008 11.278 5.48376 12.4449 6.43527 13.3964C7.38679 14.348 8.55366 14.8237 9.93586 14.8237Z" fill="#8E9696" />
                            </svg></span>
                      ...
                      <div className="tab-content" id="myTabContent">
                        ...
                        <div className={`tab-pane fade py-1 px-0 ${tablist === "discussion" ? 'show active' : ''}`} style={{ backgroundColor: 'transparent' }} id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabIndex="0">
                          <div className='w-100 mt-3'>
                            <div className='row' onClick={() => {
                              setindex1(-1)
                              setreportdropdownstate(false)
                            }}>
                              <div className='d-flex col-lg-8 col-sm-7 tabs'>
                                <span className='d-flex align-items-center message'>
                                ...
                                <span className='d-flex align-items-center pinned'>
                              ...
                              <div className='mt-md-0 col-lg-4 col-sm-5'>
                                <div className="input-group bg-white rounded border">
                                  <span className="input-group-text bg-transparent border-0" id="basic-addon1"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                                ...
                                <div className="modal-content">
                                  <div className="modal-body">
                                    <div className='col-12 bg-white px-lg-3 pt-2 pb-3'>
                                      ...
                                      <form onSubmit={(e) => postQuestion(e)} className='w-100'>
                                        <div className='d-flex gap-1 gap-lg-3 align-items-center'>
                                          <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub" />
                                          {userdetails.nickname != undefined ?
                                            (<div className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'}
                                          ...
                                          <div className="input-group bg-white border rounded pe-lg-3">
                                            <input
                                              type="text"
                                              name="question"
                                              value={question}
                                              ...
                                              // value={editedpost}
                                              className="form-control py-3 ps-3 bg-white border-0 shadow-none post-input" placeholder="Post your Comment....."
                                            ...
                                            <div className='d-flex align-items-center bg-white'>
                                              <input
                                                ...
                                                className="bg-white text-center p-3 btn"
                                              />
                                              ...
                                              <button disabled={question.length > 0 ? false : true} data-bs-dismiss="modal" className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' >
                                      ...
                                      <div className='d-flex gap-3 mt-3'>
                                        {selectedImages.length > 0 &&
                                          selectedImages.map((image, index) => (
                                            <div key={index} className="image-preview bg-white p-2" style={{ position: 'relative' }}>
                          ...
                          <div className="pe-lg-0 d-lg-none create-post-div" onClick={() => { setindex1(-1); setreportdropdownstate(false) }}>
                            <div onClick={() => { setindex1(-1); setreportdropdownstate(false) }} className="rounded d-flex flex-column align-items-center mb-lg-3">
                              ...
                              <Link to={`/uploadpage/${course_id}/${course_name}`} className={`btn mt-lg-3 w-100 fw-medium ${tablist === "documents" ? "d-block" : "d-none"}`} style={{ border: '1px solid #5D5FE3', color: '#5D5FE3', fontWeight: 500 }}>{translate_value.common_words.upload_document}</Link>
                              <Link to='/flashcard' className={`btn mt-lg-3 w-100 fw-medium ${tablist === "flashcards" ? "d-block" : "d-none"}`} style={{ border: '1px solid #5D5FE3', color: '#5D5FE3', fontWeight: 500 }}>{translate_value.common_words.create_flashcard}</Link>
                            </div>
                          </div>
                          ...
                          <div className={`${(pinnedcomment_status && !searchcomment.length) ? '' : 'd-none'}`} onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                          ...
                          <div className={`${(usercomment_status && !searchcomment.length) ? '' : 'd-none'}`} onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                            ...
                            {discussions.slice(0, visibleDiscussions).map((x, index) => {
                              return (
                                <div className='mt-3' key={index}>
                                  <div className='col-12 bg-white rounded shadow-sm border mb-3'>
                                    <div className="d-flex ps-2 justify-content-between border-bottom py-2 m-0 align-items-center">
                                      <div className='d-flex gap-2 align-items-center'>
                                        <div className="d-flex justify-content-center" onClick={() => { setindex1(-1); setreportdropdownstate(false) }}>
                                          <img src={x.user_details.profile_pic} className={x.user_details.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub" />
                                          <div className={x.user_details.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{x.user_details.nickname.slice(0, 1)}</span><span>{x.user_details.nickname.slice(-1)}</span></div>
                                        </div>
                                        <div onClick={() => { setindex1(-1); setreportdropdownstate(false) }}>
                                          <Link to={`/profile/${x.user_details.user_id}`} className='my-0 fw-medium text-decoration-none text-dark' style={{ fontSize: '14px' }}>{x.user_details.nickname}
                                            <span className='fw-normal ms-2 text-secondary' style={{ fontSize: '13px' }}>{x.created_at}</span>
                                          </Link>
                                          <p className='ms-sm-0 my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA' }}>
                                            {user.program_name}</p>

                                        </div>
                                      </div>

                                      <div className={`col-3 ps-4 d-flex gap-2 align-items-center p-0 ${x.pinned_status ? 'col-md-3 justify-content-end' : 'col-md-2 justify-content-end'}`}>
                                        ...
                                        <button data-bs-toggle="tooltip" data-bs-placement="top"
                                          data-bs-custom-class="custom-tooltip"
                                          data-bs-title="Unfollow" onClick={() => { unpin(x.discid) }} className={`p-1 btn btn-sm border px-2 ${x.pinned_status ? '' : 'd-none'}`} style={{ cursor: 'pointer' }}><i className="fa-solid fa-link-slash d-md-none d-inline"></i><span className='d-none d-md-inline'>Followed</span></button>
                                        ...
                                        <div className="btn-group dropstart">
                                          ...
                                          <ul className={`bg-white shadow-sm border rounded mt-0 p-0 px-3 ${index1 == index ? '' : 'd-none'}`} style={{ width: '160px', position: 'absolute', left: '-160px' }}>
                                            ...
                                            {/* ----------------------------------------------------Report button--------------------------------------------------- */}
                                            {userdata.user_id !== x.user_details.user_id &&
                                              <>
                                              ...
                                              <div className="modal-body">
                                                <div className=' bg-white px-3 pt-2 pb-3 rounded'>
                                                  ...
                                                  <div className='d-flex gap-3'>
                                                    <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub" />
                                                    {userdetails.nickname != undefined ? (<div className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></div>) : (<></>)}
                                                    <form className='w-100' onSubmit={(e) => sendEditedData(e, x.discid)}>
                                                      <div className="input-group border rounded bg-white pe-3">
                                                        <input
                                                          type="text"
                                                          name="question"
                                                          ...
                                                          style={{ position: 'relative' }} />
                                                        <button type='submit' data-bs-dismiss="modal" disabled={editedpost.length > 0 ? false : true} className='text-info bg-transparent border-0'>
                                    ...
                                    <div className='py-2 px-2' onClick={() => { setindex1(-1) }}>
                                      ...
                                      {x.images_attached && (
                                        x.images_attached.map((z) => {
                                          return (
                                            <div>
                                              <img src={z.image} width={300} alt="sub" className='mt-3' />
                                            </div>
                                          )
                                        })
                                      )}
                                    ...
                                    <div className='d-flex justify-content-between border-bottom pb-2 px-2 mb-0' onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                                      <div className='d-flex align-items-center gap-2'>
                                        ...
                                        <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: x.dis_like_status ? "#ff845d" : "gray" }} onClick={() => {
                                      ...
                                      {x.replies_count > 0 ?
                                        (!replies_layout) ?
                                          <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_layout(!replies_layout); setSelectedReply(index); getreplies(x.discid, index) }}>
                                            {translate_value.dashboard.view_all} {x.replies_count} {translate_value.dashboard.replies}
                                          </button>
                                          :
                                          (selectedReply == index) ?
                                        ...
                                        {fetchedreplies.map((y, index2) => {
                                          return (
                                            <div className='py-2 px-2' onClick={() => { setindex1(-1) }}>
                                              <div className="w-100 d-flex justify-content-between px-2">
                                                <div className='d-flex align-items-center gap-2'>
                                                  <div className="d-flex justify-content-end">
                                                    <img src={y.user.profile_pic} className={y.user.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="sub" />
                                                ...
                                                <div className="p-0 d-flex gap-3">
                                                  <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: y.liked_status ? "#ff845d" : "gray" }} onClick={() => {
                                                    (y.liked_status == true) ? handleReplyLike1(y.disrid, x.discid, index) : handleReplyLike(y.disrid, x.discid, index)
                                                  }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                      <path d="M21.8269 25.0002H9.27884V11.2502L17.1154 3.50977L17.6683 4.06267C17.7821 4.17644 17.8778 4.32228 17.9555 4.50017C18.0333 4.67805 18.0721 4.84151 18.0721 4.99055V5.18767L16.7933 11.2502H25.4808C26.0032 11.2502 26.4704 11.4561 26.8822 11.868C27.2941 12.2798 27.5 12.747 27.5 13.2694V14.8079C27.5 14.9217 27.4872 15.0459 27.4615 15.1805C27.4359 15.3151 27.4023 15.4393 27.3606 15.553L23.9471 23.6444C23.7756 24.029 23.4872 24.3511 23.0818 24.6108C22.6763 24.8704 22.258 25.0002 21.8269 25.0002ZM10.5288 23.7502H21.8269C22.0032 23.7502 22.1835 23.7021 22.3678 23.606C22.5521 23.5098 22.6923 23.3495 22.7885 23.1252L26.25 15.0002V13.2694C26.25 13.045 26.1779 12.8607 26.0337 12.7165C25.8894 12.5723 25.7051 12.5002 25.4808 12.5002H15.2404L16.6875 5.67324L10.5288 11.7838V23.7502ZM9.27884 11.2502V12.5002H5V23.7502H9.27884V25.0002H3.75V11.2502H9.27884Z" fill="currentColor" />
                                                    </svg> <span className='ms-1'>{y.like_count}</span>
                                                  </button>

                                                  <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: y.dis_liked_status ? "#ff845d" : "gray" }} onClick={() => { handlereplydislike(y.disrid, x.discid, index) }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                      <path d="M8.17306 4.99983H20.7212V18.7498L12.8846 26.4902L12.3317 25.9373C12.2179 25.8236 12.1222 25.6777 12.0445 25.4998C11.9667 25.322 11.9279 25.1585 11.9279 25.0095V24.8123L13.2067 18.7498H4.51922C3.99678 18.7498 3.52963 18.5439 3.11778 18.132C2.70593 17.7202 2.5 17.253 2.5 16.7306V15.1921C2.5 15.0783 2.51282 14.9541 2.53847 14.8195C2.56409 14.6849 2.59774 14.5607 2.63941 14.447L6.05288 6.35558C6.22435 5.97097 6.51281 5.64886 6.91825 5.38923C7.32371 5.12963 7.74198 4.99983 8.17306 4.99983ZM19.4712 6.24983H8.17306C7.99679 6.24983 7.81651 6.2979 7.63222 6.39404C7.44793 6.49021 7.3077 6.65047 7.21153 6.87483L3.75 14.9998V16.7306C3.75 16.955 3.82211 17.1393 3.96634 17.2835C4.11057 17.4277 4.29486 17.4998 4.51922 17.4998H14.7596L13.3125 24.3268L19.4712 18.2162V6.24983ZM20.7212 18.7498V17.4998H25V6.24983H20.7212V4.99983H26.25V18.7498H20.7212Z" fill="currentColor" />
                                                    </svg>
                                                  </button>
                                                  <button className={userdata.user_id === y.user.user_id ? 'bg-transparent border-0 ms-1 d-flex align-items-center' : 'd-none'} onClick={() => { deleteReply(y.disrid, x.discid) }} style={{ height: '20px' }}>
                                              ...
                                              <div className='ps-5 mt-2' style={{ width: '100%', overflowWrap: 'break-word', overflowX: 'hidden' }}>
                                                <p className='m-0' style={{ fontSize: '14px' }}>{y.post}</p>
                                                {y.images_attached.map((a) => {
                                                  return (
                                                    <div className='d-flex justify-content-center'>
                                                      <img src={a.doc_reply_images} width={300} alt="sub" className='mt-3' />
                                                    </div>
                                                  )
                                                })}
                                              </div>
                                              ...
                                              <div className="mt-2 ps-5">
                                                ...
                                                {!replies_for_reply_status ?
                                                  <p style={{ cursor: 'pointer' }} className={`view_reply_for_reply mt-2 mb-0 ${y.replies_replies_count > 0 ? '' : 'd-none'}`} onClick={() => { setreplies_for_reply_status(!replies_for_reply_status); getreplies_for_reply(y.disrid); setSelectedReplyOfReply(index2) }}>
                                                    ---View {y.replies_replies_count} replies
                                                  </p>
                                                  :
                                                  selectedReplyOfReply == index2 ?
                                                    <p style={{ cursor: 'pointer' }} className={`view_reply_for_reply mt-2 mb-0 ${y.replies_replies_count > 0 ? '' : 'd-none'}`} onClick={() => { setreplies_for_reply_status(!replies_for_reply_status); getreplies_for_reply(y.disrid); setSelectedReplyOfReply(null) }}>
                                                      {'View less'}
                                                    </p>
                                                    :
                                                    <p style={{ cursor: 'pointer' }} className={`view_reply_for_reply mt-2 mb-0 ${y.replies_replies_count > 0 ? '' : 'd-none'}`} onClick={() => { setreplies_for_reply_status(!replies_for_reply_status); getreplies_for_reply(y.disrid); setSelectedReplyOfReply(index2) }}>
                                                        ...
                                                        <div className='py-2 px-2' onClick={() => { setindex1(-1) }} >
                                                          <div className="w-100 d-flex justify-content-between align-items-center">
                                                            <div className='d-flex gap-2'>
                                                              <div className="d-flex justify-content-center">
                                                                <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="sub" />
                                                            ...
                                                            <div className="d-flex gap-3 justify-content-between align-items-center">
                                                              <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: z.liked_status ? "#ff845d" : "gray" }} onClick={() => { handleReplies_reply_like(z.id, y.disrid) }}>
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                                ...
                                                                </svg> <span className="ms-1">{z.likes_count}</span></button>
                                                              <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: z.dis_like_status ? "#ff845d" : "gray" }} onClick={() => { handlereplies_replydislike(z.id, y.disrid) }}>
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                              ...
                                                              <button className={userdata.first_name === z.user_id.first_name ? 'bg-transparent border-0 d-flex align-items-center' : 'd-none'} onClick={() => { deleteReply_for_reply(z.id, y.disrid, x.discid) }} style={{ height: '20px' }}>
                                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="18" viewBox="0 0 18 20" fill="none">
                                                          ...
                                                          <div className='ps-5 mt-2 m-0'>
                                                            <p className='m-0' style={{ fontSize: '14px' }}>{z.post}</p>
                                                            {z.images_attached.map((b) => {
                                                              return (
                                                                <div className='d-flex justify-content-center'>
                                                                  <img src={b.dis_reply_reply_images} width={260} alt="sub" className='mt-3' />
                                                                </div>
                                                              )
                                                            })}
                                                          </div>
                                    ...
                                    <div onClick={() => { setindex1(-1) }} className='d-flex gap-2 py-2 px-2 border-secondary-subtle align-items-center'>
                                      <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub" />
                                      {userdetails.nickname != undefined ? (<div className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ fontSize: '14px', height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></div>) : (<></>)}
                                      <form onSubmit={(e) => postReplies(e, x.discid, index)} className='w-100'>
                                        <div className="input-group border rounded pe-lg-3 bg-white">
                                          <input key={index} type="text" name={post} onChange={(e) => { repliesData(e); setSelectedPostForComment(x.discid) }} style={{ position: 'relative' }}
                                            className="form-control py-2 ps-3 shadow-none border-0 bg-light reply-input" placeholder={translate_value.dashboard.reply_here} />
                                          <div className='d-flex align-items-center bg-light'>
                                            <input id="file" type='file' name='file' accept="image/*" multiple onChange={handleReplyImage} className="bg-white text-center p-3 btn" />
                                            <label data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Attach Image" htmlFor="file" className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                                              <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                            ...
                                            <button type='submit' disabled={replies.length > 0 ? false : true} className='h-100 bg-transparent border-0 ms-2'>
                                              <div style={{ width: '30px' }} className={`${(load && selectedPostForComment == x.discid) ? '' : 'd-none'}`}>
                                                <div className={`spinner-border spinner-border-sm`} role="status">
                                                  <span className="visually-hidden">Loading...</span>
                                                </div>
                                              </div>
                                              <svg data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Post" xmlns="http://www.w3.org/2000/svg"
                                                className={`${(load && selectedPostForComment == x.discid) ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696" />
                                              </svg>
                                            </button>
                                          </div>
                                          ...
                                          <div className="modal-content">
                                            <div className="modal-body">
                                              <div className='text-center'>
                                                <input id="file" type='file' name='file' accept="image/*" multiple onChange={handleReplyImage} className="bg-white text-center p-3 btn" />
                                                <label htmlFor='file' className="custom-file-input rounded-pill border-0 bg-primary bg-gradient text-white px-4 py-2">Upload Image</label>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                    ...
                                    {(repliesImage && repliesImage.length > 0) &&
                                      <div className='d-flex gap-3 mt-3'>
                                        {
                        ...
                        <div className={`tab-pane fade bg-light mt-2 ${tablist === "documents" ? 'show active' : ''}`} id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabIndex="0">
                              ...
                              {subloading ? (<Small_Preloader />) : (
                                <div className="row">
                                  <div className="col-12">
                                    ...
                                    <div className='d-flex flex-column flex-lg-row justify-content-between  py-3 align-items-lg-center'>
                                      <span className='fw-bold' style={{ color: '#8E9696' }}>{courseDocuments.length} Documents</span>
                                      <div>
                                        <select name="" id="filter" className='border py-2 px-4 rounded bg-white' onChange={(e) => {
                                        ...
                                        <select name="" id="filter" className={`border border py-2 px-2 px-md-3 rounded bg-white ms-2 ${selectedCategory === "select_filter1" ? 'd-none' : ''}`} onChange={(e) => {
                                    ...
                                    <div id='original'>
                                      {courseDocuments.map((x) => {
                                        return (
                                          <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded bg-white border'>
                                            <div className="row m-0 align-items-center">
                                              ...
                                              <div className="col-10 col-lg-10 d-flex flex-column justify-content-center ">
                                                <div className="row m-0 border-bottom">
                                                  <div className='d-flex justify-content-between'>
                                                    <Link to={`/showpdf/${x.document_id}`} state={{ backNavPath : nav_paths}} className='w-75 text-decoration-none text-primary' onClick={() => sessionStorage.setItem("selectedTab", 'documents')}>
                                                      <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip(course_name)}>
                                                        <>
                                                          <p className='fw-bold d-none d-md-inline' style={{ fontSize: '16px' }}>{x.file.title}</p>
                                                          <p className='fw-bold d-inline d-md-none' style={{ fontSize: '16px' }}>{x.file.title.slice(0, 16)}</p>
                                                        </>
                                                      </OverlayTrigger>
                                                    </Link>

                                                    <div className='d-flex'>
                                                      {x.is_delete && <span style={{ fontSize: '14px', color: '#AAB0B0' }} className='me-2 d-flex align-items-center'><img className='text-start' src={deleteDoc_icon} alt="Delete Icon" width="20" height="20" /></span>}
                                                      <button onClick={() => { unsave(x.document_id) }} className={`bg-transparent border-0 d-flex align-items-center ${x.study_list_status ? '' : 'd-none'}`} style={{ color: '#8587EA' }}>
                                                        ...
                                                        <span className='ms-2 d-none d-md-inline'>Saved</span>
                                                      </button>
                                                      <button data-bs-toggle="modal" data-bs-target="#studylist_modal" className={`bg-transparent border-0 ${x.study_list_status ? 'd-none' : ''}`} style={{ color: '#8587EA' }} onClick={() => { setdocument_id(x.document_id) }}>
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                                        ...
                                                        </svg> <span className='ms-1 d-none d-md-inline'>Save</span>
                                                      </button>

                                                      <span style={{ fontSize: '14px', color: '#AAB0B0' }} className='ms-3 d-flex align-items-center'>
                                                        ...
                                                        <span className='ms-1'>{x.download_count}</span>
                                                      </span>
                                                    </div>
                                                  </div>
                                                  {/* <p className='mt-1 d-flex align-items-center' style={{ fontSize: '14px', color: '#5D5FE3' }}>
                                                    <Link to={`/profile/${x.user_info.user_id}`}><img src={x.user_info.profile_pic} width={28} height={28} className={`me-1 rounded-circle ${x.user_info.profile_pic != null ? '' : 'd-none'}`} /></Link>
                                                    {x.user_info.nickname != undefined ? (<p className={x.user_info.profile_pic == null ? 'd-flex justify-content-center bg-warning text-white rounded-circle my-auto align-items-center me-2' : 'd-none'} style={{ width: '30px', height: '30px', fontSize: '15px' }}><span>{x.user_info.nickname.slice(0, 1)}</span><span>{x.user_info.nickname.slice(-1)}1</span></p>) : (<></>)}
                                                    <Link to={`/profile/${x.user_info.user_id}`}>{x.user_info.nickname}</Link> <span style={{ color: '#8E9696' }} className='ms-1'>{x.created_on}</span>
                                                  </p> */}
                                                  <p className="mt-1 d-flex align-items-center" style={{ fontSize: '14px', color: '#5D5FE3' }}>
                                                    {x.user_info.profile_pic && (
                                                      <Link to={`/profile/${x.user_info.user_id}`}><img src={x.user_info.profile_pic} width={28} height={28} className="me-1 rounded-circle" /></Link>
                                                    )}
                                                    {!x.user_info.profile_pic && x.user_info.nickname && (
                                                      <div className="d-flex justify-content-center bg-warning text-white rounded-circle my-auto align-items-center me-2" style={{ width: '30px', height: '30px', fontSize: '15px' }}>
                                                        <Link to={`/profile/${x.user_info.user_id}`} className='text-decoration-none'>
                                                          <span className='text-white'>{x.user_info.nickname[0]}</span>
                                                          <span className='text-white'>{x.user_info.nickname.slice(-1)}</span>
                                                        </Link>
                                                      </div>
                                                    )}
                                                    <Link to={`/profile/${x.user_info.user_id}`}>{x.user_info.nickname || 'Unknown User'} </Link>
                                                    <span style={{ color: '#8E9696' }} className="ms-1">{x.created_on}</span>
                                                  </p>
                                                ...
                                                <div className="m-0 d-flex align-items-center mt-2">
                                                  ...
                                                  <span style={{ fontSize: '14px', color: '#AAB0B0' }} className={`ms-3 d-none d-md-flex align-items-center`}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="18" fill="currentColor" className="bi bi-tags" viewBox="0 0 16 16">
                                    ...
                                    <div id='filterlike' style={{ display: 'none' }}>
                                      {filtereddocuments.length <= 0 && (<h6 className='text-center text-secondary mt-3'>No Documents Found !!!</h6>)}
                                      {filtereddocuments.map((x) => {
                                        return (
                                          <div className='shadow-sm mb-3 py-2 px-2 px-lg-0 bg-white'>
                                            <div className="row m-0 align-items-center">
                                              <div className="col-2 col-lg-2 d-flex align-items-center justify-content-center" style={{ overflow: 'hidden' }}>
                                              ...
                                              <div className="col-10 col-md-10 ms-md-0 d-flex flex-column justify-content-center ">
                                                <div className="row m-0 border-bottom">
                                                  <div className='d-flex justify-content-between'>
                                                    <Link to={`/showpdf/${x.document_id}`} className='w-75 text-decoration-none text-primary' onClick={() => sessionStorage.setItem("selectedTab", 'documents')}>
                                                      <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip(course_name)}>
                                                        <>
                                                          <p className='fw-bold d-none d-md-inline' style={{ fontSize: '16px' }}>{x.title}</p>
                                                          <p className='fw-bold d-inline d-md-none' style={{ fontSize: '16px' }}>{x.title.slice(0, 16)}</p>
                                                        </>
                                                      </OverlayTrigger>
                                                    </Link>

                                                    {/* Save document to studylist buttons------------------------------------------- */}
                                                    <button onClick={() => { unsave(x.document_id) }} className={`bg-transparent border-0 ${x.study_list_status ? '' : 'd-none'}`} style={{ color: '#8587EA' }}>
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                                        <path d="M0.25 16.3128V1.84967C0.25 1.37025 0.41059 0.969944 0.731771 0.648763C1.05295 0.327582 1.45325 0.166992 1.93268 0.166992H11.0673C11.5467 0.166992 11.947 0.327582 12.2682 0.648763C12.5894 0.969944 12.75 1.37025 12.75 1.84967V16.3128L6.5 13.6285L0.25 16.3128Z" fill="#5D5FE3" />
                                                      </svg> <span className='ms-2 d-none d-md-inline'>Saved</span></button>
                                                    <button data-bs-toggle="modal" data-bs-target="#studylist_modal" onClick={() => { setdocument_id(x.document_id) }} className={`bg-transparent border-0 ${x.study_list_status ? 'd-none' : ''}`} style={{ color: '#8587EA' }}>
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                                        <path d="M0.25 16.3123V1.84919C0.25 1.36976 0.41059 0.969455 0.731771 0.648275C1.05295 0.327094 1.45325 0.166504 1.93268 0.166504H11.0673C11.5467 0.166504 11.947 0.327094 12.2682 0.648275C12.5894 0.969455 12.75 1.36976 12.75 1.84919V16.3123L6.5 13.628L0.25 16.3123ZM1.29167 14.6978L6.5 12.4582L11.7083 14.6978V1.84919C11.7083 1.68894 11.6416 1.54204 11.508 1.40848C11.3745 1.27494 11.2276 1.20817 11.0673 1.20817H1.93268C1.77244 1.20817 1.62554 1.27494 1.49198 1.40848C1.35844 1.54204 1.29167 1.68894 1.29167 1.84919V14.6978Z" fill="#5D5FE3" />
                                                ...
                                                <div className='d-lg-block d-none'>
                                                  <div className="m-0 d-flex align-items-center mt-2">
                                                    ...
                                                    <span style={{ fontSize: '14px', color: '#AAB0B0' }} className={`ms-3 ${selectedCategory === 'chapter' && x.chapter_name !== "" ? 'd-none d-md-flex align-items-center' : 'd-none'}`}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="18" fill="currentColor" className="bi bi-tags" viewBox="0 0 16 16">
                                                      <path d="M3 2v4.586l7 7L14.586 9l-7-7zM2 2a1 1 0 0 1 1-1h4.586a1 1 0 0 1 .707.293l7 7a1 1 0 0 1 0 1.414l-4.586 4.586a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 2 6.586z" />
                                                      <path d="M5.5 5a.5.5 0 1 1 0-1 .5.5 0 0 1 0 1m0 1a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3M1 7.086a1 1 0 0 0 .293.707L8.75 15.25l-.043.043a1 1 0 0 1-1.414 0l-7-7A1 1 0 0 1 0 7.586V3a1 1 0 0 1 1-1z" />
                        ...
                        {/* --------------------------------------------Flashcards Section----------------------------------------------------- */}
                        <div className={`tab-pane fade bg-white mt-3 ${tablist === "flashcards" ? 'show active' : ''}`} id="contact-tab-pane" role="tabpanel" aria-labelledby="contact-tab" tabIndex="0">
                          <div>
                            {subloading ? (<Small_Preloader />) : (
                              <div className="row">
                                <div className="col-12">
                                  ...
                                  <div className='d-flex justify-content-between flash-tabs-box py-3 align-items-center'>
                                    <div className='d-flex w-100'>
                                      {/* <span className='fw-bold' style={{color:'#8E9696'}}>{flashsets.length} Flashsets</span> */}
                                      <div className={`fw-medium p-1 px-3 py-2 px-md- public-tab`}
                                        style={{ textAlign: 'center', cursor: 'pointer', color: flashsetstatus === "public" ? '#fff' : '', borderRadius: '5px 0px 0px 5px', backgroundColor: flashsetstatus === "public" ? '#5D5FE3' : '', border: '1px solid #5D5FE3' }}
                                        onClick={() => {
                                          setflashsetstatus("public")
                                          getallflashsetsundersubject();
                                          sessionStorage.setItem("selectedSubTab", 'public')
                                        }}>Public</div>
                                      <div className={`fw-medium p-1 px-3 py-2 px-md-2 flashcard-tab`}
                                        style={{ textAlign: 'center', cursor: 'pointer', color: flashsetstatus === "myflashcards" ? '#fff' : '', borderRadius: '0px 5px 5px 0px', backgroundColor: flashsetstatus === "myflashcards" ? '#5D5FE3' : '', border: '1px solid #5D5FE3' }}
                                        onClick={() => { setflashsetstatus("myflashcards"); getparticularusersflashsets(); sessionStorage.setItem("selectedSubTab", 'myflashcards') }}>My Flashcards</div>
                                    </div>

                                    <select name="" id="filter" className={` filter-list border px-4 py-2 rounded bg-white ${flashsetstatus === "myflashcards" ? '' : 'd-none'}`} onChange={(e) => { filterflashsets(e.target.value) }}>
                                      <option value="select_filter" className='mt-2'>All</option>
                                      <option value="likes" className=''>Likes</option>
                                      <option value="date" className=''>Date</option>
                                    </select>
                                    <select name="" id="filter" className={` filter-list border px-4 py-2 rounded bg-white ${flashsetstatus === "public" ? '' : 'd-none'}`} onChange={(e) => { filterpublicflashsets(e.target.value) }}>
                                      <option value="select_filter" className='mt-2'>All</option>
                                      <option value="likes" className=''>Likes</option>
                                      <option value="date" className=''>Date</option>
                                    </select>
                                        ...
                                        <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded border'>
                                          <div className="row m-0 align-items-center ps-2 ps-lg-3">
                                            ...
                                            <div className="col-10 col-lg-10 d-flex flex-column ">
                                              <div className="row m-0 border-bottom">
                                                <div className='d-flex justify-content-between'>
                                                  <Link to={`/viewflashcard/subject/${course_id}/${x.flashset_id}`} onClick={() => sessionStorage.setItem("selectedTab", 'flashcards')} className='fw-bold w-75 text-primary text-decoration-none'>{x.name}</Link>
                                    ...
                                    {searchedFlashsets && (
                                      searchedFlashsets.map((x) => {
                                        return (
                                          <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded border'>
                                            <div className="row m-0 align-items-center">
                                              <div className="col-3 col-lg-2 d-flex align-items-center justify-content-center rounded" >
                                                <div className='d-flex align-items-center justify-content-center' style={{ overflow: 'hidden', backgroundColor: '#CFF4D2', height: '100px', width: '100px', border: '0.5px solid #21B3A9' }}>
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 50 50" fill="none" >
                                                    <path d="M8.28125 41.2501L6.51042 40.5209C5.43403 40.0696 4.71354 39.2883 4.34896 38.1772C3.98438 37.0661 4.04514 35.9723 4.53125 34.8959L8.28125 26.7709V41.2501ZM16.6146 45.8334C15.4688 45.8334 14.4879 45.4255 13.6719 44.6095C12.8559 43.7935 12.4479 42.8126 12.4479 41.6668V29.1668L17.9688 44.4793C18.0729 44.7223 18.1771 44.9567 18.2813 45.1824C18.3854 45.4081 18.5243 45.6251 18.6979 45.8334H16.6146ZM27.3438 45.6251C26.2326 46.0418 25.1563 45.9897 24.1146 45.4689C23.0729 44.948 22.3438 44.1321 21.9271 43.0209L12.6563 17.6043C12.2396 16.4932 12.2743 15.4081 12.7604 14.3491C13.2465 13.29 14.0451 12.5696 15.1563 12.1876L30.8854 6.45844C31.9965 6.04178 33.0729 6.09386 34.1146 6.61469C35.1563 7.13553 35.8854 7.9515 36.3021 9.06261L45.5729 34.4793C45.9896 35.5904 45.9549 36.6755 45.4688 37.7345C44.9826 38.7935 44.184 39.514 43.0729 39.8959L27.3438 45.6251ZM22.8646 20.8334C23.4549 20.8334 23.9497 20.6338 24.349 20.2345C24.7483 19.8352 24.9479 19.3404 24.9479 18.7501C24.9479 18.1598 24.7483 17.665 24.349 17.2657C23.9497 16.8664 23.4549 16.6668 22.8646 16.6668C22.2743 16.6668 21.7795 16.8664 21.3802 17.2657C20.9809 17.665 20.7813 18.1598 20.7813 18.7501C20.7813 19.3404 20.9809 19.8352 21.3802 20.2345C21.7795 20.6338 22.2743 20.8334 22.8646 20.8334Z" fill="#21B3A9" />
                                                  </svg>
                                                </div>
                                              </div>
                                              <div className="col-9 ps-0 col-lg-10 ms-md-0 d-flex flex-column justify-content-center ">
                                                <div className="row m-0 border-bottom">
                                                  <div className='d-flex justify-content-between'>
                                                    <Link to={`/viewflashcard/subject/${course_id}/${x.flashset_id}`} onClick={() => sessionStorage.setItem("selectedTab", 'flashcards')} className='fw-bold w-75 text-primary text-decoration-none'>{x.name}</Link>
                                                    <button className={`bg-transparent border-0 ${x.studylist_status ? 'd-none' : ' d-flex align-items-center'}`} style={{ color: '#8587EA' }} onClick={() => {
                    ...
                    <div className="col-lg-3 pe-lg-0 d-none d-lg-block" onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                      <div onClick={() => { setindex1(-1); setreportdropdownstate(false) }} className="border px-2 rounded d-flex flex-column align-items-center mb-lg-3 shadow" style={{ height: '400px' }}>
                        <img className={`mt-2 ${tablist === "documents" || tablist === "flashcards" ? "d-block" : "d-none"}`} src={require('../img/images_icons/send-img2.png')} width={60} alt="sub" />
                        <img className={`mt-2 ${tablist === "discussion" ? "d-block" : "d-none"}`} src={require('../img/images_icons/send-img1.png')} width={60} alt="sub" />
                        <img src={require('../img/images_icons/mascot1.png')} style={{ height: '250px', width: '118px' }} alt="sub" />
                        <button id='Subject__CreatePost' className={`btn mt-3 w-100 fw-medium ${tablist === "discussion" ? "d-block" : "d-none"}`} style={{ border: '1px solid #5D5FE3', color: '#5D5FE3', fontWeight: 500 }} data-bs-toggle="modal"
                          data-bs-target="#postmodal">{translate_value.dashboard.create_a_post}</button>
              ...
              <div className="modal-content">
                <div className="modal-body pt-4 pb-3">
                  <div>
                    ...
                    <textarea onChange={(e) => { setexitgroup_reason(e.target.value) }} className='form-control shadow-sm mt-2 py-3'></textarea>
                  </div>
              ...
              <div className="modal-content">
                <div className="modal-body pt-4 pb-3">
                  <div>
                    ...
                    <textarea placeholder='Reason must be more than 20 characters...' onChange={(e) => { setReportvalue(e.target.value) }} className='form-control shadow-sm mt-2 py-3'></textarea>
                  </div>
                  <div className='mt-3 text-end'>
              ...
              <div className="modal-content">
                <div className="modal-body">
                  <div className='col-12 bg-white px-lg-3 pt-2 pb-3'>
                    ...
                    <form className='w-100' onSubmit={(e) => postreply_for_replies(e)}>
                      <div className='d-flex gap-1 gap-lg-3 align-items-center'>
                        <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub" />
                        {userdetails.nickname != undefined ? (<div className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></div>) : (<></>)}

                        <div className="input-group bg-white border rounded pe-lg-3">
                          <input type="text" name="question" value={reply_for_reply} onChange={(e) => { setReply_for_reply(e.target.value) }} style={{ position: 'relative' }}
                            className="form-control py-3 ps-3 bg-white border-0 shadow-none post-input" placeholder="Post your reply....." />
                          <div className='d-flex align-items-center bg-white'>
                            <input id="fileInput2" type='file' name='file' accept="image/*" multiple onChange={handleReply_reply_Image} className="bg-white text-center p-3 btn" />
                            <label data-bs-toggle="tooltip" data-bs-placement="top"
                              data-bs-custom-class="custom-tooltip"
                              data-bs-title="Attach Image"
                              htmlFor="fileInput2"
                              className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                              <img src={require('../img/attachment.png')} width={22} height={22} alt="sub" />
                            </label>
                            <button disabled={reply_for_reply.length > 0 ? false : true} data-bs-dismiss="modal" className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' >
                              <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                data-bs-custom-class="custom-tooltip"
                                data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696" />
