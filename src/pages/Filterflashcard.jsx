...
const Filterflashcard = () => {
  ...
  let { id } = useParams();
  let { type } = useParams();
  const renderTooltip = (props) => (<Tooltip id="button-tooltip" {...props}>Stop the current playing mode and then mute the audio</Tooltip>);
  const renderTooltip2 = (props) => (<Tooltip id="button-tooltip" {...props}>Stop the current playing mode and then unmute the audio</Tooltip>);
  ...
  const fetchSavedflashsets = () => {
    axiosInstance.post(`${ipaddress2}/SavedStudylistFlashsets/`, { 'user_id': user.user_id })
      .then((r) => { setSavedflashsets(r.data) })
      .catch(() => { console.log("SavedStudylistFlashsets Error") })
  }
  // ------------------------------------------------To fetch the Bookmarked flashcards-------------------------------------------
  const fetchBookmarkedflashcards = () => {
    axiosInstance.post(`${ipaddress2}/UserBookmarkedFlashcardsView/`, { 'user_id': user.user_id })
      .then((r) => {
      ...
      }).catch(() => { console.log("UserBookmarkedFlashcardsView Error") })
  }
  ...
  useEffect(() => {
    if (flashcards.length === 0) { // Triggering the function when flashcards length becomes zero
      setCount(1);
      studyFlashcardsrepeatly();
    }
  }, [flashcards]);
  ...
  const studyFlashcardsrepeatly = () => {
    if (flashcards.length > 0) {
      ...
      getflashcards(); // setflashcardcount(0)
    }
  }
  ...
  useEffect(() => {
    if (studyflashcardshuffle_status == true) { studyFlashcardsshuffle() };
  }, [studyflashcardshuffle_status]);

  const studyFlashcardsshuffle = () => {
    if (flashcards.length > 0 && studyflashcardshuffle_status == true) {
      const shuffledFlashcards = [...flashcards].sort(() => Math.random() - 0.5); // Shuffle the flashcards array
      setflashcards((prevFilteredcards) => {
        const newFilteredcards = [...prevFilteredcards];
        newFilteredcards.shift(); // Remove the first element
        return newFilteredcards;
      });
      displayCard(shuffledFlashcards[0]); // Display the first card from the shuffled array
      setTimeout(() => {
        setCount((prev) => prev + 1)
        studyFlashcardsshuffle(); // Call the function recursively
      }, 5000);
    } else {
      getflashcards(); // console.log("Stopped")
    }
  }
  ...
  //   script.src = "https://code.responsivevoice.org/responsivevoice.js?key=auvTMQpf";
  //   script.async = true;
  ...
  const getflashcards = () => {
    ...
    axiosInstance.get(`${ipaddress2}/flashcardsList/${flashset_id}/${user.user_id}/`)
      ...
      }).catch(() => { console.log("Particular Flashset details2 Fetching Error") })
  }

  const [count3, setCount3] = useState(0);
  // Stop the loop
  const stop_loop = () => { toggleConfirmModal() }
  ...
  useEffect(() => {
    ...
    prevloopstate.current = loopstate; // Update the previous state reference
  }, [loopstate]);
  ...
  const textToSpeech = () => {
    if (typeof responsiveVoice !== "undefined") {
      const text = document.getElementById("paratext").textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate, // Updated rate
        volume: settings.volume, // Updated volume
        pitch: settings.pitch, // Updated pitch
        onend: handleClick, // Callback for when speech ends
      });
    } else {
  ...
  const textToSpeech2 = () => {
    if (typeof responsiveVoice !== "undefined") {
      const text = document.getElementById("paratext1").textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate, // Updated rate
        volume: settings.volume, // Updated volume
        pitch: settings.pitch, // Updated pitch
        onend: studyFlashcardnew, // Callback for when speech ends
      });
    } else {
      console.error("ResponsiveVoice.js is not loaded or supported.");
    }
  ...
  const studyFlashcardnew = () => {
    ...
    setFlashcardsindex(prev => { return prev + 1; });
    setIsFlipped(false);
  };
  ...
  const initiateshuffle = () => {
    ...
    //console.log("Flashcard rendered object term-----------------------------222222222----------------", flashcard_obj.term)
  ...
  const textToSpeech_shuffle = () => {
    const element = document.getElementById("paratext3");
    if (!element) {
      console.log("Element with ID 'paratext3' does not exist.");
      return;
    }
    if (typeof responsiveVoice !== "undefined") {
      const text = element.textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate,
        volume: settings.volume,
        pitch: settings.pitch,
        onend: handleClick_shuffle,
      });
    } else {
      console.log("ResponsiveVoice.js is not loaded or supported.");
    }
    // if (typeof responsiveVoice !== "undefined") {
    //   const text = document.getElementById("paratext3").textContent || 'Sorry! It is Empty.';
    //   window.responsiveVoice.speak(text, speakLanguaue, {
    //     rate: settings.rate, // Updated rate
    //     volume: settings.volume, // Updated volume
    //     pitch: settings.pitch, // Updated pitch
    //     onend: handleClick_shuffle, // Callback for when speech ends
    //   });
    // } else {
    //   console.error("ResponsiveVoice.js is not loaded or supported.");
    // }
    // Below is old Code by vivek
    // if ('speechSynthesis' in window) {
    ...
    //   speech.rate = 1; // Adjust this value to decrease the speed further if needed
    //   // Increase volume
    //   speech.volume = mute ? 0 : 5;
    //   window.speechSynthesis.speak(speech);
    //   speech.onend = () => { handleClick_shuffle() }
    // } else {
    //   console.error('Speech synthesis not supported in your browser.');
    // }
  };
  const stopSpeech_shuffle = () => { window.speechSynthesis.cancel(); };
  // Step3
  ...
  const textToSpeech2_shuffle = () => {
    if (typeof responsiveVoice !== "undefined") {
      const text = document.getElementById("paratext4").textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate,
        volume: settings.volume,
        pitch: settings.pitch,
        onend: studyFlashcardnew_shuffle
      });
    } else {
      console.error("ResponsiveVoice.js is not loaded or supported.");
    }
  ...
  useEffect(() => {
    // if (flashcardsindex < flashcards.length) {
    //   console.log("flashcards updated index--------useEffect-------", flashcardsindex1)
    //   console.log("-----------------ffalsh objectttttttt--------useEffect-------", flashcard_obj_shuffle)
    //   console.log("-----------------setflashcard_obj--------useEffect-------", flashcards[flashcardsindex])
    //   setflashcard_obj(flashcards[flashcardsindex])
    //   setflashcard_obj_shuffle(flashcards[flashcardsindex])
  ...
  // Stop the loop
  const stop_repeat = () => { toggleConfirmModal(); }
  ...
  const textToSpeech_repeat = () => {
    if (typeof responsiveVoice !== "undefined") {
      const text = document.getElementById("paratext5").textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate,
        volume: settings.volume,
        pitch: settings.pitch,
        onend: handleClick_repeat
      });
    } else {
      console.error("ResponsiveVoice.js is not loaded or supported.");
    }
  ...
  const textToSpeech2_repeat = () => {
    if (typeof responsiveVoice !== "undefined") {
      const text = document.getElementById("paratext6").textContent || 'Sorry! It is Empty.';
      window.responsiveVoice.speak(text, speakLanguaue, {
        rate: settings.rate,
        volume: settings.volume,
        pitch: settings.pitch,
        onend: studyFlashcardnew_repeat
      });
    } else {
      console.error("ResponsiveVoice.js is not loaded or supported.");
    }
  ...
  const handleTempConfigChange = ({ target: { name, value } }) => {
    const formattedValue = parseFloat(parseFloat(value).toFixed(2)); // Format and ensure it's a number
  ...
  return (
    <div className='d-flex'>
      ...
      <div onClick={() => {
        ...
        <div className="container" onClick={() => { setnavbar_dropdown_visible(false) }}>
          {/* -----------------------------------------------DISPLAY FLASHCARDS-------------------------------------------------- */}
          <div className="container" id='matchflashcard'>
            <div className='row m-0 mt-5 d-flex justify-content-center bg-light pb-4'>
              <div style={{ position: 'relative' }} className="col-md-9 bg-white rounded shadow-sm mb-3 mb-lg-0 d-flex align-items-center py-4 px-4">
                <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip3(flashsetdata.name)}>
                ...
                <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip3("Adjust Speech Settings")}>
                  <i className="fa fa-sliders" aria-hidden="true" style={{ cursor: 'pointer' }} onClick={toggleConfigModal}></i>
                </OverlayTrigger>
                <span className='ms-1' style={{ cursor: 'pointer' }} onClick={() => { setDropdownstate(!dropdownstate) }}>
                ...
                <div className={`bg-white border rounded p-2 ${dropdownstate ? '' : 'd-none'}`} style={{ position: 'absolute', right: '4%', zIndex: '1', top: '68%' }}>
                  <span className='d-block border-bottom pb-1' style={{ cursor: 'pointer' }} onClick={() => { fetchBookmarkedflashcards() }}>Bookmarked Flashcards</span>
                  <span className='pt-1' style={{ cursor: 'pointer' }} onClick={() => { setDropdownstate(false); getflashcards(); }}>All Flashcards</span>
                </div>
              </div>
                  ...
                  <div className="" style={{ position: 'relative' }}>
                    ...
                    <div className={`${shufflestate || repeatstate ? 'd-none' : ''}`}>
                      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal">
                        <div className='w-100 d-flex flex-column align-items-center bg-white rounded' style={{ height: '323px', boxShadow: '0px 5px 3px 0px rgba(0, 0, 0, 0.07)' }}>
                          ...
                          <button className='mt-5 pt-5 mb-4 d-flex align-items-center btn bg-transparent border-0 fw-bold' style={{ color: '#5d5fe3' }} onClick={() => { setIsFlipped((prev) => !prev) }}>
                        ...
                        {/* Back side div */}
                        <div className='w-100 d-flex flex-column align-items-center rounded' style={{ height: '323px', boxShadow: '0px 5px 3px 0px rgba(0, 0, 0, 0.07)', background: 'white' }}>
                          <div className='text-center mt-3'></div>
                          <div className='mt-5 pt-4 px-1 px-md-5'>
                            {flashcard_obj && (
                    ...
                    <div className={`${shufflestate ? '' : 'd-none'}`}>
                      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal" >
                        ...
                        {/* Back side div */}
                        <div className='w-100 d-flex flex-column align-items-center' style={{ height: '323px', boxShadow: '0px 5px 3px 0px rgba(0, 0, 0, 0.07)', background: 'white' }}>
                          <div className='text-center mt-3'></div>
                          <div className='mt-5 px-1 px-md-5'>
                            {flashcard_obj_shuffle && (
                    ...
                    <div className={`${repeatstate ? '' : 'd-none'}`}>
                      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal" >
                        <div className='w-100 d-flex flex-column align-items-center bg-white rounded' style={{ height: '323px', boxShadow: '0px 5px 3px 0px rgba(0, 0, 0, 0.07)' }}>
                          ...
                          <button className='mt-4 pt-5 mb-4 d-flex align-items-center btn bg-transparent border-0 fw-bold' style={{ color: '#5d5fe3' }} onClick={() => { setIsFlipped((prev) => !prev) }}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                              <path d="M10.4998 20.3957C8.25758 20.3957 6.2517 19.7213 4.48221 18.3724C2.71271 17.0236 1.54284 15.2675 0.972598 13.104H2.02627C2.59252 14.9844 3.65556 16.4961 5.21538 17.6393C6.77521 18.7824 8.53669 19.354 10.4998 19.354C12.2466 19.354 13.8358 18.8682 15.2675 17.8967C16.6991 16.9251 17.8396 15.6748 18.6889 14.1457H15.1873V13.104H20.3957V18.3123H19.354V15.0271C18.4045 16.627 17.1572 17.9217 15.612 18.9113C14.0669 19.9009 12.3628 20.3957 10.4998 20.3957ZM10.4998 12.5832C9.91223 12.5832 9.41811 12.3828 9.01747 11.9822C8.61683 11.5816 8.4165 11.0874 8.4165 10.4998C8.4165 9.91223 8.61683 9.41811 9.01747 9.01747C9.41811 8.61683 9.91223 8.4165 10.4998 8.4165C11.0874 8.4165 11.5816 8.61683 11.9822 9.01747C12.3828 9.41811 12.5832 9.91223 12.5832 10.4998C12.5832 11.0874 12.3828 11.5816 11.9822 11.9822C11.5816 12.3828 11.0874 12.5832 10.4998 12.5832ZM0.604004 7.89567V2.68734H1.64567V5.97257C2.59518 4.37269 3.84251 3.07796 5.38765 2.08838C6.93279 1.0988 8.63685 0.604004 10.4998 0.604004C12.7421 0.604004 14.748 1.27841 16.5175 2.62723C18.287 3.97607 19.4568 5.73222 20.0271 7.89567H18.9734C18.4072 6.01532 17.3441 4.50357 15.7843 3.36041C14.2245 2.21725 12.463 1.64567 10.4998 1.64567C8.75305 1.64567 7.16384 2.13145 5.73221 3.10301C4.30059 4.07456 3.16011 5.32489 2.31075 6.854H5.81234V7.89567H0.604004Z" fill="#5D5FE3" />
                            </svg> <span className='ms-2'>Flip</span></button>
                        </div>
      ...
      <Modal isOpen={shuffle_audio} toggle={() => { setshuffle_audio(false) }} centered>
        <ModalHeader className='text-center' toggle={() => { setshuffle_audio(false) }}><h4 className="modal-title fs-5">Play in Shuffle</h4></ModalHeader>
        <ModalBody className="text-center"><p>Do you want Audio for Flashcard?</p></ModalBody>
      ...
      <Modal isOpen={loop_audio} toggle={() => { setloop_audio(false) }} centered>
        <ModalHeader className='text-center' toggle={() => { setloop_audio(false) }}><h4 className="modal-title fs-5">Play in Loop</h4></ModalHeader>
        <ModalBody className="text-center"><p>Do you want Audio for Flashcard?</p></ModalBody>
        <ModalFooter className="justify-content-end">
          <Button
            color="primary"
            size="sm"
            onClick={() => { setloopstate(true); setloop_audio(false); }}>
            Yes
          </Button>
          <Button
            color="danger"
      ...
      <Backtotop />
      <Modal isOpen={repeat_audio} toggle={() => { setrepeat_audio(false) }} centered>
        <ModalHeader className='text-center' toggle={() => { setrepeat_audio(false) }} >
          <h4 className="modal-title fs-5">Play in Repeat</h4>
        </ModalHeader>
        <ModalBody className="text-center">
          <p>Do you want Audio for Flashcard?</p>
        </ModalBody>
        <ModalFooter className="justify-content-end">
          <Button
            color="primary"
      ...
      <Modal isOpen={configModal} toggle={toggleConfigModal} centered>
        <ModalHeader>
          <div className="d-flex justify-content-end">
            <h5>{translate_value.flashcard.flashcard_Setting_title}</h5>
            <div style={{ paddingLeft: "216px" }}>
              <OverlayTrigger placement="left" delay={{ show: 250, hide: 250 }} overlay={renderTooltip3("Settings applied from the next card.")}>
                <i className="fa fa-exclamation-circle" aria-hidden="true" style={{ color: "#dc3545", cursor: "pointer" }}></i>
              </OverlayTrigger>
            </div>
          </div>
        </ModalHeader>
        ...
        <ModalBody className="p-4">
          <div className="mb-5">
            <Label className="mb-3" for="pitch">{translate_value.flashcard.var_setting_tone} </Label>
            <InputRange
              maxValue={2}
              minValue={0}
              step={0.01}
              value={tempSettings.pitch}
              onChange={(value) => handleTempConfigChange({ target: { name: 'pitch', value } })}
            />
          </div>
          <hr />
          <div className="mb-5">
            <Label className="mb-3" for="volume">{translate_value.flashcard.var_setting_vol}</Label>
            <InputRange
              maxValue={1}
              minValue={0}
              step={0.01}
              value={tempSettings.volume}
              onChange={(value) => handleTempConfigChange({ target: { name: 'volume', value } })}
            />
          </div>
          <hr />
          <div className="mb-4">
            <Label className="mb-3" for="rate">{translate_value.flashcard.var_setting_speed}</Label>
            <InputRange
              maxValue={1.5}
              minValue={0}
              step={0.01}
              value={tempSettings.rate}
              onChange={(value) => handleTempConfigChange({ target: { name: 'rate', value } })}
            />
          </div>
        </ModalBody>
        <ModalFooter className="d-flex justify-content-between">
          <Button color="secondary" size="sm" onClick={toggleResetConfigModal}>{translate_value.common_words.Reset}</Button>
          <div className="d-flex gap-2">
            <Button color="primary" size="sm" onClick={saveConfig}>{translate_value.common_words.Save}</Button>
            <Button color="danger" size="sm" onClick={toggleConfigModal}>{translate_value.common_words.Cancel}</Button>
          </div>
        </ModalFooter>
      </Modal>
      <Modal isOpen={isConfirmModalOpen} toggle={toggleConfirmModal} centered>
        ...
        <ModalBody>Stop the shuffle and then click and play in Loop mode</ModalBody>
        <ModalFooter>
