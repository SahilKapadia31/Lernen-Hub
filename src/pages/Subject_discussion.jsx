...
const Subject_discussion = ({ course_id, pinnedcomments_status, course_name, setreportdropdownstate, setCount }) => {
  ...
  const fetchPinned_comments = () => {
    axiosInstance.get(`${ipaddress}/coursediscussionpinningview/${user.user_id}/${course_id}/`)
      .then((r) => { setpinnedComments(r.data.reverse()) })
      .catch(() => { console.log("coursediscussionpinningview Error") })
  }
  ...
  const removeImage = (index) => { setRepliesImage((prevImages) => prevImages.filter((_, i) => i !== index)) };
  ...
  const postReplies = async (e, dis_id, index) => {
    ...
    if (replies.length > 0) {
      try {
        ...
        const response = await fetch(`${ipaddress}/CourseDiscussionReply/${user.user_id}/${dis_id}/""/?post=${encoded_reply}`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
        if (response.ok) {
          fetchPinned_comments();
  ...
  function handleLike1(discussion_id) {
    axiosInstance.delete(`${ipaddress}/DiscussionLikes/${user.user_id}/${discussion_id}/`)
      .then((r) => {
        fetchPinned_comments();
        setCount((prev) => prev + 1);
  ...
  function handleLike(discussion_id) {
    axiosInstance.post(`${ipaddress}/DiscussionLikes/${user.user_id}/${discussion_id}/`)
      .then((r) => {
        fetchPinned_comments();
        setCount((prev) => prev + 1);
  ...
  function handleReplyLike1(discussion_id, discid, index) {
    axiosInstance.delete(`${ipaddress}/DiscussionRepliesLikes/${user.user_id}/${discussion_id}/`)
  ...
  function handleReplyLike(discussion_id, discid, index) {
    axiosInstance.post(`${ipaddress}/DiscussionRepliesLikes/${user.user_id}/${discussion_id}/`)
      .then((r) => { getreplies(discid, index) })
      .catch((err) => { console.log("User Reply like error", err) })
  }
  // ------------------------------------------------To dislike the post in discussion-------------------------------------------------------
  function handledislike(discussion_id) {
    axiosInstance.post(`${ipaddress}/CourseDiscussionDisLike/${user.user_id}/${discussion_id}/`)
      .then((r) => {
        fetchPinned_comments();
        setCount((prev) => prev + 1);
      })
  ...
  function handlereplydislike(discussion_id, discid, index) {
    axiosInstance.post(`${ipaddress}/CourseDiscussionRepliesDisLikes/${user.user_id}/${discussion_id}/`)
      .then((r) => { getreplies(discid, index) })
      .catch(() => { console.log("User like error") })
  }
  ...
  function handleReplies_reply_like(replies_reply_id, disc_replyid) {
    axiosInstance.post(`${ipaddress}/CourseDiscussionRepliesReplyLikes/${user.user_id}/${replies_reply_id}/`)
  ...
  const sendEditedData = (e) => {
    ...
    axiosInstance.put(`${ipaddress}/CourseDiscussionEdit/${user.user_id}/${course_id}/${discussionId}/`, formData)
      .then((r) => {
        fetchPinned_comments();
        setCount((prev) => prev + 1)
  ...
  const getreplies = (discussion_id, index) => {
    axiosInstance.get(`${ipaddress}/coursecommentsreplies/${discussion_id}/${user.user_id}/`)
      .then((r) => {
  ...
  const getreplies_for_reply = (particular_reply_id) => {
    axiosInstance.get(`${ipaddress}/CourseDiscussionRepliesReply/${user.user_id}/${particular_reply_id}/`)
      .then((r) => {
  ...
  const deleteReply_for_reply = (reply_reply_id, disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/CourseDiscussionRepliesReply/${user.user_id}/${reply_reply_id}/`)
      .then((r) => {
        toast.success('Reply deleted successfully', { autoClose: 2000 });
        getreplies(discid);
  ...
  return (
    <div className={`${pinnedcomments_status ? '' : 'd-none'}`}>
      <div className={`${pinnedComments.length > 0 ? 'd-none' : ''}`}>
      <img src="https://starlight-b3f27.web.app/assets/images/nodataimg.svg" className='not-found-image'/>
      <h6 className={`text-center py-2  py-3`} style={{ color: '#5d5fe3', fontSize: '14px', marginLeft: "27px" }}>No Followed comments available ...💬</h6>
      </div>
            ...
            <div className='rounded shadow-sm border'>
              <div className="d-flex justify-content-between border-bottom py-2 ps-2 m-0 align-items-center">
                <div className='d-flex gap-2 align-items-center'>
                  <div className="d-flex justify-content-center" onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                    <img src={x.disid.user_id.profile_pic} className={x.disid.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="subject" />
                  ...
                  <div className="p-0" onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                    <Link to={`/profile/${x.disid.user_id.user_id}`} className='my-0 fw-medium text-decoration-none text-dark' style={{ fontSize: '14px' }}>{x.disid.user_id.nickname}
                ...
                <div className="d-flex justify-content-end align-items-center p-0 gap-2">

                  <p className='my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA', cursor: 'pointer' }}
                    onClick={() => { navigate(`/profile/${x.disid.user_id.user_id}`) }}><span className={`ms-2 edit ${x.disid.edited ? '' : 'd-none'}`}>Edited</span></p>
                  <span data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip"
                    data-bs-title="Unfollow" style={{ cursor: 'pointer' }} onClick={() => { unpin(x.disid.discid) }}><i className="fa-solid fa-link-slash" style={{ color: '#2A3941' }}></i></span>
                  <div className="btn-group dropstart">
                    ...
                    <ul className={`bg-white shadow-sm border rounded mt-0 p-0 px-3 ${index1 == index ? '' : 'd-none'}`} style={{ width: '160px', position: 'absolute', left: '-160px' }}>
                      ...
                      {/* ----------------------------------------------------Report button--------------------------------------------------- */}
                      {user.user_id !== x.disid.user_id.user_id &&
                        <>
                      ...
                      <button className={`bg-transparent border-0 ${user.user_id === x.disid.user_id.user_id ? 'd-flex align-items-center my-2' : 'd-none'}`} onClick={() => {
                        ...
                        <div className="modal-body">
                          <div className=' bg-white px-3 pt-2 pb-3 rounded'>
                            ...
                            <div className='d-flex gap-3'>
                              <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="sub-diss" />
                              ...
                              <form className='w-100' onSubmit={(e) => sendEditedData(e, x.disid.discid)}>
                                <div className="input-group rounded bg-light pe-3 border">
                                  ...
                                  <button type='submit' data-bs-dismiss="modal" disabled={editedpost.length > 0 ? false : true} className='bg-transparent border-0'>
              ...
              <div className='py-2 px-2' onClick={() => {
                ...
                {x.disid.images_attached && (
                  x.disid.images_attached.map((z) => {
                    return (
                      <div>
                        <img src={z.image} width={300} alt="sub-diss" className='mt-3' />
                      </div>
                    )
                  })
                )}
              ...
              <div className='d-flex justify-content-between border-bottom px-2 pb-2 mb-0' onClick={() => { setindex1(-1) }}>
                <div className='d-flex align-items-center gap-2'>
                  ...
                  <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: x.dis_liked_status ? "#ff845d" : "gray" }} onClick={() => {
                ...
                {x.discussion_replies_count > 0 ?
                  (!replies_layout) ?
                    <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_layout(!replies_layout); setSelectedReply(index); getreplies(x.disid.discid, index) }}>
                      {translate_value.dashboard.view_all} {x.discussion_replies_count} {translate_value.dashboard.replies}
                    </button>
                    :
                    (selectedReply == index) ?
              ...
              {fetchedreplies && fetchedreplies.length > 0 && (
                <div className={fetchedreplies[0].discid == x.disid.discid && replies_layout ? 'd-block' : 'd-none'}>
                  {fetchedreplies.map((y, index2) => {
                    return (
                      <div className='px-2 py-2' onClick={() => { setindex1(-1) }}>
                        <div className="px-2 d-flex align-items-center justify-content-between w-100">
                          <div className='d-flex gap-2'>
                            <div className="d-flex justify-content-end">
                              <img src={y.user.profile_pic} className={y.user.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="sub-diss" />
                          ...
                          <div className="d-flex gap-3 justify-content-between">
                            ...
                            <button className={user.user_id === y.user.user_id ? 'bg-transparent border-0 ms-1 d-flex align-items-center' : 'd-none'}
                              onClick={() => { deleteReply(y.disrid, x.disid.discid) }} style={{ height: '20px' }}>
                          ...
                          {y.images_attached.map((a) => {
                            return (
                              <div className='d-flex justify-content-center'>
                                <img src={a.doc_reply_images} width={300} alt="sub-diss" className='mt-3' />
                              </div>
                            )
                          })}
                        </div>
                        ...
                        <div className="mt-2 ps-5">
                          ...
                          {/* <p style={{ cursor: 'pointer' }} className={`view_reply_for_reply mt-2 mb-0 ${y.replies_replies_count > 0 ? '' : 'd-none'}`} onClick={() => {
                                ...
                                return (
                                  <div className='px-2 py-2' onClick={() => { setindex1(-1) }} >
                                    <div className="d-flex align-items-center justify-content-between w-100">
                                      <div className='d-flex gap-2'>
                                        <div className="d-flex justify-content-center">
                                          <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="sub-diss" />
                                    ...
                                    <div className='ps-5 mt-2 m-0'>
                                      ...
                                      {z.images_attached.map((b) => {
                                        return (
                                          <div className='d-flex justify-content-center'>
                                            <img src={b.dis_reply_reply_images} width={260} alt="sub-diss" className='mt-3' />
                                          </div>
                                        )
                                      })}
                                    </div>
              ...
              <div onClick={() => {
                ...
                <form className='w-100' onSubmit={(e) => postReplies(e, x.disid.discid, index)}>
                  <div className="input-group border rounded pe-lg-3 bg-light">
                    <input key={index}
                      type="text"
                      onChange={(e) => { repliesData(e); setSelectedPostForComment(x.disid.discid) }}
                      className="form-control py-2 ps-3 shadow-none border-0 bg-light pin-reply-input" placeholder={translate_value.dashboard.reply_here}
                      style={{ position: 'relative' }} />
                    <div className='d-flex align-items-center bg-light'>
                      ...
                      <button type='submit' disabled={replies.length > 0 ? false : true}
                        className='h-100 bg-transparent border-0 ms-2'
                      >
                        <div style={{ width: '30px' }} className={`${(load && selectedPostForComment == x.disid.discid) ? '' : 'd-none'}`}>
                        ...
                        <svg data-bs-toggle="tooltip" data-bs-placement="top"
                          data-bs-custom-class="custom-tooltip"
                          data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" className={`${(load && selectedPostForComment == x.disid.discid) ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 30 30" fill="none">
                          <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696" />
                        </svg>
                      </button>
                    </div>
                    ...
                    <div className="input-group bg-light border rounded pe-3">
                      ...
                      <div className='d-flex align-items-center bg-light'>
                        ...
                        <label data-bs-toggle="tooltip" data-bs-placement="top"
                          ...
                          htmlFor="fileInput3"
                          className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                          <img src={require('../img/attachment.png')} width={22} height={22} alt="sub-diss" />
                        </label>
