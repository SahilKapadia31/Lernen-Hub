...
const initialValues = { groupname: "", category: "", city: "", scope1: "", group_description: "", agreeTerms: false };
const validationSchema = Yup.object().shape({
  groupname: Yup.string().required("Group Name is a required field."),
  category: Yup.string().required("Please Select the Category field."),
  group_description: Yup.string().required("Group Description is a required field."),
  agreeTerms: Yup.boolean()
    .oneOf([true], "You must accept the terms and conditions.")
    .required("You must accept the terms and conditions"),
});

const initialPrivateValues = { groupname: "", scope1: "", agreeTerms: false };
const validationPrivateSchema = Yup.object().shape({
  groupname: Yup.string().required("Group Name is a required field."),
  agreeTerms: Yup.boolean()
    .oneOf([true], "You must accept the terms and conditions.")
    .required("You must accept the terms and conditions"),
...
const Opengroups = () => {
  ...
  const [AddpublicgroupModal, setAddpublicgroupModal] = useState(false);
  const [AddprivategroupModal, setAddprivategroupModal] = useState(false);

  useEffect(() => {
    if (grouptype === "opengroup") {
      axiosInstance.get(`${ipaddress}/GroupCategories/`)
        .then((r) => {
          setCategories(r.data); // console.log("Group Category",r.data)
        }).catch((err) => { console.log("catch error", err) });
      axiosInstance.get(`${ipaddress}/SearchGroupWithCity/${user.user_id}/`)
        .then((r) => {
          setpublicgroups(r.data.available_groups);  // console.log("Public Groupssssssssssss",r.data)
          setloading(false);
        }).catch((err) => { console.log("catch error", err) });
    } else {
    ...
    axiosInstance.get(`${ipaddress}/DisplayCityAndCountry/`)
      .then((r) => {
        if (r.data && r.data.cities) {
          const city = [];
          r.data.cities.map((item) => {
  ...
  const fetchprivategroups = () => {
    axiosInstance.get(`${ipaddress}/ParticularUserGroup/${user.user_id}/`)
      .then((r) => {
        ...
        setloading(false)
      }).catch((err) => { console.log("Group details fetching error", err) })
  }
  ...
  const groupNameData = (e) => { setGroupname(e.target.value) }
  const groupImage = (e) => {
    console.log(e)
    var fileSize = (e.target.files[0].size / 1024 / 1024).toFixed(2);
    if (fileSize > 0.5) {
      toast.error('File size must be less than 500 KB.', toastConfig);
    } else {
  ...
  const createGroup = async (values) => {
    try {
      ...
      const response = await axiosInstance.post(`${ipaddress}/PrivateGroups/${user.user_id}/`, formdata);
      if (response.status === 201) { // Handle success
        toast.success('Your Group Created Succesfully!!!', toastConfig);
        setCount(count + 1)
        setGroupimage(null);
        setpublic_group_state1(false)
        setpublic_group_state2(true)
        setAddprivategroupModal(false);
      } else if (response.status === 404) {
        toast.success(response, toastConfig);
      } else {
        toast.error('Failed to Group Creation Error', toastConfig);
      }
    } catch (error) {
  ...
  // --------------------------------Create Public Group--------------------------------------
  const createpublicGroup = async (values) => {
    try {
      const formdata4 = new FormData()
      formdata4.append('group_name', values.groupname)
      formdata4.append('file', groupimage2)
      formdata4.append('city', user.city)
      formdata4.append('scope', scope1)
      formdata4.append('category', values.category)
      formdata4.append('description', values.group_description)
      const response = await axiosInstance.post(`${ipaddress}/PrivateGroups/${user.user_id}/`, formdata4); // Make the API call
      if (response.status === 201) { // Handle success
        toast.success('Group Created Successfully', toastConfig);
        setGroupimage2(null)
        setAddpublicgroupModal(false);
        toast.info('Request successfully sent to the Admin !!!', toastConfig);
      } else if (response.status === 404) {
        toast.success(response, toastConfig);
      } else {
        toast.error('Failed to Group Creation Error', toastConfig);
  ...
  const resetAndCloseModal = () => {
    // Reset form values and open the modal
    setGroupimage2(null)
    setAddpublicgroupModal(false);
  };
  const resetPrivateCloseModal = () => {
    setGroupimage(null)
    setAddprivategroupModal(false);
  };
  ...
  const [joinedPublicCategoryGroups, setJoinedPublicCategoryGroups] = useState([])
  ...
  const getJoinedPublicCategorywiseGroups = (category_id) => {
    if (!city?.value) {
      axiosInstance.get(`${ipaddress}/SearchWithName-v2/${category_id}/${user.user_id}/`)
        .then((r) => {
          console.log("getJoinedPublicCategorywiseGroups", r.data)
          setgroupsbycategories(r.data?.available_groups);
          setJoinedPublicCategoryGroups(r.data?.joined_groups);
        }).catch((err) => { console.log("catch error", err) });
    }
  }
    ...
    axiosInstance.post(`${ipaddress}/SearchGroup/${user.user_id}/${private_group_id}/`)
      .then((r) => {
        ...
        if (r.data === "Group Chat Room is Full") {
          toast.warn('Group Chat Room is Full', { autoClose: 2000 })
        } else {
          toast.success('Request sent to the Admin', { autoClose: 2000 })
          setCount(count + 1)
        }
      }).catch((err) => { console.log("Group ID Sending Error", err) })
  }
  ...
  const joinPublicgroup = (group_id) => {
    axiosInstance.post(`${ipaddress}/SearchGroup/${user.user_id}/${group_id}/`)
      .then((r) => {
        setCount(count + 1);  // console.log("Successfully Joined",r.data)
        setsearchvalue("")
        setSearchedGroups([])
        toast.success('Successfully Joined the Group', { autoClose: 1000 })
        setTimeout(() => { window.location.reload() }, 2000);
      }).catch(() => { console.log("Public group joining Error") });
  }
  ...
  const searchGroups = (searchvalue) => {
    if (searchvalue.length > 0) {
      axiosInstance.post(`${ipaddress}/SearchWithName/${searchvalue}/${user.user_id}/`)
        .then((r) => {
          setSearchedGroups(r.data); // console.log("Successfully search and fetched",r.data)
        }).catch(() => { console.log("Search group error") })
  ...
  const searchprivateGroups = (searchvalue) => {
    if (searchvalue.length > 0) {
      axiosInstance.get(`${ipaddress}/SearchPrivateGroup/${user.user_id}/${searchvalue}/`)
        ...
        }).catch((err) => { console.log("Search group error", err) })
    } else { setSearchedprivateGroups([]) }
  }
  ...
  const sendselectedcity_tosearch = (city, category_id) => {
    if (city && (category_id || categoryId)) {
      const formdata = new FormData()
      formdata.append('category_id', category_id || categoryId)
  ...
  const copy = () => {
    const groupID = groupIDElement.textContent; // Copy the group ID to the clipboard
  ...
  const fetchJoinedgroups = () => {
    axiosInstance.get(`${ipaddress}/ParticularUserGroup/${user.user_id}/`)
      .then((r) => {
        setjoined_public_groups(r.data.user_joined_public_groups); console.log(r.data.user_joined_public_groups);
      })
  ...
  const handleChange = (option) => {
    setcity(option)
    sendselectedcity_tosearch(option.value);
  ...
  useEffect(() => {
    let selectedTab = (grouptype === "opengroup") ? 'recommended' : 'private_searched';
  ...
  useEffect(() => {
    ...
    if (grouptype === "opengroup") {
      if (selectedTab === "public") {
        items = categoryname === "all" ? joined_public_groups : joinedPublicCategoryGroups;
      } else if (selectedTab === "recommended") {
        items = categoryname === "all" ? publicgroups : groupsbycategories;
      }
    } else {
      if (selectedTab === "private_searched") {
        items = SearchedprivateGroups;
      } else if (selectedTab === "private_joined") {
        items = joinedPrivategroups;
      }
    }

    getPaginatedItems(items);

  }, [joined_public_groups, joinedPublicCategoryGroups, publicgroups, groupsbycategories, SearchedprivateGroups, joinedPrivategroups, categoryname, selectedTab, currentPage, itemsPerPage, totalPages, searchQuery]);
    ...
    <div>
      {loading ? (<Preloader />) : (
        <div>
            ...
            <div onClick={() => {
              ...
              <div onClick={() => { setnavbar_dropdown_visible(false) }}>
                <div className=''>
                  <div className='row align-items-center'>
                    <div className='col-md-8'>
                      ...
                      <h5 className={`fw-bold d-flex align-items-center ${grouptype === "opengroup" ? 'd-flex' : 'd-none'} flex-wrap`} style={{ fontSize: '2rem', fontWeight: 500, lineHeight: 'normal', letterSpacing: '0.4px' }}>
                        ...
                        <span className='page6-head m-0 page6-head-group-title' style={{ marginRight: '16px' }}>{translate_value.group.create_public_group}</span>
                      </h5>
                      ...
                      <h5 className={`fw-bold d-flex align-items-center ${grouptype === "opengroup" ? 'd-none' : 'd-flex'} flex-wrap`} style={{ fontSize: '2rem', fontWeight: 500, lineHeight: 'normal', letterSpacing: '0.4px' }}>
                        ...
                        <span className='page6-head m-0 page6-head-group-title'>{translate_value.group.create_private_group}</span>
                      </h5>
                    </div>
                    {/* Public Group add more group button before ************/}
                    <div className='col-md-4 d-flex justify-content-md-end mt-3 mt-md-0'>

                      <button onClick={() => setAddpublicgroupModal(true)}
                        className={`btn btn-sm border border-primary-subtle text-decoration-none ms-lg-auto d-flex align-items-center ${grouptype === "opengroup" ? '' : 'd-none'}`}
                        style={{
                      ...
                      <button onClick={() => { setAddprivategroupModal(true) }}
                        className={`btn btn-sm border border-primary-subtle text-decoration-none ms-lg-auto d-flex align-items-center ${grouptype === "privategroup" ? '' : 'd-none'}`}
                        style={{
                          padding: '0.5vw 1vw', // Dynamic padding based on viewport width
                          color: '#8587EA',
                          height: '64px',
                    ...
                    <div className='mt-3'>
                      <div className='d-flex flex-wrap' style={{ gap: '10px' }}>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          ...
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}> {translate_value.public_group.row1_A}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-user-friends' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row1_B}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-file-alt' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row1_C}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                        ...
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          ...
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row2_C}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-question" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row3_A}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-handshake" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row3_B}</span>
                        </div>
                        {/* <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-search" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.public_group.row3_C}</span>
                        </div> */}
                      </div>
                    </div>
                  }
                    ...
                    <div className='mt-3'>
                      <div className='d-flex flex-wrap' style={{ gap: '10px' }}>
                        {/* <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          ...
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row1_A}</span>
                        </div> */}
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-users-cog' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row1_B}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-share-alt' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row1_C}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-calendar-alt' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row2_A}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-bullseye' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row2_B}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className='fas fa-book' style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row2_C}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-question" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row3_A}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-tasks" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row3_B}</span>
                        </div>
                        <div className='col-12 col-sm-6 col-md-4 p-2 card-option' style={{ backgroundColor: '#f8f9fa', borderRadius: '8px', flex: '1 1 300px', padding: '20px', transition: 'background-color 0.3s', display: 'flex', alignItems: 'center' }}>
                          <i className="fas fa-lock" style={{ marginRight: '10px', fontSize: '1.5rem', color: '#6f42c1' }}></i>
                          <span className='gradient-text' style={{ fontSize: '1rem', fontWeight: 500, lineHeight: '1.5rem', letterSpacing: '0.32px' }}>{translate_value.private_group.row3_C}</span>
                        </div>
                      </div>
                    </div>
                  }
                  ...
                  <style jsx>{`
                      .card-option:hover {background-color: #e2e6ea;}
                      .gradient-text {
                ...
                <div className="mt-4">
                  ...
                  <div className={`input-group mb-3 bg-white py-2 rounded shadow-sm ${grouptype === "opengroup" ? '' : 'd-none'}`} style={{ border: '2px solid #7A57D1', borderRadius: '8px' }}>
                    ...
                    <input type="text" value={searchvalue} onChange={(e) => {
                      ...
                      handleSearch(e)
                    }} className="form-control border-0 bg-transparent ps-1" placeholder={translate_value.public_group.Search_Public_Group} aria-label="Username" aria-describedby="basic-addon1" />
                  </div>

                  {/* <div className={`${grouptype === "opengroup" && val == true ? '' : 'd-none'}`} style={{ border: '2px solid #7A57D1', borderRadius: '4px', width: '100%' }}>
                    <Select
                      options={cities}
                      value={city}
                  ...
                  <div className={`input-group mb-3 bg-white py-2 rounded shadow-sm ${grouptype === "privategroup" ? '' : 'd-none'}`} style={{ border: '2px solid #7A57D1', borderRadius: '8px' }}>
                    ...
                    }} className="form-control border-0 bg-transparent ps-1" placeholder={translate_value.private_group.Search_Private_Group} aria-label="Search Private Group" aria-describedby="basic-addon1" style={{ color: '#333' }} />
                  </div>
                          ...
                          <li className={`mb-3 py-3 rounded px-2 shadow-sm d-flex align-items-center px-4 bg-white`} key={index}>
                            <div className='d-flex flex-column'>
                              ...
                              <div className='mt-2 d-flex'>
                                ...
                                </svg><span className='ms-1'>{x.member_count}</span> <span className='ms-1 d-lg-inline d-md-inline d-none ms-2'>Members</span></span>
                                {x.city && (
                            ...
                            }} className='btn border border-primary-subtle px-3 px-lg-5 py-2 fw-medium text-decoration-none ms-lg-auto d-flex align-items-center' style={{ height: '44px', color: '#8587EA', fontSize: '19px' }}>{translate_value.common_words.join}</button>
                          </li>
                        )
                      })
                    )}
                  ...
                  <div className={`mt-4 py-2 d-flex ${grouptype === "opengroup" ? '' : 'd-none'}`} style={{ overflowX: 'auto' }}>
                    <div className='rounded-pill py-2 px-3 me-2' onClick={() => {
                      setval(false)
                      setcategoryname("all")
                    }} style={{ fontSize: '14px', lineHeight: "1", whiteSpace: "nowrap", height: "fit-content", width: 'fit-content', cursor: 'pointer', border: categoryname === "all" ? '1px solid #5D5FE3' : '1px solid #5D5FE3', color: categoryname === "all" ? '#fff' : '#5D5FE3', backgroundColor: categoryname === 'all' ? '#5d5fe3' : '#fff' }}>All</div>
                    {
                      categories.map((x, index) => {
                        return (
                          <>
                            <div className='rounded-pill py-2 px-3 me-2' key={index} onClick={() => {
                              setval(true)
                              setCategoryId(x.id)
                              setcategoryname(x.name)
                              // filtergroupsbycategory(x.id)
                              getJoinedPublicCategorywiseGroups(x.id)
                              sendselectedcity_tosearch(city?.value, x.id)
                              setSelectedTab('recommended')
                            }} style={{ fontSize: '14px', height: "fit-content", whiteSpace: "nowrap", lineHeight: "1", width: 'fit-content', cursor: 'pointer', border: categoryname === x.name ? '1px solid #5D5FE3' : '1px solid #AAB0B0', color: categoryname === x.name ? '#fff' : '#5D5FE3', backgroundColor: categoryname === x.name ? '#5d5fe3' : '#fff' }}>{x.name}
                            </div>
                          </>
                        )
                      })
                ...
                <div className="">
                  {(grouptype === "opengroup") ?
                    <div className='d-flex gap-2 gap-lg-5 mb-4'>
                      <div className='' style={{ cursor: 'pointer', borderBottom: selectedTab == 'recommended' ? '5px solid rgb(93, 95, 227)' : '' }} onClick={() => { setSelectedTab('recommended'); setCurrentPage(1) }}>
                      ...
                      <div className='' style={{ cursor: 'pointer', borderBottom: selectedTab == 'public' ? '5px solid rgb(93, 95, 227)' : '' }} onClick={() => { setSelectedTab('public'); setCurrentPage(1) }}>
                        <span className='page6-month' style={{ fontSize: '1.5rem', verticalAlign: 'middle', color: '#7A57D1' }}>{translate_value.public_group.Joined_Public_Groups}</span>
                      </div>
                    </div>
                    :
                    <div className='d-flex gap-2 gap-lg-5 mb-4'>
                      <div className='' style={{ cursor: 'pointer', borderBottom: selectedTab == 'private_searched' ? '5px solid rgb(93, 95, 227)' : '' }} onClick={() => { setSelectedTab('private_searched'); setCurrentPage(1) }}>
                        ...
                        <span className='page6-month' style={{ fontSize: '1.5rem', verticalAlign: 'middle', color: '#7A57D1' }}>{translate_value.group.your_groups}</span>
                      </div>
                      <div className='' style={{ cursor: 'pointer', borderBottom: selectedTab == 'private_joined' ? '5px solid rgb(93, 95, 227)' : '' }} onClick={() => { setSelectedTab('private_joined'); setCurrentPage(1) }}>
                        <span className='page6-month' style={{ fontSize: '1.5rem', verticalAlign: 'middle', color: '#7A57D1' }}>{translate_value.private_group.Joined_Private_Groups}</span>
                      </div>
                    </div>
                  }

                  {/* ---------------------------------------Public groups layout------------------------------------------------ */}
                  <ul className={`ps-0 ${(grouptype === "opengroup" && selectedTab == 'recommended') ? '' : 'd-none'}`} style={{ listStyleType: 'none' }}>
                    {/* -----------------------------------------------Public Groups Section---------------------------------------------------------------- */}
                    {(paginationItems && paginationItems.length) ? (
                      ...
                      <p className='rounded border py-2 text-center'>{translate_value.group.no_recommended_groups_ava}</p>
                    }
                  </ul>
                  ...
                  <div className={`${grouptype === "privategroup" ? '' : 'd-none'}`}>
                    ...
                    {(paginationItems.length > 0 && selectedTab == 'private_searched') ? (
                        ...
                        <ul style={{ listStyleType: 'none', padding: 0 }}>
                          {
                            paginationItems && (
                              paginationItems.map((x, index) => {
                                return (<li className="mb-3 py-3 rounded px-2 shadow-sm d-flex align-items-center bg-white" key={index} style={{ gap: '20px' }}>
                                  <div style={{ width: '80px' }}>
                                    <img src={x.group_image} className='rounded' style={{ width: '100%' }} alt="open-grp" />
                                  </div>
                    ...
                    ) : (selectedTab == 'private_searched') ? <p className='rounded border py-2 text-center'>{'No groups found'}</p> : ''}
                    {/* --------------------------------------------To view all private groups---------------------------------------------- */}
                    <ul style={{ listStyleType: 'none', padding: 0 }}>
                      {(paginationItems && selectedTab == "private_joined") ? (
                        paginationItems.map((x, index) => (
                      ...
                      ) : (selectedTab == 'private_joined') ? <p className='rounded border py-2 text-center'>{'No groups found'}</p> : ''}
                    </ul>
                  </div>
                  {/* -----------------------------------------------JOINED PUBLIC GROUPS---------------------------------------------- */}

                  {(grouptype === "opengroup" && selectedTab == 'public') &&
                    <div>
                      {((paginationItems && paginationItems.length)) ? (
                        paginationItems.map((x, index) => {
                          return (
                            <li className={`mb-3 py-3 rounded px-2 shadow-sm d-flex align-items-center justify-content-between px-4 bg-white`} key={index}>
                              <div className='d-flex flex-column '>
                                <Link to={`/groupchat/opengroup/${x.group_id}`} className='d-flex align-items-center text-decoration-none text-dark' style={{ fontSize: '18px', fontWeight: 450, lineHeight: 'normal', letterSpacing: '0.36px' }}>{x.group_name}</Link>
                                <div className='mt-2 d-flex align-items-center'>
                                  <span className='text-secondary d-flex align-items-center' style={{ fontSize: "15px" }}><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                              ...
                              <button disabled='true' className='btn border border-primary-subtle px-3 px-lg-4 py-2 fw-medium text-decoration-none ms-lg-auto d-flex align-items-center' style={{ height: '44px', color: '#ffff', fontSize: '19px', backgroundColor: '#5d5fe3' }}>{translate_value.public_group.Joined}</button>
                            </li>
                          )
                        })
                      ) : <p className='rounded border py-2 text-center'>{translate_value.group.no_joined_public_groups}</p>}
                ...
                {paginationItems && paginationItems.length ?
                  <div className='mb-3 d-flex justify-content-end'>
                    <button className='px-3 py-2 border' onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1}>Prev</button>
                    {getPageNumbers().map((page) => (
                      <button className='px-3 py-2 border' key={page} onClick={() => setCurrentPage(page)} style={{ margin: "0 5px", color: currentPage === page ? '#fff' : '', backgroundColor: currentPage === page ? 'rgb(122, 87, 209)' : '', fontWeight: currentPage === page ? "bold" : "normal" }}>
                        {page}
                      </button>
                    ))}
                    <button className='px-3 py-2 border' onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages}>Next</button>
                  </div> : ''
                }
          ...
          {/* ADD PUBLIC GROUP MODAL * Actual code update by Vivek
          {/*------------------Start- Updated public group application-form changed by VBM------------------------ */}
          <Modal isOpen={AddpublicgroupModal} size={"lg"} backdrop="static">
            <ModalHeader className="d-flex justify-content-center" style={{ backgroundColor: '#5d5fe3' }}>
                  ...
                  <Form>
                    <div className="row px-2 px-lg-2">
                      <div className="mb-3 col-sm-6">
                        ...
                        <input type="text" id="groupname" name="groupname" className="form-control" value={values.groupname} onChange={handleChange} onBlur={handleBlur} maxLength={50} style={{ borderColor: '#6f42c1' }} placeholder="min 6 characters" />
                        <ErrorMessage className="validation-error" name='groupname' component='div' />
                      </div>
                      <div className="mb-3 col-sm-6">
                        <label htmlFor="formGroupExampleInput2" className="form-label">Category</label>
                        <select className="form-control" name="category" id="formGroupExampleInput2" value={values.category} onChange={handleChange} onBlur={handleBlur} style={{ borderColor: '#6f42c1', backgroundColor: '#e9ecef', appearance: 'none', backgroundImage: 'url("data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 140 140\'><path d=\'M35 55l35 35 35-35z\'/></svg>")', backgroundRepeat: 'no-repeat', backgroundPosition: 'right .7em top 50%', backgroundSize: '1em auto' }}>
                          <option value="">Select Category *</option>
                          {categories.map((category, index) => (
                            <option key={index} value={category.id}>{category.name}</option>
                          ))}
                        </select>
                        <ErrorMessage className="validation-error" name='category' component='div' />
                      </div>
                      <div className="mb-3 col-sm-6">
                        ...
                        {/* <select className="form-control" id="formGroupExampleInput2" onChange={(e) => showcity(e.target.value)} value={city} style={{ borderColor: '#6f42c1', backgroundColor: '#e9ecef', appearance: 'none', backgroundImage: 'url("data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' viewBox=\'0 0 140 140\'><path d=\'M35 55l35 35 35-35z\'/></svg>")', backgroundRepeat: 'no-repeat', backgroundPosition: 'right .7em top 50%', backgroundSize: '1em auto' }} required>
                            <option value="">Select City Name </option>{cities.map((city, index) => (<option key={index} value={city.name}>{city.name}</option>))}</select> */}
                      ...
                      <div className="mb-3 col-12">
                        ...
                        <textarea id="group_description" name="group_description" className="form-control" value={values.group_description} onChange={handleChange} onBlur={handleBlur} style={{ borderColor: '#6f42c1' }} minLength="20" placeholder="Enter at least 20 characters for the group description..." />
                        <ErrorMessage className="validation-error" name='group_description' component='div' />
                      </div>
                    </div>
                    <div className='text-center'>
                      ...
                      {groupimage2 && (<p className='text-center text-muted mt-2'>{groupimage2.name}</p>)}
                    </div>
                    <div className='mb-3 d-flex d-lg-block'>
                      <Field type="checkbox" id="termsCheckbox" name="agreeTerms" className="me-2" />
                      ...
                      <ErrorMessage name="agreeTerms" component="div" className="validation-error" />
                      {termsVisible && (
                    ...
                    <div className='mt-3 text-end'>
                      <button type="submit" style={{ backgroundColor: '#5d5fe3' }} className='text-white fw-medium btn btn-sm'>{translate_value.group.create}</button>
                      <button type="button" onClick={resetAndCloseModal} style={{ backgroundColor: '#5d5fe3' }} className='text-white fw-medium btn btn-sm ms-3'>{translate_value.group.exit}</button>
                    </div>
                  </Form>
                )}
              </Formik>
          ...
          {/*------------------End- Updated public group application form -Chanage by VBM------------------------ */}
          {/*------------------Start- Updated private group application-form changed by VBM------------------------ */}
          <Modal isOpen={AddprivategroupModal} size={"lg"} backdrop="static">
            <ModalHeader className="d-flex justify-content-center" style={{ backgroundColor: '#5d5fe3' }}>
              <h1 className="modal-title fs-5 text-white" id="staticBackdropLabel">Create Your Private Group</h1>
            </ModalHeader>
            <ModalBody>
              <Formik initialValues={initialPrivateValues} validationSchema={validationPrivateSchema} onSubmit={createGroup}>
                {({ values, handleChange, handleBlur }) => (
                  <Form>
                    <div className="row px-2 px-lg-2">
                      <div className="mb-3 col-sm-6">
                      ...
                      <div className="mb-3 col-sm-6">
                        <label htmlFor="formGroupExampleInput2" className="form-label">Scope</label>
                        <input type="text" className="form-control" id="formGroupExampleInput2" placeholder="public/private" onChange={(e) => setscope(e.target.value)} value={scope} style={{ borderColor: '#6f42c1', backgroundColor: '#e9ecef' }} />
                      </div>
                    </div>
                    <div className='text-center'>
                      ...
                      {groupimage && (<p className='text-center text-muted mt-2'>{groupimage.name}</p>)}
                    </div>
                    <div className='mb-3 d-flex d-lg-block'>
                      <Field type="checkbox" id="termsCheckbox" name="agreeTerms" className="me-2" />
                      <label htmlFor="termsCheckbox" className="form-check-label">
                    ...
                    <div className='mt-3 text-end'>
                      <button type="submit" style={{ backgroundColor: '#5d5fe3' }} className='text-white fw-medium btn btn-sm'>{translate_value.group.create}</button>
                      <button type="button" onClick={resetPrivateCloseModal} style={{ backgroundColor: '#5d5fe3' }} className='text-white fw-medium btn btn-sm ms-3'>{translate_value.group.exit}</button>
                    </div>
                  </Form>
                )}
              </Formik>
          ...
          {/*------------------End- Updated private group application-form changed by VBM------------------------ */}
          {/* -----------------------------------------Copy Created Group Id------------------------------------------------------ */}
          <Modal isOpen={public_group_state2} size={"lg"} backdrop="static" centered>
            <ModalHeader className="d-flex justify-content-center" style={{ backgroundColor: '#5d5fe3' }} toggle={() => setpublic_group_state2(false)}>
              <h1 className="modal-title fs-5 text-white" id="staticBackdropLabel">Private Group</h1>
            </ModalHeader>
            <ModalBody>
              <div className="rounded p-2 px-3 mx-autoanimate__animated animate__fadeIn">
                ...
                <div className='bg-white shadow-sm rounded row m-0 my-2 py-3'>
                  <div className="col-1 d-flex align-items-center justify-content-center">
                    {joinedPrivategroups[0] && joinedPrivategroups[0].group_image != undefined && (<img src={joinedPrivategroups[0].group_image} style={{ width: '100%', height: '90%' }} alt="open-grp" />)}
                  </div>
                  ...
                  <div className="col-1 d-flex align-items-center justify-content-center">
                    <button className='btn btn-sm border-0' onClick={() => { copy() }}>
