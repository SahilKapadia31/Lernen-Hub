...
import { ModalFooter, Input, Button, Modal, ModalBody, ModalHeader, Label, FormGroup } from 'reactstrap';
...
const Showpdfpage = ({ settranslated_pdf_url }) => {
  ...
  const [previewbtnstate, setpreviewbtnstate] = useState(false);
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`${ipaddress}/SpecificDocumentDisplay/${user.user_id}/${id}/`)
      .then((response) => {
        ...
        setLoading(false);
        try {
          const docOwner = response?.data?.pdf_data?.[0]?.user_id?.user_id ?? '';
          const currentUser = user?.user_id ?? '';
          const DocDelete = response?.data?.pdf_data?.[0]?.documnet_id?.is_delete ? setDocVisible(true) : setDocVisible(false);
    ...
    axiosInstance.get(`${ipaddress}/DocumentFollowResponse/${id}/`)
      .then((r) => {
        setFollowersdata(r.data); // console.log("Document Followers data fetched",r.data)
      })
  ...
  const postQuestion = async (e) => {
    ...
    if (selection != null) {
      formData.append('coordinatevalues', JSON.stringify({ ...selection, note, page: selection.page }));
    }
  ...
  function handleReplyLike1(discussion_reply_id, discid, index) {
    axiosInstance.delete(`${ipaddress}/CommentRepliesLikes/${user.user_id}/${discussion_reply_id}/`)
      .then((r) => {
  ...
  function handleReplyLike(discussion_reply_id, discid, index) {
    axiosInstance.post(`${ipaddress}/CommentRepliesLikes/${user.user_id}/${discussion_reply_id}/`)
      .then((r) => {
  ...
  const sendRatingToBackend = async (rating) => {
    await axiosInstance.post(`${ipaddress}/DocumentRating/${user.user_id}/${id}/`, rating)
      .then((r) => {
  ...
  const followDocument = () => {
    axiosInstance.post(`${ipaddress}/FavouriteDocs/${user.user_id}/${id}/`)
      .then((r) => {
  ...
  const unfollowDocument = () => {
    axiosInstance.delete(`${ipaddress}/FavouriteDocs/${user.user_id}/${id}/`)
      .then((r) => {
  ...
  const dislikeDocument = () => {
    axiosInstance.post(`${ipaddress}/DisLikeDocuments/${user.user_id}/${id}/`)
      .then((r) => {
  ...
  const sendreport = () => {
    ...
    axiosInstance.post(`${ipaddress}/document_report/${id}/`, formdata)
      ...
      }).catch(() => { console.log("report sending error") })
  }

  // ----------------------------------------------------ADD TO STUDY LIST--------------------------------------------------
  const addtostudylist = () => {
    axiosInstance.post(`${ipaddress}/UserStudyListView/${user.user_id}/${id}/`)
      .then((r) => {
        // console.log(r.data)
  ...
  const extractTextFromPdf = async () => {
    try {
      const response = await axiosInstance.get(pdfurldata.documnet_id.document_url, { responseType: 'arraybuffer' });
  ...
  // ---------------------------------------------Download pdf functionality--old Flow--------------------------------------------------
  ...
  const handlePDFChecksum = async (url, checksum) => {
    ...
    try {
      const result = await axiosInstance.post(`${ipaddress}/GetPDFChecksum/`, formint);
      const resultData = result?.data;
      if (resultData?.status === false && resultData?.message === "Checksum not found") {
  ...
  const handlePreviewPDF = () => {
    const pdfLayout = document.querySelector(".showpdflayout");
    const postLayout = document.querySelector(".showpost");
    if (pdfLayout && postLayout) {
      pdfLayout.classList.toggle("d-none");
      postLayout.classList.toggle("d-none");
    }
    setpreviewbtnstate(!previewbtnstate);
  };
  const getYears = () => {
  ...
  const handelDeleteConfirm = async (response) => {
    try {
      if (response) {
        const Result = await axiosInstance.get(`${ipaddress}/DocumentDelete/${user.user_id}/${id}/`);
        if (Result.status) {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (!isModalOpen && rndRef.current && !ReactDOM.findDOMNode(rndRef.current).contains(e.target)) {
        setSelection(null);
      }
    };
  ...
  const scrollToQue = (id) => {
    ...
    setview_comment('');
    const pageContainer = document.querySelector(`#page-container-${page}`);
    if (pageContainer) {
  ...
  const scrollToPOST = (id) => {
    const myHigh = documentDetails.filter((x) => null !== x.comment.docmid && x.comment.docmid.markup_location == id)
    setview_comment(myHigh[0]?.comment?.ddpid || '')
    const markup_location = myHigh[0].comment.docmid.markup_location;
    const { page, x, y } = markup_location;
    const commentContainer = document.querySelector(`#comment-container-${myHigh[0].comment.ddpid}`);
    if (commentContainer) {
      ...
      {loading ? (<Preloader />) : (
        <div className="d-flex" style={{ position: 'relative' }}>
          <div onClick={() => {
            ...
            {isDocVisible && (
              <div
                style={{
                  ...
                  zIndex: 9998,
                  pointerEvents: 'all', // Allow overlay to block events
                }}
            ...
            {isDocVisible && (
              <div style={{ position: "fixed", width: "50%", top: "50%", left: "27%", zIndex: "9999" }}>
                <div className="card border-1 border-danger rounded-2 p-4">{translate_value.show_pdf.document_delete_note}</div>
              </div>
            )}
            <div onClick={() => { setnavbar_dropdown_visible(false) }} className="row mt-0 m-0 w-100 align-items-center">
              <div className="w-100 p-1 px-4 py-3 showpdf-div pb-4" onClick={() => { setindex1(-1) }}>
                <div className="d-lg-flex justify-content-between pb-2 align-items-center">
                  <Navpath type={"group"} />
                  <div className="d-flex justify-content-lg-end align-items-center">
                    ...
                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                      ...
                      </svg><span className="ms-1">  {!loading && !error && (pdfurldata && (pdfurldata.document_view_count))}</span>
                    </span>

                    {/* ---------------------------------------------Share Document---------------------------------------------------------- */}
                    <span data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="custom-tooltip" data-bs-title="Share the Document">
                    ...
                    <span onClick={() => { toast.warn('You have already reported', { autoClose: 2000 }) }}
                ...
                <OverlayTrigger placement="bottom" delay={{ show: 250, hide: 250 }}
                  overlay={renderTooltip(!loading && !error && pdfurldata?.documnet_id?.title ? pdfurldata.documnet_id.title : "No title available")}
                >
                  <p className="fw-medium d-block text-white mt-3 mt-md-0 pe-auto" style={{ cursor: "pointer", fontSize: '18px', fontWeight: 500, lineHeight: 'normal', letterSpacing: '0.4px' }}>
                    {!loading && !error && pdfurldata?.documnet_id?.title ? (
                      <>
                        {pdfurldata.documnet_id.title.length > 380
                          ? `${pdfurldata.documnet_id.title.slice(0, 380)}...`
                ...
                <div className="m-0 d-flex align-items-center mt-4 gap-lg-3">
                  ...
                  <span style={{ fontSize: '14px', color: '#fff' }} className='ms-3 d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                    ...
                    <span className='ms-2'>{!loading && !error && (pdfurldata && (pdfurldata.documnet_id.pages))}<span className="d-none d-md-inline"> Pages</span></span></span>
                  <span style={{ fontSize: '14px', color: '#fff' }} className='ms-3 d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                    ...
                    <span className='ms-2'>{!loading && !error && (pdfurldata && (pdfurldata.followers_count))}
                  ...
                  </svg><span className='ms-2'>{!loading && !error && (pdfurldata && (pdfurldata.documnet_id.created_on))}</span></span>
                </div>

                <OverlayTrigger placement="bottom" delay={{ show: 250, hide: 250 }}
                  overlay={renderTooltip(!loading && !error && pdfurldata?.documnet_id?.doc_description ? pdfurldata.documnet_id.doc_description : "No Description available")}>
                  <p className="text-white m-0 mt-2" style={{ cursor: "pointer" }}>
                    {!loading && !error && pdfurldata?.documnet_id?.doc_description ? (
                      <><span className="me-2"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" className="bi bi-journal-check" viewBox="0 0 16 16">
                        <path fill-rule="evenodd" d="M10.854 6.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 8.793l2.646-2.647a.5.5 0 0 1 .708 0" />
                        <path d="M3 0h10a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2v-1h1v1a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v1H1V2a2 2 0 0 1 2-2" />
                        <path d="M1 5v-.5a.5.5 0 0 1 1 0V5h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1zm0 3v-.5a.5.5 0 0 1 1 0V8h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1zm0 3v-.5a.5.5 0 0 1 1 0v.5h.5a.5.5 0 0 1 0 1h-2a.5.5 0 0 1 0-1z" />
                      </svg></span>
                        {pdfurldata.documnet_id.doc_description.length > 380 ? `${pdfurldata.documnet_id.doc_description.slice(0, 380)}...` : pdfurldata.documnet_id.doc_description}
                      </>
                    ) : (
                      ""
                    )}
                ...
                <div className="mt-4 d-flex justify-content-end align-items-center gap-3">
                  {pdfurldata && (
                    <Link className='text-white' to={`/profile/${pdfurldata.user_id.user_id}`}>
                      <span className="text-white d-flex align-items-center" style={{ fontSize: '14px' }}>
                        <img src={pdfurldata.user_id.profile_pic != undefined && pdfurldata.user_id.profile_pic} className={`me-2 my-auto ${pdfurldata.user_id.profile_pic != null ? 'rounded-circle' : 'd-none'}`} width={30} height={30} alt="doc-pdf" />
                        ...
                        <span style={{ fontSize: '14px', fontWeight: 450, lineHeight: 'normal', letterSpacing: '0.28px', color: 'white', textDecoration: 'none' }}>
                ...
                <div className="row">
                  <div className="col-lg-8 ms-auto mt-4 mt-lg-0 mt-lg-0 d-flex justify-content-center">
                    ...
                    <span data-bs-toggle="tooltip" data-bs-placement="top"
                      ...
                      <button onClick={handlePreviewPDF} className="btn btn-sm ms-2 d-block d-md-none d-lg-none px-3 py-2" style={{ border: previewbtnstate ? '1px solid #5d5fe3' : '1px solid #445259', color: previewbtnstate ? '#ffff' : '#445259', height: '40px', backgroundColor: previewbtnstate ? '#5D5FE3' : '#ffff' }}>
              ...
              <div className="col-12 mt-4">
                <div className="row bg-light">
                  <div style={{ position: 'sticky', height: '90vh' }} className={`${discussionpagelayout ? 'col-lg-12 pb-4' : 'col-lg-4'} ${documentpagelayout ? 'd-none' : ''} mt-3 mt-lg-0 showpost d-none d-lg-block d-md-block`} id="disc-div">
                    <div style={{ position: 'relative' }}>
                      <div style={{ width: discussionpagelayout ? '802px' : '100%' }} id="btn" className="mx-auto">
                        ...
                        {!isSticky &&
                          <span data-bs-toggle="tooltip" data-bs-placement="top"
                            data-bs-custom-class="custom-tooltip"
                            data-bs-title="Expand the Comment Section" className="border border-dark rounded-circle d-none d-lg-flex d-md-flex justify-content-center align-items-center" onClick={() => { setdiscussionpagelayout(!discussionpagelayout) }} style={{ height: '30px', width: '30px', position: 'absolute', right: '-40px', top: '10px', cursor: 'pointer' }}>
                        ...
                        <div className='bg-white rounded shadow-sm py-3 px-3 d-flex' onClick={() => {
                          setindex1(-1)
                        }}>
                          ...
                          <span data-bs-toggle="tooltip" data-bs-placement="top"
                            ...
                            <button
                              ...
                              className="btn btn-primary d-flex ms-1 align-items-center justify-content-center rounded-circle"
                              style={{
                                backgroundColor: 'rgb(102 112 238)',
                                width: '40px',
                                height: '40px',
                                fontSize: '20px',
                                lineHeight: 'normal',
                          ...
                          <div>
                                  ...
                                  <div className='mb-3' key={index}>
                                    <div className='col-12 bg-white px-3 pt-2 pb-3 rounded border'>
                                      <div className="row align-items-center">
                                        <div className="col-1" onClick={() => { setindex1(-1) }}>
                                          <img src={x.user_id.profile_pic} className={x.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                          {x.user_id.nickname != undefined ? (<p className={x.user_id.profile_pic == null ? 'bg-info text-white rounded-circle my-auto d-flex align-items-center justify-content-center' : 'd-none'} style={{ fontSize: '18px', height: '30px', width: '30px' }}><span>{x.user_id.nickname.slice(0, 1)}</span><span>{x.user_id.nickname.slice(-1)}</span></p>) : (<></>)}
                                        </div>
                                        <div className={`${discussionpagelayout ? 'col-9' : 'col-9'}`} onClick={() => { setindex1(-1) }}>
                                          <Link to={`/profile/${x.user_id.user_id}`} className='ms-3 ms-sm-0 my-0 fw-medium text-dark text-decoration-none' style={{ fontSize: '15px', fontWeight: '700' }}>{x.user_id.nickname}
                                          </Link>
                                        ...
                                        <div className={`col-2 d-flex ${discussionpagelayout && x.pinned_status ? 'justify-content-between' : 'justify-content-end'} p-0 ${x.pinned_status ? 'col-2' : 'col-2'}`}>
                                          <button data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Unfollow" onClick={() => { unpin(x.ddpid) }}
                                          ...
                                          <div className="btn-group dropstart">
                                            ...
                                            <ul className={`bg-white border shadow-sm mt-0 p-0 ps-3 rounded ${index1 === index && dropdownstate ? '' : 'd-none'}`} style={{ top: '10px', width: '105px', position: 'absolute', left: '-95px' }}>
                                              <Button
                                                className={
                                                  x.user_id.user_id !== user.user_id ||
                                                    x.created_on.includes("day") ||
                                                    x.created_on.includes("week") ||
                                                    x.created_on.includes("year")
                                                    ? "d-none"
                                              ...
                                              {/* ----------------------------------------------------Report button--------------------------------------------------- */}
                                              {user.user_id !== x.user_id.user_id &&
                                                <>
                                      ...
                                      <div className='py-2' onClick={() => {
                                        ...
                                        {x.images_attached.map((z) => {
                                          return (
                                            <div>
                                              <img src={z.image} width={300} className="mt-3 img-fluid" alt="doc-pdf" />
                                            </div>
                                          )
                                        })}
                                      </div>
                                      ...
                                      <div className="border-bottom pt-3 pb-3" onClick={() => { setindex1(-1) }}>
                                        <div className='d-flex justify-content-between'>
                                          <div className="d-flex align-items-center">
                                            <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: x.liked_status ? "#ff845d" : "gray", fontSize: '17px' }}
                                              onClick={() => {
                                                const likeHandler = x.liked_status ? handleLike1 : handleLike;
                                                likeHandler(x.ddpid);
                                              }}>
                                              {/* VBM */}
                                              ...
                                              </svg><span style={{ fontSize: '14px' }}>{x.like_count == 0 ? '' : x.like_count}</span>
                                            </button>
                                            {/* -------------------------------------------------Dislike Button----------------------------------------------- */}
                                            <button className='bg-transparent border-0 ms-1 d-flex align-items-center' style={{ height: '20px', color: x.dis_liked_status ? "red" : "gray" }} onClick={() => {
                                      ...
                                      {fetchedreplies && fetchedreplies.length > 0 && (
                                        <div className={fetchedreplies[0].ddpid == x.ddpid && reply_layout_status ? 'd-block' : 'd-none'}>
                                          {fetchedreplies.map((y) => {
                                            ...
                                            return (
                                              <div className='py-2 mt-3' onClick={() => { setindex1(-1) }}>
                                                <div className="row m-0 w-100 align-items-center">
                                                  <div className="col-1 p-0 d-flex justify-content-end">
                                                    <img src={y.user_details.profile_pic} className={y.user_details.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                                  ...
                                                  <div className={`col-8 ps-2 p-0 ${discussionpagelayout ? 'col-lg-9' : 'col-lg-7'}`}>
                                                    <h6 className='ms-3 ms-sm-0 my-0' style={{ fontSize: '14px', fontWeight: '700' }}>{y.user_details.nickname}</h6>
                                                    <p className='ms-3 ms-sm-0 my-0' style={{ fontSize: '13px' }}>{y.created_on}</p>
                                                  </div>
                                                  <div className={`col-3 ${discussionpagelayout ? 'col-lg-2' : 'col-lg-4'} p-0 d-flex justify-content-end align-items-center`}>
                                                ...
                                                <div className='ps-0 ps-lg-5 mt-2 m-0'>
                                                  ...
                                                  {y.image.map((a) => {
                                                    return (
                                                      <div className=''>
                                                        <img src={a.image} style={{ width: '250px' }} alt="doc-pdf" className='mt-3' />
                                                      </div>
                                                    )
                                                  })}
                                                </div>
                                                  ...
                                                  {fetchedreplies_for_reply && fetchedreplies_for_reply.length > 0 && (
                                                    <div className={fetchedreplies_for_reply[0].reply == y.dprid && replies_for_reply_status ? 'd-block' : 'd-none'}>
                                                      {fetchedreplies_for_reply.map((z) => {
                                                        ...
                                                        return (
                                                          <div className={`ps-0 ${discussionpagelayout ? 'ps-md-3' : 'ps-md-0'} py-2 mt-3 bg-white`} onClick={() => { setindex1(-1) }} >
                                                            <div className="row w-100 align-items-center m-0">
                                                              <div className="col-2 col-lg-1 d-flex justify-content-center">
                                                                <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                                              ...
                                                              <div className={`col-3 ${discussionpagelayout ? 'col-lg-2' : 'col-lg-4 ms-auto'} p-0 d-flex justify-content-end align-items-center`}>
                                                              ...
                                                              {z.images_attached.map((b) => {
                                                                return (
                                                                  <div className='d-flex justify-content-center'>
                                                                    <img src={b.images} width={260} alt="doc-pdf" className='mt-3' />
                                                                  </div>
                                                                )
                                                              })}
                                                            </div>
                                      ...
                                      <div className='d-flex gap-2 pt-3 border-secondary-subtle align-items-center' onClick={() => { setindex1(-1) }}>
                                        <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                        ...
                                        <form className="w-100" onSubmit={(e) => postReplies(e, x.ddpid, index)}>
                                          <div className="input-group border rounded pe-3 bg-light">
                                            <input key={index}
                                              type="text"
                                              name={post}
                                              onChange={(e) => { repliesData(e); setSelectedPostForComment(x.ddpid) }}
                                              className="form-control py-1 ps-3 shadow-none border-0 bg-light originalreply-input" placeholder="Post your Reply..."
                                            ...
                                            <div className='d-flex align-items-center bg-light'>
                                              ...
                                              >
                                                <div style={{ width: '25px' }} className={`${(load && selectedPostForComment == x.ddpid) ? '' : 'd-none'}`}>
                                                ...
                                                <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                                  data-bs-custom-class="custom-tooltip"
                                                  data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" className={`${(load && selectedPostForComment == x.ddpid) ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 25 25" fill="none">
                            ...
                            <div className={`${original_status ? '' : 'd-none'}`}>
                              {documentDetails.map((x, index) => {
                                return (
                                  <div id={`comment-container-${x.comment.ddpid}`} className={`mb-3 me-2 ${view_mark_comment === x.comment.ddpid ? 'rounded purple-border' : ''}`} key={index}>
                                    <div className='col-12 bg-white px-3 pt-2 pb-3 rounded border'>
                                      <div className="row align-items-center">
                                        <div className="col-1" onClick={() => { setindex1(-1) }}>
                                          <img src={x.user.profile_pic} className={x.user.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                          {x.user.nickname != undefined ? (<p className={x.user.profile_pic == null ? 'bg-info text-white rounded-circle my-auto d-flex align-items-center justify-content-center' : 'd-none'} style={{ fontSize: '18px', height: '30px', width: '30px' }}><span>{x.user.nickname.slice(0, 1)}</span><span>{x.user.nickname.slice(-1)}</span></p>) : (<></>)}
                                        </div>
                                        <div className={`${discussionpagelayout ? 'col-9' : 'col-9'}`} onClick={() => { setindex1(-1) }}>
                                          <Link to={`/profile/${x.user.user_id}`} className='ms-3 ms-sm-0 my-0 fw-medium text-dark text-decoration-none' style={{ fontSize: '15px', fontWeight: '700' }}>{x.user.nickname}</Link>
                                        ...
                                        <div className={`col-2 d-flex align-items-center ${discussionpagelayout && x.pinned_status ? 'justify-content-between' : 'justify-content-end'} p-0 ${x.pinned_status ? 'col-2' : 'col-2'}`}>
                                          <button data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="custom-tooltip" data-bs-title="Unfollow" onClick={() => { unpin(x.comment.ddpid) }}
                                            className={`p-1 btn btn-sm border px-2 ${x.pinned_status && discussionpagelayout ? '' : 'd-none'}`} style={{ cursor: 'pointer' }}><i className="fa-solid fa-link-slash d-md-none d-inline"></i><span className='d-none d-md-inline'>Followed</span></button>
                                          <button data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="custom-tooltip" data-bs-title="Unfollow" onClick={() => { unpin(x.comment.ddpid) }}
                                            className={`p-1 btn btn-sm p-0 px-2 me-2 border-0 ${x.pinned_status && discussionpagelayout == false ? '' : 'd-none'}`} style={{ cursor: 'pointer' }}> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pin" viewBox="0 0 16 16">
                                              <path d="M4.146.146A.5.5 0 0 1 4.5 0h7a.5.5 0 0 1 .5.5c0 .68-.342 1.174-.646 1.479-.126.125-.25.224-.354.298v4.431l.078.048c.203.127.476.314.751.555C12.36 7.775 13 8.527 13 9.5a.5.5 0 0 1-.5.5h-4v4.5c0 .276-.224 1.5-.5 1.5s-.5-1.224-.5-1.5V10h-4a.5.5 0 0 1-.5-.5c0-.973.64-1.725 1.17-2.189A6 6 0 0 1 5 6.708V2.277a3 3 0 0 1-.354-.298C4.342 1.674 4 1.179 4 .5a.5.5 0 0 1 .146-.354m1.58 1.408-.002-.001zm-.002-.001.002.001A.5.5 0 0 1 6 2v5a.5.5 0 0 1-.276.447h-.002l-.012.007-.054.03a5 5 0 0 0-.827.58c-.318.278-.585.596-.725.936h7.792c-.14-.34-.407-.658-.725-.936a5 5 0 0 0-.881-.61l-.012-.006h-.002A.5.5 0 0 1 10 7V2a.5.5 0 0 1 .295-.458 1.8 1.8 0 0 0 .351-.271c.08-.08.155-.17.214-.271H5.14q.091.15.214.271a1.8 1.8 0 0 0 .37.282" />
                                          ...
                                          <div className="btn-group dropstart">
                                            <span className="border-0" type="button" style={{ position: 'relative' }} onClick={() => handleDropdownClick(index)}>
                                              <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 35 35" fill="none">
                                                <path d="M17.4998 27.0354C17.0988 27.0354 16.7555 26.8926 16.4699 26.607C16.1843 26.3214 16.0415 25.9781 16.0415 25.5771C16.0415 25.176 16.1843 24.8327 16.4699 24.5471C16.7555 24.2615 17.0988 24.1187 17.4998 24.1187C17.9009 24.1187 18.2442 24.2615 18.5298 24.5471C18.8154 24.8327 18.9582 25.176 18.9582 25.5771C18.9582 25.9781 18.8154 26.3214 18.5298 26.607C18.2442 26.8926 17.9009 27.0354 17.4998 27.0354ZM17.4998 18.9585C17.0988 18.9585 16.7555 18.8157 16.4699 18.5301C16.1843 18.2445 16.0415 17.9012 16.0415 17.5001C16.0415 17.0991 16.1843 16.7558 16.4699 16.4702C16.7555 16.1846 17.0988 16.0418 17.4998 16.0418C17.9009 16.0418 18.2442 16.1846 18.5298 16.4702C18.8154 16.7558 18.9582 17.0991 18.9582 17.5001C18.9582 17.9012 18.8154 18.2445 18.5298 18.5301C18.2442 18.8157 17.9009 18.9585 17.4998 18.9585ZM17.4998 10.8815C17.0988 10.8815 16.7555 10.7388 16.4699 10.4532C16.1843 10.1676 16.0415 9.82424 16.0415 9.42318C16.0415 9.02214 16.1843 8.67882 16.4699 8.39323C16.7555 8.10764 17.0988 7.96484 17.4998 7.96484C17.9009 7.96484 18.2442 8.10764 18.5298 8.39323C18.8154 8.67882 18.9582 9.02214 18.9582 9.42318C18.9582 9.82424 18.8154 10.1676 18.5298 10.4532C18.2442 10.7388 17.9009 10.8815 17.4998 10.8815Z" fill="#2A3941" />
                                              </svg>
                                            </span>
                                            <ul className={`bg-white border shadow-sm mt-0 p-0 ps-3 rounded ${index1 === index && dropdownstate ? '' : 'd-none'}`} style={{ top: '10px', width: '105px', position: 'absolute', left: '-95px' }}>
                                              <button className={x.user.user_id != user.user_id || x.comment.created_on.includes("day") || x.comment.created_on.includes("week") || x.comment.created_on.includes("year") ? 'd-none' : 'd-flex align-items-center bg-transparent border-0 my-2 d-flex align-items-center'} onClick={() => {
                                      ...
                                      <div className='py-2' onClick={() => { setindex1(-1) }}>
                                      ...
                                      <div className="border-bottom pt-3 pb-3" onClick={() => { setindex1(-1) }}>
                                        <div className='d-flex justify-content-between'>
                                          <div className="d-flex align-items-center">
                                            ...
                                            {/* -------------------------------------------------Dislike Button----------------------------------------------- */}
                                            <button className='bg-transparent border-0 ms-1 d-flex align-items-center' style={{ height: '20px', color: x.dis_like_status ? "red" : "gray" }} onClick={() => {
                                        ...
                                        {x.comment.docmid &&
                                          <div className=" mt-4 px-2">
                                            <button onClick={() => { scrollToQue(x.comment.ddpid) }} className="btn btn-sm w-100 py-2" style={{ backgroundColor: '#F3F0FF', color: '#5D5FE3', border: '1px solid #5D5FE3', fontWeight: 500, fontSize: '16px', letterSpacing: '0.32px', lineHeight: '22px' }}>View Mark</button>
                                          </div>
                                        }
                                      </div>
                                      {/* --------------------------------Replies section for the particular comment-------------------------------------- */}
                                            ...
                                            return (
                                              <div className='py-2 mt-3' onClick={() => { setindex1(-1) }}>
                                                <div className="row m-0 w-100 align-items-center">
                                                  ...
                                                  <div className={`col-3 ${discussionpagelayout ? 'col-lg-2' : 'col-lg-4'} p-0 d-flex justify-content-end align-items-center`}>
                                                    <button className='bg-transparent border-0 d-flex align-items-center pe-1' style={{ height: '20px', color: y.liked_status ? "#ff845d" : "gray" }}
                                                      onClick={() => {
                                                        if (y.liked_status == true) {
                                                    ...
                                                    >
                                                      ...
                                                      {y.like_count == 0 ? '' : y.like_count}
                                                    </button>
                                                    <button className='bg-transparent border-0 d-flex align-items-center pe-1' style={{ height: '20px', color: y.disliked_status ? "#ff845d" : "gray" }} onClick={() => {
                                                      handlereplydislike(y.dprid, x.comment.ddpid, index)
                                                    }}><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 30 30" fill="none">
                                                        <path d="M8.17306 4.99983H20.7212V18.7498L12.8846 26.4902L12.3317 25.9373C12.2179 25.8236 12.1222 25.6777 12.0445 25.4998C11.9667 25.322 11.9279 25.1585 11.9279 25.0095V24.8123L13.2067 18.7498H4.51922C3.99678 18.7498 3.52963 18.5439 3.11778 18.132C2.70593 17.7202 2.5 17.253 2.5 16.7306V15.1921C2.5 15.0783 2.51282 14.9541 2.53847 14.8195C2.56409 14.6849 2.59774 14.5607 2.63941 14.447L6.05288 6.35558C6.22435 5.97097 6.51281 5.64886 6.91825 5.38923C7.32371 5.12963 7.74198 4.99983 8.17306 4.99983ZM19.4712 6.24983H8.17306C7.99679 6.24983 7.81651 6.2979 7.63222 6.39404C7.44793 6.49021 7.3077 6.65047 7.21153 6.87483L3.75 14.9998V16.7306C3.75 16.955 3.82211 17.1393 3.96634 17.2835C4.11057 17.4277 4.29486 17.4998 4.51922 17.4998H14.7596L13.3125 24.3268L19.4712 18.2162V6.24983ZM20.7212 18.7498V17.4998H25V6.24983H20.7212V4.99983H26.25V18.7498H20.7212Z" fill="currentColor" />
                                                      </svg></button>
                                                    <button className={user.first_name === y.user_details.first_name ? 'bg-transparent border-0 d-flex align-items-center pe-1' : 'd-none'} onClick={() => {
                                      ...
                                      <div className='d-flex gap-2 pt-3 border-secondary-subtle align-items-center' onClick={() => { setindex1(-1) }}>
                                        <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="doc-pdf" />
                                        {userdetails.nickname != undefined ? (<p className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ fontSize: '14px', height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>) : (<></>)}
                                        <form className="w-100" onSubmit={(e) => postReplies(e, x.comment.ddpid, index)}>
                                          <div className="input-group border rounded pe-3 bg-light">
                                            <input key={index}
                                              type="text"
                                              name={post}
                                              onChange={(e) => { repliesData(e); setSelectedPostForComment(x.comment.ddpid) }}
                                              className="form-control py-1 ps-3 shadow-none border-0 bg-light originalreply-input" placeholder="Post your Reply..."
                                              style={{ position: 'relative' }} />
                                            <div className='d-flex align-items-center bg-light'>
                                              <input
                                              ...
                                              <button disabled={replies.length > 0 ? false : true}
                                                ...
                                                className='h-100 bg-transparent border-0 ms-2'
                                              >
                                                <div style={{ width: '25px' }} className={`${(load && selectedPostForComment == x.comment.ddpid) ? '' : 'd-none'}`}>
                                                  <div className={`spinner-border spinner-border-sm`} role="status">
                                                    <span className="visually-hidden">Loading...</span>
                                                  </div>
                                                </div>
                                                <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                                  data-bs-custom-class="custom-tooltip"
                                                  data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" className={`${(load && selectedPostForComment == x.comment.ddpid) ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 25 25" fill="none">
                                                  <path d="M4.1665 19.2707V5.729L20.2322 12.4998L4.1665 19.2707ZM5.20817 17.7082L17.5519 12.4998L5.20817 7.2915V11.338L10.2562 12.4998L5.20817 13.6617V17.7082Z" fill="#8E9696" />
                                                </svg>
                                              </button>
                                            </div>
                  ...
                  <Modal isOpen={isModalOpen} toggle={() => setIsModalOpen(!isModalOpen)} centered size="lg">
                    <ModalHeader toggle={() => setIsModalOpen(!isModalOpen)}>Post Your Comments</ModalHeader>
                    <ModalBody>
                      <div className="col-12 bg-white px-lg-3 pt-2">
                        <div className="d-flex gap-1 gap-lg-3 align-items-center">
                          <img
                            src={userdetails.profile_pic}
                            className={userdetails.profile_pic ? 'rounded-circle' : 'd-none'}
                            width={40}
                            height={40}
                            alt="doc-pdf"
                          />
                            ...
                            <div className="input-group bg-light rounded border py-1">
                              ...
                              <div className="bg-light">
                                <Input
                                  ...
                                  className="d-none"
                                />
                                <Label htmlFor="fileInput" className="custom-file-input bg-transparent border-0 px-lg-1 py-1 mb-0">
                                ...
                                <Button type="submit" className="cursor-pointer mb-0" color="link" size="sm" disabled={discussionquestion.length === 0}>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 30 30" fill="none"><path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696"></path></svg>
                                </Button>
                              </div>
                            </div>
                          </form>
                        ...
                        <p className="mt-1 ms-5" style={{ color: '#ff845d', fontSize: '13px', fontStyle: 'italic' }}>
                  ...
                  <Modal isOpen={isModalPostEditOpen} toggle={togglePostEditModal} centered size="lg">
                    <ModalHeader toggle={togglePostEditModal}>Create a Post</ModalHeader>
                    <ModalBody>
                      <div className="bg-white px-3 pt-2 pb-3 rounded">
                        ...
                        <div className="d-flex gap-3 align-items-center">
                          <img
                            src={userdetails.profile_pic}
                            className={
                              userdetails.profile_pic == null ? "d-none" : "rounded-circle"
                            }
                            width={40}
                            height={40}
                            alt="doc-pdf"
                  ...
                  <div className={`col-lg-8 mx-auto ${discussionpagelayout ? 'd-none' : ''}`} id="btn" style={{ position: 'relative' }}>
                    <div className="mx-auto d-none d-md-block" id="pdf-div" style={{ width: documentpagelayout ? '802px' : '100%' }}>
                      ...
                      <div className="px-4">
                        <div className={`button-container rounded d-flex justify-content-center align-items-center border border-bottom-0 py-1 mb-2 ${isSticky ? 'sticky2' : ''}`} style={{ backgroundColor: '#5d5fe3' }}>
                          <button className="btn text-white btn-sm border-0 me-2" onClick={handleZoomIn} disabled={pageScale >= 2}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
                          ...
                          {/* <span className="border border-white rounded text-white py-2 px-2" style={{ fontSize: '14px', width: '50px' }}><span className={pageScale === 1 ? '' : 'd-none'}>100</span>
                            ...
                            %</span> */}
                          <span className="border border-white rounded text-white py-2 px-2" style={{ fontSize: "14px", width: "50px", textAlign: "center" }}>
                            {Math.round(pageScale * 100)}%
                          </span>
                          <button className="btn text-white btn-sm ms-2 border-0" onClick={handleZoomOut} disabled={pageScale <= 0.2}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 35 35" fill="none">
                      ...
                      {isNotify && (
                        <div className="waviy">
                          <center>
                            <span style={{ "--i": 1 }}>click to set a markup on the desired location.”</span>
                          </center>
                        </div>
                      )}
                      {!loading && !error && (
                        <div id="pdf-container" className="border pdf-layout d-flex justify-content-center" style={{ position: 'relative', marginBottom: '20px', zIndex: '1px', width: '100%' }}>
                          {pdfurldata && (
                            <div className="pdf-container" style={{ height: '80vh', overflowY: 'scroll', position: 'relative', background: '#f5f5f5', userSelect: 'none', }}>
                              <Document file={pdfurldata.documnet_id.document_url} onLoadSuccess={onDocumentLoadSuccess}>
                                {Array.from(new Array(numPages), (_, index) => (
                                  <div
                                    ...
                                    onDoubleClick={(e) => {
                                      ...
                                      activateRectangle(e, index + 1); // Pass the page index
                                    }}
                                    onClick={(e) => {
                                      e.preventDefault(); // Prevent selection behavior
                                      e.stopPropagation(); // Stop event from propagating
                                  ...
                                  >
                                    ...
                                    <Page pageNumber={index + 1} width={600} scale={pageScale} style={{ display: index + 1 === currentPage ? 'block' : 'none', pointerEvents: 'none', userSelect: 'none', }} />
                                    {highlights
                                      .filter((highlight) => highlight.page === index + 1)
                                      .map((highlight, idx) => {
                                        const adjustedStyle = adjustHighlightPosition(highlight);
                                        return (
                                          <div
                                            key={idx}
                                            className={`highlight-box ${view_mark_id === highlight ? "markup-color1" : "markup-color2"
                                              }`}
                                            style={{
                                              position: 'absolute',
                                            ...
                                            onClick={() => scrollToPOST(highlight)}
                                          >
                                            {/* <span className="highlight-note-label">Mark {idx + 1}</span> */}
                                          </div>
                                        );
                                      })}

                                    {/* Render RND selection on this page if it belongs here */}
                                    {/* {selection && selection.page === index + 1 && (
                                      <Rnd
                                        bounds="parent"
                                        ref={rndRef}
                                        ...
                                        onDragStop={(e, d) =>
                                          setSelection({ ...selection, x: d.x, y: d.y })
                                        }
                                    ...
                                    {selection && selection.page === index + 1 && (
                                      <Rnd
                                        ...
                                        ref={rndRef}
                                        size={{
                                          width: selection.width * pageScale,
                                          height: selection.height * pageScale,
                                        }}
                                        position={{
                                        ...
                                        onResizeStop={(e, direction, ref, delta, position) => {
                                          setSelection({
                                            x: position.x / pageScale, // Normalize to original scale
                                            y: position.y / pageScale, // Normalize to original scale
                                            width: parseInt(ref.style.width, 10) / pageScale, // Normalize to original scale
                                            height: parseInt(ref.style.height, 10) / pageScale, // Normalize to original scale
                                            page: selection.page,
                                          });
                                        }}
                                        className="selection-box"
                  ...
                  <div onClick={() => { setindex1(-1) }} className={`col-12 mx-auto d-md-none  d-block showpdflayout`}>
                    <div className="mx-auto">
                      ...
                      {error && <p>{error}</p>}
                      {!loading && !error && (
                        <div id="pdf-container mt-3" className="border d-flex justify-content-center" style={{ position: 'relative', marginBottom: '20px', zIndex: '1px', width: '100%', overflowX: 'scroll' }}>
                          {pdfurldata && (
                            <Document className=""
                    ...
                    <div className='row py-3 py-md-4 mx-auto d-flex bg-white align-items-center px-3'>
                      <div className={`mb-4 col-md-12`}>
                        ...
                        <input
                          ...
                          onBlur={handleBlur} />
                        <ErrorMessage className="validation-error" name='document_title' component='div' />
                      </div>
                      <div className='mt-4 text-center'>
                        <button type="submit" className='btn text-white fw-medium' style={{ backgroundColor: '#5D5FE3' }}>Save</button>
                      </div>
                    </div>
                  </div>
                </Form>
              ...
              <div className="modal-content">
                <div className="modal-body">
                  <div className='col-12 bg-white px-3 pt-2 pb-3'>
                    ...
                    <div className='d-flex gap-3 align-items-center'>
                      <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="doc-pdf" />
                      ...
                      <form className="w-100" onSubmit={(e) => { postreply_for_replies(e) }}>
                        <div className="input-group bg-light border rounded pe-3">
                          <input
                            ...
                            className="form-control py-1 ps-3 bg-light border-0 shadow-none post-input" placeholder="Post your reply....."
                            style={{ position: 'relative' }} />
                          <div className='d-flex align-items-center bg-light' data-bs-toggle="tooltip" data-bs-placement="left"
                            ...
                            <label data-bs-toggle="tooltip" data-bs-placement="top"
                              ...
                              htmlFor="fileInput2"
                              className="custom-file-input bg-transparent border-0 px-4 py-2">
                              <img src={require('../img/attachment.png')} width={22} height={22} alt="doc-pdf" />
                            </label>

                            <button data-bs-toggle="tooltip" data-bs-placement="left"
                              data-bs-custom-class="custom-tooltip"
                              type="submit"
                              data-bs-title="Post" disabled={reply_for_reply.length > 0 ? false : true} data-bs-dismiss="modal" className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0'>
                              <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                data-bs-custom-class="custom-tooltip"
          ...
          {(deleteDocModal) &&
            <Modal isOpen={deleteDocModal} toggle={() => setDeleteDocModal(false)} centered>
              <ModalHeader toggle={() => setDeleteDocModal(false)}>Delete the document</ModalHeader>
              <ModalBody> Type "delete" and click "Confirm" to confirm  deleting the selected document
                <Input
                  label="delete"
                  placeholder="delete"
                  type="text"
              ...
              <ModalFooter>
                <Button color="secondary" onClick={() => { setDeleteDocModal(false); setdeleteMessage(""); }}>Cancel</Button>
                <Button color="danger" disabled={(deleteMessage == 'delete') ? false : true} onClick={(response) => handelDeleteConfirm(response)}>Confirm</Button>
              </ModalFooter>
            </Modal>
          }
          ...
          <Viewpdf_page view_pdf_status={view_pdf_status} setview_pdf_status={setview_pdf_status} url={pdfurldata.documnet_id.document_url} doc_name={pdfurldata.documnet_id.doc_name} />
          <Backtotop />
        </div>
      )
      }
