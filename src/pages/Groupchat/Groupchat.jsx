...
const Groupchat = () => {
  ...
  // -------------------------------------------------Set Path-------------------------------------------------------------
  setEncryptedData("name2", JSON.stringify('Groups'), 180);
  setEncryptedData("path2", JSON.stringify(`/groupchat/${grouptype}/${group_id}`), 180);
  removeData("name3");
  removeData("path3");
  ...
  const fetchgroupdetails = () => {
    axiosInstance.get(`${ipaddress}/DisplayGroupDetails/${group_id}/${user.user_id}/`)
      .then((r) => {
        setGroupdetails(r.data); //console.log("Particular Group Details", r.data)
        console.log(r.data);
        const parts = location.pathname.split("/");
        const group = `/groups/${parts[2]}`; // "/groupchat/opengroup/"
        setNavPaths([
          { name: "Dashboard", path: "/dashboard/page" },
          { name: "Groups", path: group }, // Adjust path as needed
          { name: r.data.group_name, path: location.pathname }
        ]);
        console.log("----------nav_paths----------", nav_paths);
      }).catch((err) => { console.log("Particular group error", err) })
  }
  ...
  const fetchdocs = () => {
    ...
    axiosInstance.get(`${ipaddress}/GroupDocuments/${user.user_id}/${group_id}/`)
      .then((r) => {
        const docs = r.data.reverse();  // console.log("Particular Group Documents",r.data)
        setGroupdocuments(docs);
        setsubLoading(false);
      }).catch((err) => { console.log("Particular group documents error", err) })
  }
  ...
  const getallflashsetsundergroup = () => {
    ...
    axiosInstance.get(`${ipaddress2}/GroupFlashCardSetRetrieveView/${group_id}/${user.user_id}/`)
      .then((r) => {
        setFlashsets(r.data); // console.log("Private Group Flashsets Fetched Successfully :",r.data)
        setsubLoading(false);
      }).catch(() => { console.log("Private Group Flashsets fetching Error") })
  }
  // ------------------------------------------------FILTER DOCUMENTS-------------------------------------------------------
  ...
  const filterfunction = (value) => {
    axiosInstance.get(value)
      .then((r) => {
        setFiltereddocuments(r.data); // console.log("Group Documents Filtered Successfuly",r.data)
        document.getElementById('original').style.display = 'none';
        document.getElementById('filterlike').style.display = 'block';
      }).catch((err) => { console.log("filterfunction error", err) })
  }
  // ----------------------------------------Filter flashsets----------------------------------------------------
  const [searchedFlashsets, setSearchedFlashsets] = useState([])
  const filterflashsets = (value) => {
    if (value === "likes") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersGroups/${user.user_id}/${group_id}/`, { 'like': true, 'datetime': false })
        .then((r) => {
          setSearchedFlashsets(r.data); // document.getElementById('flashsetoriginal').style.display='none'
        }).catch((err) => { console.log("filterflashsets error", err) })
    }
    if (value === "date") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersGroups/${user.user_id}/${group_id}/`, { 'like': false, 'datetime': true })
        .then((r) => {
          setSearchedFlashsets(r.data); // document.getElementById('flashsetoriginal').style.display='none'
        }).catch((err) => { console.log("filterflashsets error", err) })
    }
    if (value === "select_filter") { setSearchedFlashsets([]) }
  }
  ...
  const repliesData = (e) => { setReplies(e.target.value) }
  // ----------------------------------------POST IMAGE UPLOAD--------------------------------------------------------
  ...
  const removemain_image = (index) => { setSelectedImages((prevImages) => prevImages.filter((_, i) => i !== index)) };
  const postQuestion = async (e) => {
    ...
    if (question.length > 0) {
      try {
        ...
        } else { console.error('Group discussion posting error') }
      } catch (error) {
  ...
  const clearInput = (index) => {
    ...
    if (inputElement) { inputElement[index].value = '' }
  };
  ...
  const postreply_for_replies = async (e) => {
    e.preventDefault();
    const formData = new FormData();
    for (const file of replies_reply_Image) { formData.append('images_attached', file) }
  ...
  const getdiscussion = () => {
    axiosInstance.get(`${ipaddress}/NewGroupDiscussionView/${group_id}/${user.user_id}/`)
      ...
      }).catch((err) => { console.log("Particular group discussion error", err) })
  }
  useEffect(() => {
    setSearchcomment("");
    getdiscussion();
  ...
  function handleLike1(discussion_id) {
    axiosInstance.delete(`${ipaddress}/NewGroupDiscussionLikeView/${user.user_id}/${group_id}/${discussion_id}/`)
      .then((r) => {
        setCount(count + 1); // console.log("User Unliked the Post",r.data)
  ...
  function handleLike(discussion_id) {
    axiosInstance.post(`${ipaddress}/NewGroupDiscussionLikeView/${user.user_id}/${group_id}/${discussion_id}/`)
      .then((r) => {
        setCount(count + 1); //  console.log("User liked the Post",r.data)
  ...
  function handledislike(discussion_id) {
    axiosInstance.post(`${ipaddress}/NewGroupDiscussionDisLikeView/${user.user_id}/${group_id}/${discussion_id}/`)
      .then((r) => {
        setCount(count + 1); // console.log("User Disliked the Post",r.data)
      }).catch(() => { console.log("User like error") })
  }
  //  ------------------------------------------------Reply Likes---------------------------------------------------------
  function handleReplyLike1(disc_reply_id, discid, index) {
    axiosInstance.delete(`${ipaddress}/NewGroupDiscussionReplyLikeView/${user.user_id}/${discid}/${disc_reply_id}/`)
      .then((r) => {
        getreplies(discid, index); //console.log("Group Unliked the Reply",r.data)
      }).catch(() => { console.log("Group Reply Unlike error") })
  }
  function handleReplyLike(disc_reply_id, discid, index) {
    axiosInstance.post(`${ipaddress}/NewGroupDiscussionReplyLikeView/${user.user_id}/${discid}/${disc_reply_id}/`)
      .then((r) => {
        getreplies(discid, index); // console.log("Group liked the Reply",r.data)
      }).catch(() => { console.log("Group Reply like error") })
  }
  ...
  function handleReplyDisLike(disc_reply_id, discid, index) {
    axiosInstance.post(`${ipaddress}/NewGroupDiscussionReplyDisLikeView/${user.user_id}/${discid}/${disc_reply_id}/`)
      .then((r) => {
        getreplies(discid, index); //console.log("Group disliked the Reply", r.data)
      }).catch(() => { console.log("Group Reply dislike error") })
  }
  ...
  const sendEditedData = (e) => {
    ...
    axiosInstance.patch(`${ipaddress}/NewGroupDiscussionView/${discussionId}/${user.user_id}/`, formData)
      .then((r) => {
        setCount1(count1 + 1); // console.log("Post Edited Successfully",r.data)
  ...
  const getreplies = (discussion_id, index) => {
    ...
    axiosInstance.get(`${ipaddress}/NewGroupDiscussionReplyView/${user.user_id}/${discussion_id}/${group_id}/`)
      .then((r) => {
        setFetchedreplies(r.data.reverse()); // console.log("Group Replies fetched successfully",r.data)
        setCount1(count1 + 1);
      }).catch(() => { console.log("getreplies Error") })
  }
  ...
  const getreplies_for_reply = (particular_reply_id) => {
    axiosInstance.get(`${ipaddress}/GroupDiscussionRepliesReplyView/${user.user_id}/${particular_reply_id}/`)
      .then((r) => {
        setFetchedreplies_for_reply(r.data.reverse()); // console.log("Group Replies under reply fetched successfully",r.data)
        // setCount(count+1)
      }).catch(() => { console.log("getreplies_for_reply Error") })
  }
  // ---------------------------------------------------DELETE POST--------------------------------------------------------
  const deletePost = (discussion_id) => {
    axiosInstance.delete(`${ipaddress}/NewGroupDiscussionView/${discussion_id}/${user.user_id}/`)
      .then((r) => {
        setCount1(count1 + 1); // console.log("Post Successfully Deleted")
        toast.success('Comment deleted successfully', { autoClose: 2000 });
        setindex1(-1);
        setCount(count + 1);
      }).catch(() => { console.log("Post Delete Error") })
  }
  ...
  const deleteReply = (disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/NewGroupDiscussionReplyView/${user.user_id}/${disc_reply_id}/${group_id}/`)
      .then((r) => {
        ...
        setCount(count + 1);  // console.log("Group Reply Successfully Deleted",r.data)
  ...
  const deleteReply_for_reply = (reply_reply_id, disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/GroupDiscussionRepliesReplyView/${user.user_id}/${reply_reply_id}/`)
      .then((r) => {
        getreplies(discid);  // console.log("Replies reply deleted successfully",r.data)
  ...
  const pincomment = (discId) => {
    axiosInstance.post(`${ipaddress}/UserPinnedComments/${user.user_id}/${group_id}/${discId}/`)
      ...
      }).catch((err) => { console.log("Pincomment Error", err) })
  }
  ...
  const unpin = (discid) => {
    axiosInstance.delete(`${ipaddress}/UserPinnedComments/${user.user_id}/${group_id}/${discid}/`)
      .then((r) => {
        getdiscussion(); // console.log("UnPinned Successfully",r.data)
        toast.success('Comment unfollowed successfully', { autoClose: 2000 });
        setCount((prev) => prev + 1);
      }).catch(() => { console.log("Unpinning error") });
  }
    ...
    axiosInstance.post(`${ipaddress}/ReportGroup/${group_id}/${user.user_id}/`, report)
      .then((r) => {
        ...
        if (r.data === "Already Reported") {
          toast.warn('Already reported', { autoClose: 2000 });
        ...
        setreportdropdownstate(false);
        fetchgroupdetails();
      }).catch((err) => { console.log("ReportGroup Error", err) });
  }
  ...
  const searchDocuments = (value) => {
    if (value.length > 0) {
      axiosInstance.post(`${ipaddress}/SearchFlashcardsDocumentsUnderPrivateGroup/${user.user_id}/${group_id}/${value}/`)
        .then((r) => {
          setFiltereddocuments(r.data.documents); // console.log("Search documents under group found",r.data)
          if (r.data.documents.length > 0) {
            document.getElementById('original').style.display = 'none';
            document.getElementById('filterlike').style.display = 'block';
          }
        }).catch((err) => { console.log("SearchFlashcardsDocumentsUnderPrivateGroup Error", err) })
    } else {
  ...
  const searchFlashsets = (value) => {
    if (value.length > 0) {
      axiosInstance.get(`${ipaddress2}/SearchFlashsetsGroups/${user.user_id}/${group_id}/${value}/`)
        .then((r) => {
          setSearchedFlashsets(r.data);  // console.log("Search flashsetsunder group found",r.data)
          if (r.data.length > 0) {
  ...
  const leaveGroup = () => {
    axiosInstance.post(`${ipaddress}/exitGroup/${user.user_id}/${group_id}/`, { 'reason': exitgroup_reason })
      .then((r) => {
        navigate('/dashboard/page');
      }).catch((err) => { console.log("exitGroup Error", err) })
  }
  // -----------------------------------Make other members as Admin-----------------------------------------------------------
  const makeAdmin = (user_id, nickname) => {
    axiosInstance.put(`${ipaddress}/makeadmin/${user_id}/${group_id}/`)
      .then((r) => {
        fetchgroupdetails(); // console.log("Make as admin successfull",r.data)
        setCount(count + 1);
        toast.success(`Successfully make ${nickname} as Admin`, { autoClose: 2000 });
      }).catch((err) => { console.log("makeadmin Error", err) })
  }
  ...
  const kickout = (user_id, nickname) => {
    axiosInstance.delete(`${ipaddress}/kickout/${user_id}/${group_id}/`)
      ...
      }).catch((err) => { console.log("kickout Error", err) })
  }
  ...
  const unsave = (doc_id) => {
    axiosInstance.delete(`${ipaddress}/UserStudyListView/${user.user_id}/${doc_id}/`)
      .then((r) => {
        fetchdocs(); // console.log("Document successfully unsaved",r.data)
  ...
  const unsave_flashset = (flashset_id) => {
    axiosInstance.put(`${ipaddress2}/Unsave_flashcard_studylist/${user.user_id}/${flashset_id}/`)
      .then((r) => {
        getallflashsetsundergroup();  // console.log("Flashset successfully unsaved",r.data)
      ...
      {loading ? (<Preloader />) : (
        <div className='groupchat'>
          <div className="d-flex">
            <div className='container-fluid' onClick={() => { setcourse_visible(false); setgroup_visible(false); setstudylist_visible(false); }} >
              <Navpath type={"group"} group_id={group_id} navPaths={nav_paths} />
              <div onClick={() => { setnavbar_dropdown_visible(false) }}>
                <div className="w-100 py-3 px-1 px-lg-3 mt-1 d-flex flex-column groupchat-div" onClick={() => { setindex1(-1) }} >
                  <div className='d-flex justify-content-between'>
                    <div>
                      <h5 onClick={() => { setreportdropdownstate(false) }} className='fw-bold mb-3 text-white d-flex align-items-center'><svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="currentColor" className={`bi bi-unlock-fill ${grouptype === "opengroup" ? '' : 'd-none'}`} viewBox="0 0 16 16">
                        ...
                        <span className='ms-2'>{groupdetails.group_name}</span>
                        {(groupdetails.data && groupdetails.data.group_image) &&
                          <img src={groupdetails.data.group_image} className='ms-3 rounded' style={{ width: '80px', height: '50px' }} alt="grp-cht" />
                        }
                      </h5>

                      <div onClick={() => { setreportdropdownstate(false) }} className='d-flex gap-4 text-secondary mb-3'>
                        {groupdetails.data && (
                        ...
                        <span className='text-white'>
                          ...
                          <span className='ms-1 text-white' style={{ fontSize: '14px' }}>{groupdetails?.data?.city || ''}</span>
                        </span>
                        <span className={`text-white ${grouptype === "opengroup" ? 'd-none' : ''}`}><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                      ...
                      <div className='d-flex gap-3' onClick={() => { setreportdropdownstate(false) }}>
                        <Link to={`/groups/${grouptype}`} className='btn border px-3 py-2 fw-medium text-decoration-none d-flex align-items-center' style={{ height: '44px', backgroundColor: '#fff', color: '#5D5FE3', fontSize: '14px' }}>
                          {!groupdetails.group_member_status &&
                          ...
                          <span className='ms-2'>{groupdetails.group_member_status ? 'View Joined Groups' : 'Join Group'}</span>
                        </Link>
                        <button onClick={() => { navigate(`/group_upload_page/${group_id}`) }} className={`btn px-4 py-2 text-decoration-none fw-medium d-flex align-items-center ${grouptype === "opengroup" ? 'd-none' : ''}`} style={{ height: '44px', color: '#fff', fontSize: '14px', border: '1px solid #fff' }}>
                    ...
                    <div className='d-flex align-items-center gap-3 align-self-start'>
                      <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Exit the Group")}>
                      ...
                      <div className="position-relative">
                        <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Report the Group")}>
                          <span className={`text-white ${groupdetails.group_member_status ? '' : 'd-none'}`} style={{ cursor: 'pointer', position: 'relative' }} onClick={() => {
                        ...
                        <ul className={`${reportdropdownstate ? '' : 'd-none'} rounded bg-white border-0 py-2 px-2 ps-3 report-dropdown`} style={{ width: '180px', listStyleType: 'none', position: 'absolute', top: 0, right: '30px' }}>
                ...
                <div className={`m-0 mt-3 mb-5 row justify-content-center`}>
                  <div className="col-lg-9 p-0">
                    <div className='bg-white p-3 rounded shadow-sm border d-md-flex align-items-center' onClick={() => {
                      setindex1(-1)
                      setreportdropdownstate(false)
                    }}>
                      <ul className="nav nav-underline gap-1 w-md-50" id="myTab" role="tablist">
                        <li className="nav-item" role="presentation">
                          <button className="nav-link active bg-white fw-normal p-2" id="home-tab" data-bs-toggle="tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="home-tab-pane" aria-selected="true" style={{ color: tablist === "discussion" ? '#5D5FE3' : '#8E9696', fontSize: '16px', letterSpacing: '0.36px', lineHeight: 'normal' }}
                            onClick={() => { setTablist("discussion") }}>Discussion ({groupdiscussion.length})</button>
                        </li>
                    ...
                    <div className="tab-content" id="myTabContent">
                      ...
                      <div className="tab-pane fade show active bg-white py-1" id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabIndex="0">
                        <div className='w-100 mt-3'>
                          <div onClick={() => {
                            setindex1(-1)
                          }} className='d-md-flex justify-content-between mb-md-3'>
                            <div className='d-flex'>
                            ...
                            <div className='mt-3 mt-md-0 px-md-0 mb-3 mb-lg-0'>
                                  ...
                                  <div className='col-12 bg-white px-3 pt-2 pb-3'>
                                    ...
                                    <form onSubmit={(e) => { postQuestion(e) }}>
                                      <div className='d-flex gap-3 align-items-center'>
                                        <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="grp-cht" />
                                        ...
                                        <div className="input-group bg-light border rounded pe-3">
                                          ...
                                          <div className='d-flex align-items-center bg-light'>
                                            ...
                                            <label data-bs-toggle="tooltip" data-bs-placement="top"
                                              ...
                                              <img src={require('../../img/attachment.png')} width={22} height={22} alt="grp-cht" />
                                            </label>
                        ...
                        {!searchcomment.length && <Group_discussion_pincomments pincomments_status={pincomment_status} setCount={setCount} group_id={group_id} />}
                        {/* -----------------------------------------User comments section layout---------------------------------------------- */}
                        {!searchcomment.length && <Group_discussion_modals usercomments_status={usercomments_status} setCount={setCount} group_id={group_id} />}
                        {/* --------------------------------------------Discussion under particular Group-------------------------------------- */}
                        <div className={`${searchcomment.length > 0 ? 'd-none' : ''}`}>
                          {((groupdiscussion && groupdiscussion.length == 0) && original_status == true) &&
                            <>
                              <img src="https://starlight-b3f27.web.app/assets/images/nodataimg.svg" className='not-found-image' />
                              <h6 className={`text-center py-3 py-3}`} style={{ color: '#5d5fe3', fontSize: '14px' }}>Post your comments...💬</h6>
                            </>
                          }

                          <div className={`${original_status ? '' : 'd-none'}`}>
                            {groupdiscussion.length > 0 &&
                              groupdiscussion.map((x, index) => {
                                return (
                                  <div className='' key={index}>
                                    <div className='col-12 rounded shadow-sm border mb-3 bg-white' key={index}>
                                      <div className="d-flex justify-content-between border-bottom py-2 ps-2 align-items-center mb-0">
                                        <div className='d-flex gap-2 align-items-center'>
                                          <div className="px-1d-flex justify-content-center profile_img" onClick={() => { setindex1(-1); setreportdropdownstate(false) }}>
                                            {x.sender.profile_pic && <img src={x.sender.profile_pic} className={'rounded-circle'} width={40} height={40} alt="grp-cht" />}
                                            {x.sender.nickname &&
                                              (<p className={x.sender.profile_pic == null ? 'd-flex align-items-center justify-content-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{x.sender.nickname.slice(0, 1)}</span><span>{x.sender.nickname.slice(-1)}</span></p>)}
                                          </div>
                                          <div className={`p-0 `} onClick={() => { setindex1(-1); setreportdropdownstate(false) }}>
                                            <Link to={`/profile/${x.sender.user_id}`} className='profile_img my-0 fw-medium text-decoration-none text-dark' style={{ fontSize: '14px' }}>{x.sender.nickname}
                                        ...
                                        <div className={`d-flex align-items-center gap-2 p-0 ${groupdetails.group_member_status ? '' : 'd-none'}`} style={{ position: 'relative' }}>
                                          <p className='my-0 profile_view_more'
                                            onClick={() => { navigate(`/profile/${x.sender.user_id}`) }}>
                                            <span className={`ms-2 edit ${x.edited ? '' : 'd-none'}`}>Edited</span>
                                          </p>
                                          <OverlayTrigger
                                            placement="top"
                                          ...
                                          <div className="btn-group dropstart">
                                            <span className={`border-0 ${groupdetails.group_member_status ? '' : 'd-none'}`} type="button" style={{ cursor: 'pointer' }} onClick={() => {
                                              ...
                                              <img src={require('../../img/more.png')} width={30} height={30} alt="grp-cht" />
                                            </span>
                                            <ul className={`bg-white shadow-sm border rounded mt-0 p-0 px-3 ${index1 == index ? '' : 'd-none'}`} style={{ width: '160px', position: 'absolute', left: '-160px' }}>
                                              <button className={x.sender.user_id != user.user_id || x.created_at.includes("day") || x.created_at.includes("week") || x.created_at.includes("year") ? 'd-none' : 'd-flex align-items-center bg-transparent border-0 my-2'} data-bs-toggle="modal" data-bs-target="#originaleditModal" onClick={() => { editPosts(x.id) }} style={{ height: '20px' }}>
                                                ...
                                                <span className="ms-2">{translate_value.common_words.edit}</span>
                                              </button>
                                              <button className={`bg-transparent border-0 my-2 ${x.pinned_status ? 'd-none' : 'd-flex align-items-center'}`} onClick={() => { pincomment(x.id) }} style={{ height: '20px' }}>
                                                <span className='dropdownmenu'>
                                              ...
                                              {/* ----------------------------------------------------Report button--------------------------------------------------- */}
                                              {user.user_id !== x.sender.user_id &&
                                                <>
                                                  <button className={`bg-transparent border-0 my-2 ${x.report_status ? 'd-none' : 'd-flex align-items-center'}`}
                                                    onClick={() => { setreport_id(x.id); setreport_status(true) }} style={{ height: '20px' }}>
                                                    <span className='dropdownmenu'>
                                                      <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 35 35" fill="none">
                                                  ...
                                                  <button className={`bg-transparent border-0 my-2 ${x.report_status ? 'd-flex align-items-center' : 'd-none'}`} style={{ height: '20px', color: '#FF845D' }}>
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 35 35" fill="none">
                                                      <path d="M17.4997 23.9505C17.754 23.9505 17.9671 23.8645 18.1391 23.6925C18.3111 23.5205 18.3971 23.3073 18.3971 23.0531C18.3971 22.7988 18.3111 22.5856 18.1391 22.4136C17.9671 22.2416 17.754 22.1556 17.4997 22.1556C17.2454 22.1556 17.0322 22.2416 16.8602 22.4136C16.6882 22.5856 16.6023 22.7988 16.6023 23.0531C16.6023 23.3073 16.6882 23.5205 16.8602 23.6925C17.0322 23.8645 17.2454 23.9505 17.4997 23.9505ZM16.7705 19.6316H18.2288V10.7694H16.7705V19.6316ZM12.6479 29.1668L5.83301 22.3651V12.6484L12.6347 5.8335H22.3514L29.1663 12.6352V22.3519L22.3646 29.1668H12.6479ZM13.2705 27.7085H21.7288L27.708 21.7293V13.271L21.7288 7.29183H13.2705L7.29134 13.271V21.7293L13.2705 27.7085Z" fill="#FF845D" />
                                                    </svg>
                                                    <span className="ms-2">Reported</span>
                                                  </button>
                                                </>
                                              }
                                              <button className={`bg-transparent border-0 my-2 ${user.user_id === x.sender.user_id ? ' d-flex align-items-center' : 'd-none'}`} onClick={() => { deletePost(x.id) }} style={{ height: '20px' }}>
                                                <span className='dropdownmenu'>
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 30 30" fill="none">
                                                ...
                                                <span className="ms-2">{translate_value.common_words.delete}</span>
                                              </button>
                                                ...
                                                <div className="modal-body">
                                                  <div className=' bg-white px-3 pt-2 pb-3 rounded'>
                                                    ...
                                                    <div className='d-flex gap-3'>
                                                      <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="grp-cht" />
                                                      {userdetails.nickname != undefined ? (<p className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>) : (<></>)}
                                                      <form className='w-100' onSubmit={(e) => { sendEditedData(e, x.id) }}>
                                                        <div className="input-group bg-light rounded border pe-3">
                                                          <input
                                                            ...
                                                            style={{ position: 'relative' }} />
                                                          <button type='submit' data-bs-dismiss="modal" disabled={editedpost.length > 0 ? false : true} className='bg-transparent border-0'> <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                            <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696" />
                                                          </svg></button>
                                                        </div>
                                                      </form>
                                      ...
                                      <div className='px-2 py-2' onClick={() => {
                                        ...
                                        {x.images_attached && (
                                          x.images_attached.map((z) => { return (<div><img src={z.images} width={200} alt="grp-cht" className='mt-3' /></div>) })
                                        )}
                                      </div>
                                      <div className='d-flex justify-content-between border-bottom px-2 pb-2 mb-0' onClick={() => { setindex1(-1) }}>
                                        <div className="d-flex align-items-center gap-2">
                                          ...
                                          <button className={`bg-transparent border-0 ${groupdetails.group_member_status ? '' : 'd-none'}`} style={{ height: '20px', color: x.dislike_status ? "#ff845d" : "gray" }} onClick={() => {
                                        ...
                                        {x.replies > 0 ?
                                          (!replies_layout) ?
                                            <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_layout(!replies_layout); setSelectedReply(index); getreplies(x.id, index); }}>
                                              {translate_value.dashboard.view_all} {x.replies} {translate_value.dashboard.replies}
                                            </button>
                                            :
                                            (selectedReply == index) ?
                                      ...
                                      {fetchedreplies && fetchedreplies.length > 0 && (
                                        <div className={fetchedreplies[0].group_messages == x.id && replies_layout ? 'd-block' : 'd-none'}>
                                          {fetchedreplies.map((y, index2) => {
                                            return (
                                              <div className='py-2 px-2 bg-white' onClick={() => { setindex1(-1) }}>
                                                <div className="d-flex px-2 justify-content-between w-100 align-items-center">
                                                  <div className='d-flex gap-2'>
                                                    <div className="d-flex justify-content-end">
                                                      <img src={y.user_id.profile_pic} className={y.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="grp-cht" />
                                                ...
                                                <div className='ps-5 mt-2'>
                                                  ...
                                                  {y.images_attached.map((a) => {
                                                    return (<div><img src={a.images} width={300} alt="grp-cht" className='mt-3' /></div>)
                                                  })}
                                                </div>
                                                      ...
                                                      {fetchedreplies_for_reply.map((z) => {
                                                        return (
                                                          <div className='py-2 px-2 bg-white' onClick={() => { setindex1(-1) }} >
                                                            <div className="d-flex justify-content-between w-100 align-items-center">
                                                              <div className='d-flex gap-2'>
                                                                <div className="d-flex justify-content-center">
                                                                  <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="grp-cht" />
                                                            ...
                                                            <div className='ps-5 mt-2'>
                                                              <p className='m-0'>{z.post}</p>
                                                              {z.images_attached.map((b) => {
                                                                return (
                                                                  <div className='d-flex justify-content-center'>
                                                                    <img src={b.Group_Discussion_Replies_Reply_Images} width={260} alt="grp-cht" className='mt-3' />
                                                                  </div>
                                                                )
                                                              })}
                                                            </div>
                                          ...
                                          <div className="input-group border rounded pe-lg-3 bg-light">
                                            ...
                                            <div className='d-flex align-items-center bg-light'>
                                              <input disabled={!groupdetails.group_member_status}
                                                id="file" type='file' name='file' accept="image/*" multiple onChange={handleReplyImage} className="bg-light text-center p-3 btn"
                                              />
                                              <label data-bs-toggle="tooltip" data-bs-placement="top"
                                                data-bs-custom-class="custom-tooltip"
                                                data-bs-title="Attach Image"
                                              ...
                                              >
                                                <img src={require('../../img/attachment.png')} width={22} height={22} alt="grp-cht" /></label>
                                              <button
                                                type='submit'
                              ...
                              <div className="col-12">
                                ...
                                <div id='original'>
                                  {groupdocuments.map((x) => {
                                    return (
                                      <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded border bg-white'>
                                        <div className="row m-0 align-items-center">
                                          ...
                                          <div className="col-10 col-lg-10 ms-md-0 d-flex flex-column justify-content-center ">
                                            <div className="row m-0 border-bottom">
                                              <div className='d-flex justify-content-between'>
                                                <Link to={`/showpdf/${x.document_id}`} className='w-75 text-decoration-none text-primary'>
                                                  <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip(groupdetails?.group_name || 'group name')}>
                                                    <>
                                ...
                                <div id='filterlike' style={{ display: 'none' }}>
                                  {filtereddocuments.map((x) => {
                                    return (
                                      <div className='shadow-sm mt-3 py-2 px-2 px-lg-0 border'>
                                        <div className="row m-0 align-items-center">
                                          <div className="col-2 col-lg-2 d-flex align-items-center justify-content-center" style={{ overflow: 'hidden' }}>
                                            ...
                                            <div className="row m-0 border-bottom">
                                              <div className='d-flex justify-content-between'>
                                                ...
                                                <button onClick={() => { unsave(x.document_id) }} className={`bg-transparent border-0 ${x.study_list_status ? '' : 'd-none'}`} style={{ color: '#8587EA' }}>
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                                    <path d="M0.25 16.3128V1.84967C0.25 1.37025 0.41059 0.969944 0.731771 0.648763C1.05295 0.327582 1.45325 0.166992 1.93268 0.166992H11.0673C11.5467 0.166992 11.947 0.327582 12.2682 0.648763C12.5894 0.969944 12.75 1.37025 12.75 1.84967V16.3128L6.5 13.6285L0.25 16.3128Z" fill="#5D5FE3" />
                                                  </svg> <span className='ms-2 d-none d-md-inline'>Saved</span>
                                                </button>
                            ...
                            <div className="row">
                              <div className="col-12">
                                <div className='d-flex justify-content-between bg-light py-3 px-3 align-items-center'>
                                  ...
                                  <select name="" id="filter" className='border border-secondary py-1 px-2 px-md-3 rounded bg-light' onChange={(e) => { filterflashsets(e.target.value) }}>
                                    <option value="select_filter" className='mt-2'>All</option>
                                    <option value="likes" className=''>Likes</option>
                                    <option value="date" className=''>Date</option>
                                  </select>
                                      ...
                                      <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded bg-white'>
                                        <div className="row m-0 align-items-center ps-0 ps-md-3">
                                          ...
                                          <div className="col-9 ps-0 col-lg-10 ms-md-0 d-flex flex-column justify-content-center ">
                                            <div className="row m-0 border-bottom">
                                              <div className='d-flex justify-content-between'>
                                                <Link to={`/viewflashcard/group/${group_id}/${x.flashset_id}`} className='fw-bold w-75 d-lg-block d-none text-decoration-none text-primary' >{x.name}</Link>
                                                <Link to={`/viewflashcard/group/${group_id}/${x.flashset_id}`} className='fw-bold w-75 d-block d-lg-none text-decoration-none text-primary' >{x.name.slice(0, 16)}</Link>
                                                <button className={`bg-transparent border-0 ${x.studylist_status ? 'd-none' : ' d-flex align-items-center'}`} style={{ color: '#8587EA' }} onClick={() => { setflashsetid(x.flashset_id) }}
                                                ...
                                                <button className={`bg-transparent border-0 ${x.studylist_status ? ' d-flex align-items-center' : 'd-none'}`} style={{ color: '#8587EA' }} onClick={() => { unsave_flashset(x.flashset_id) }}>
                                      ...
                                      <div className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded bg-white'>
                                        <div className="row m-0 align-items-center ps-0 ps-md-3">
                                          ...
                                          <div className="col-9 ps-0 col-lg-10 ms-md-0 d-flex flex-column justify-content-center ">
                                            <div className="row m-0 border-bottom">
                                              <div className='d-flex justify-content-between'>
                                                <Link to={`/viewflashcard/group/${group_id}/${x.flashset_id}`} className='fw-bold d-none d-lg-block w-75 text-decoration-none text-primary'>{x.name}</Link>
                                                <Link to={`/viewflashcard/group/${group_id}/${x.flashset_id}`} className='fw-bold d-block d-lg-none w-75 text-decoration-none text-primary'>{x.name.slice(0, 16)}</Link>
                                                <button className={`bg-transparent border-0 ${x.studylist_status ? 'd-none' : ' d-flex align-items-center'}`} style={{ color: '#8587EA' }} onClick={() => { setflashsetid(x.flashset_id) }} data-bs-toggle="modal" data-bs-target="#flashcard_studylist_modal">
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                                                    <path d="M6.25 20.3123V5.84919C6.25 5.36976 6.41059 4.96946 6.73177 4.64827C7.05295 4.32709 7.45325 4.1665 7.93268 4.1665H17.0673C17.5467 4.1665 17.947 4.32709 18.2682 4.64827C18.5894 4.96946 18.75 5.36976 18.75 5.84919V20.3123L12.5 17.628L6.25 20.3123ZM7.29167 18.6978L12.5 16.4582L17.7083 18.6978V5.84919C17.7083 5.68894 17.6416 5.54204 17.508 5.40848C17.3745 5.27494 17.2276 5.20817 17.0673 5.20817H7.93268C7.77244 5.20817 7.62554 5.27494 7.49198 5.40848C7.35844 5.54204 7.29167 5.68894 7.29167 5.84919V18.6978Z" fill="#5D5FE3" />
                                                  </svg>
                  ...
                  <div className="col-sm-3" onClick={() => { setindex1(-1); setreportdropdownstate(false); }}>
                    <div onClick={() => { setindex1(-1); setreportdropdownstate(false); }}
                      className="ms-0 ms-lg-3 bg-white rounded d-flex flex-column align-items-center mb-3 px-2 border shadow" style={{ height: '400px' }}>
                      <img className={`mt-2 ${tablist === "documents" || tablist === "flashcards" ? "d-block" : "d-none"}`} src={require('../../img/images_icons/send-img2.png')} width={60} alt="grp-cht" />
                      <img className={`mt-2 ${tablist === "discussion" ? "d-block" : "d-none"}`} src={require('../../img/images_icons/send-img1.png')} width={60} alt="grp-cht" />
                      <img src={require('../../img/images_icons/mascot1.png')} style={{ height: '250px', width: '118px' }} alt="grp-cht" />
                ...
                <div>
                  <div className='row m-0 border-bottom pb-4'>
                    <div className="col-sm-6 d-flex align-items-center bg-white shadow-sm justify-content-center p-3">
                      {groupdetails.data.group_image != undefined ? (<img className='rounded' src={groupdetails.data.group_image} style={{ width: '80%' }} alt="grp-cht" />) : (<></>)}
                    </div>
                  ...
                  <div className="mt-4">
                    ...
                    {groupdetails.admin.map((x, i) => {
                      return (
                        <div className='bg-white row rounded shadow-sm align-items-center p-3' key={i}>
                          <div className="col-1 d-flex justify-content-center align-items-center">
                            {x.user_id.profile_pic != undefined && (<img src={x.user_id.profile_pic} style={{ width: '100%' }} className='rounded' alt="grp-cht" />)}
