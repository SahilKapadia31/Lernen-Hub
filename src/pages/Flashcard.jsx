...
const Flashcard = () => {
  ...
  const courseData = (e) => {
    let selectedCourse = joinedCourses.find(course => course.course_id == e.target.value);
  ...
  const [gender_voicetype, setGender_voicetype] = useState("")
  const Gender_voicetypeData = (e) => {
    setGender_voicetype(e.target.value)
  }
  ...
  const sendflashcardset = () => {
    ...
    formdata.append('gender_voicetype', gender_voicetype);
    axiosInstance.post(`${ipaddress2}/flashcardsets/create/${user.user_id}/${courseid}/${semesterid}/`, formdata)
      .then((r) => {
        ...
        toast.success('Flashset created successfully', { autoClose: 1000 });
    ...
    try {
      ...
      setTimeout(() => {
        if (courseValue) {
          ...
          navigate(`/subjects/${courseValue?.course_id}/${courseValue?.course_name}`);
        }
      }, 2000)
      setFlashcards([
        { term: '', definition: '', t_image: null, d_image: null }
  ...
  const addFlashcard = () => {
    // Check if any existing flashcard has an empty field
    const hasEmptyFields = flashcards.some(
      (card) => !card.term || !card.definition || card.t_image === null || card.d_image === null
    );

    if (hasEmptyFields) {
      toast.error("Please fill all fields before adding a new flashcard!", { autoClose: 1000 });
      return;
    }

    // Add new flashcard if validation passes
          ...
          <div className="p-4">
            <div className='row bg-white justify-content-center'>
              <div className="col-md-9 bg-white">
                <div className='px-2 p-3 rounded row '>
                  ...
                  <div className="col-md-6 mb-3">
                    <input type="text" name='name' className='form-control ps-4 bg-light' style={{ height: '50px' }} placeholder='Name of the Flashcard' onChange={nameData} value={name} disabled={isFieldDisable} />
                  </div>
                  ...
                  <div className="col-md-6 mb-3">
                    <input type="text" name='professor_name' className='form-control ps-4 bg-light' style={{ height: '50px' }} placeholder='Professor Name' onChange={professorData} value={professor} disabled={isFieldDisable} />
                  </div>
                  <div className="col-md-6 mb-3">
                    <input type="text" name='tags' className='form-control ps-4 bg-light' style={{ height: '50px' }} placeholder='Tag Name' onChange={tagnameData} value={tagname} disabled={isFieldDisable} />
                  </div>
                  <div className="col-md-6 mb-3">
                    <input type="text" name='description' className='form-control ps-4 bg-light' style={{ height: '50px' }} placeholder='Description' onChange={descriptionData} value={description} disabled={isFieldDisable} />
                  </div>
                  <div className="col-md-6 mb-3">
                    <select
                      name="language"
                  ...
                  <div className="col-md-6 mb-2">
                    <select
                      name="gender_voicetype"
                      className="form-select form-select-md px-5 ps-4 shadow-none bg-light" style={{ height: '50px' }}
                      aria-label="Large select example"
                      onChange={Gender_voicetypeData}
                      value={gender_voicetype}
                      disabled={isFieldDisable}
                    >
                      <option value="" selected>Select Voice Type</option>
                      <option value="Male">Male</option>
                      <option value="Female">Female</option>
                    </select>
                  </div>
                  <div className='text-end mt-3'>
                    <button className='btn fw-medium text-white' style={{ backgroundColor: '#5d5fe3' }} disabled={isFieldDisable} onClick={sendflashcardset}>Create Flashset</button>
                  </div>
                </div>
              </div>
              ...
              <div className='col-md-2 d-flex justify-content-center align-items-center'>
                <img src={require('../img/images_icons/mascot1.png')} style={{ height: '300px' }} alt="flash" />
              </div>
            </div>
