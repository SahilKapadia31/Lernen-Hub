...
const Organizationsearch_comments = ({ count, searchvalue, setCount }) => {
  ...
  const searchcomments = () => {
    if (searchvalue.length > 0) {
      axiosInstance.get(`${ipaddress}/SearchCommentsUnderUniversity/${user.user_id}/${user.organization_id}/${searchvalue}/`)
        .then((response) => { setSearchedComments(response.data) })
  ...
  const getreplies = (discussion_id, index) => {
    axiosInstance.get(`${ipaddress}/UniversityDiscussionReply/${user.user_id}/${user.organization_id}/${discussion_id}/`)
      .then((r) => { setFetchedreplies(r.data.reverse()) })
  ...
  const getreplies_for_reply = (particular_reply_id) => {
    axiosInstance.get(`${ipaddress}/UniversityDiscussionRepliesRepliesView/${user.user_id}/${user.organization_id}/${particular_reply_id}/`)
      .then((r) => { setFetchedreplies_for_reply(r.data.reverse()) })
      .catch(() => { console.log("UniversityDiscussionReply Error") })
  }
  ...
  const deleteReply = (disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/UniversityDiscussionReply/${user.user_id}/${discid}/${disc_reply_id}/`)
      .then((r) => {
        searchcomments();
        setCount((prev) => prev + 1);
        toast.success('Reply Deleted', { autoClose: 2000 });
        getreplies(discid);
      })
  ...
  const pincomment = (discId) => {
    axiosInstance.post(`${ipaddress}/UniversityPinnedCommentsView/${user.user_id}/${discId}/${user.organization_id}/`)
      .then((r) => {
        if (r.data.message === 'Message already followed') {
          ...
          searchcomments();
          setCount((prev) => prev + 1);
          toast.success('Followed successfully', { autoClose: 2000 });
          setindex1(-1);
        }
      })
  }
  ...
  const unpin = (discid) => {
    axiosInstance.delete(`${ipaddress}/UniversityPinnedCommentsView/${user.user_id}/${discid}/${user.organization_id}/`)
      .then((r) => {
        toast.success('Post Unfollowed', { autoClose: 2000 });
        searchcomments();
        setCount((prev) => prev + 1);
      })
          ...
          return (
            <div key={index} style={{ zIndex: 4 }}>
              <div className='col-12 rounded shadow-sm border mb-3 bg-white'>
                <div className="d-flex justify-content-between border-bottom py-2 ps-2 align-items-center mb-0">
                  <div className='d-flex gap-2 align-items-center'>
                    <div className="d-flex justify-content-center">
                      <img src={x.user_id.profile_pic} className={x.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="uni-ser" />
                    ...
                    <div className={``} onClick={() => {
                      ...
                      <p className='ms-sm-0 my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA' }}>{user.organization_name} </p>
                    </div>
                  </div>
                          ...
                          <div className="modal-body">
                            <div className=' bg-white px-3 pt-2 pb-3 rounded'>
                              ...
                              <div className='d-flex gap-3'>
                                <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="uni-ser" />
                                ...
                                <div className="input-group bg-light rounded pe-3 border">
                                  ...
                                  <button data-bs-dismiss="modal" disabled={editedpost.length > 0 ? false : true} onClick={() => {
                                    sendEditedData(x.id)
                                  }} className='text-secondary bg-transparent border-0'><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                ...
                <div onClick={() => {
                  ...
                  {x.images_attached && (
                    x.images_attached.map((z) => {
                      return (
                        <div className='d-flex justify-content-center'>
                          <img src={z.image} width={300} alt="uni-ser" className='mt-3' />
                        </div>
                      )
                    })
                  )}
                ...
                <div onClick={() => {
                  setindex1(-1)
                }} className='d-flex justify-content-between border-bottom px-2 pb-2 mb-0'>
                    ...
                    {fetchedreplies.map((y, index2) => {
                      ...
                      return (
                        <div key={index2} className='px-2 py-2 bg-white' onClick={() => {
                          ...
                          <div className="d-flex px-2 w-100 align-items-center justify-content-between">
                            <div className='d-flex gap-2'>
                              <div className="d-flex justify-content-center">
                                <img src={y.user_id.profile_pic} className={y.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="uni-ser" />
                            ...
                            <div className="p-0 d-flex gap-3 align-items-center">
                          ...
                          <div className='ps-5 mt-2 m-0'>
                            ...
                            {y.images_attached.map((a) => {
                              return (
                                <div className='d-flex justify-content-center'>
                                  <img src={a.organization_discussion_reply_images} width={300} alt="uni-ser" className='mt-3' />
                                </div>
                              )
                            })}
                              ...
                              {fetchedreplies_for_reply && fetchedreplies_for_reply.length > 0 && (
                                <div className={fetchedreplies_for_reply[0].reply == y.id && replies_for_reply_status ? 'd-block' : 'd-none'}>
                                  {fetchedreplies_for_reply.map((z) => {
                                    ...
                                    return (
                                      <div className='ps-0 ps-md-3 py-2 bg-white' onClick={() => {
                                        setindex1(-1)
                                      }} >
                                        <div className="w-100 d-flex px-2 align-items-center justify-content-between">
                                          <div className='d-flex gap-2'>
                                            <div className="d-flex justify-content-center">
                                              <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="uni-ser" />
                                          ...
                                          <div className="d-flex gap-3 gap-3 align-items-center">
                                        ...
                                        <div className='ps-5 mt-2 m-0'>
                                          ...
                                          {z.images_attached.map((b) => {
                                            return (
                                              <div className='d-flex justify-content-center'>
                                                <img src={b.organization_discussion_reply_reply_images} width={300} alt="uni-ser" className='mt-3' />
                                              </div>
                                            )
                                          })}
                                        </div>
                  ...
                  <div onClick={() => {
                    ...
                    <div className="input-group border rounded pe-lg-3 bg-light">
                      <input key={index}
                        ...
                        className="form-control py-2 ps-3 shadow-none border-0 bg-light reply-input4" placeholder={translate_value.dashboard.reply_here}
                        style={{ position: 'relative' }} />
                ...
                {(repliesImage4 && repliesImage4.length > 0) &&
                  <div className='d-flex gap-3'>
                    {
                    ...
                    <div className="input-group bg-light border rounded pe-lg-3">
                      ...
                      <div className='d-flex align-items-center bg-light'>
                        ...
                        <label data-bs-toggle="tooltip" data-bs-placement="top"
                          ...
                          htmlFor="fileInput4"
                          className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                          <img src={require('../img/attachment.png')} width={22} height={22} alt="uni-ser" />
                        </label>
...
export default Organizationsearch_comments;