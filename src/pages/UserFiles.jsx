...
const UserFiles = () => {
  ...
  let { translate_value, setgroup_visible, setstudylist_visible, setcourse_visible, setnavbar_dropdown_visible } = useContext(Context);
  //addsubjects_layout,navbar_dropdown_visible
  ...
  const fetchfollowed_docs = () => {
    axiosInstance.get(`${ipaddress}/FavouriteDocuments/${user_id}`)
      ...
      }).catch((err) => { console.log("FavouriteDocuments Error", err) });
  }
  ...
  const fetchcreated_flashsets = () => {
    axiosInstance.get(`${ipaddress2}/UserFlashcardsetsRetrive/${user_id}/`)
      ...
      }).catch((err) => { console.log("created flashsets fetching error", err) });
  }
  // To get saved flashsets under particular user
  const fetchsaved_flashsets = () => {
    axiosInstance.post(`${ipaddress2}/SavedStudylistFlashsets/`, { 'user_id': user_id })
      .then((p) => {
        const docs = p.data.reverse();
        setsavedFlashsets(docs);
      }).catch(() => { console.log("Saved flashcards fetching error") })
  }
  ...
  const filterdocs = (e) => {
    ...
    const endpoints = {
      likes: `${ipaddress}/userfilteredocuments/${user_id}/`,
      rating: `${ipaddress}/FilterDocumentsByRating/${user_id}/`,
      date: `${ipaddress}/FilterUserDocumentsByDate/${user_id}/`,
    };
    if (value === "select_filter") {
      setFiltereddocuments([]);
    } else if (endpoints[value]) {
      filterfunction(endpoints[value]);
    } else {
      console.log(`Unhandled filter type: ${value}`);
    }
  };

  const filterfunction = (value) => {
    axiosInstance.get(value).then((r) => { setFiltereddocuments(r.data) }).catch(() => { console.log("filterfunction error") })
  }
  ...
  const filterfolloweddocs = (value) => {
    if (value === "followeddocs_original") {
      ...
      axiosInstance.get(`${ipaddress}/FavouriteDocumentsFilter/${user_id}/${value}/`)
        ...
        }).catch(() => { console.log("FavouriteDocumentsFilter error") })
    }
  }
  ...
  const searchDocument = (document_name) => {
    if (document_name.length > 0) {
      axiosInstance.get(`${ipaddress}/UserDocumentsSearch/${user.user_id}/${document_name}/`)
        ...
        }).catch((err) => { console.log("Search Data Sent Failed", err) })
    } else { setFiltereddocuments([]) }
  }
  ...
  const search_followeddocs = (value) => {
    if (value != null && value.length > 0) {
      axiosInstance.get(`${ipaddress}/FavouriteDocumentsSearchFilter/${user.user_id}/${value}/`)
        .then((r) => {
          setfilteredfollowed_docs(r.data)
        }).catch((err) => { console.log("FavouriteDocumentsSearchFilter error", err) })
    } else { setfilteredfollowed_docs([]) }
  }
  ...
  const searchSavedflashsets = (value) => {
    ...
    } else { fetchsaved_flashsets() }
  }
  // ----------------------------------------Filter created flashsets----------------------------------------------------
  const filtercreatedflashsets = (value) => {
    if (value === "likes") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersCreatedFlashsets/${user.user_id}/`, { 'like': true, 'datetime': false })
        .then((r) => {
          setcreatedFlashsets(r.data);
    ...
    if (value === "date") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersCreatedFlashsets/${user.user_id}/`, { 'like': false, 'datetime': true })
        .then((r) => {
          setcreatedFlashsets(r.data);
          // document.getElementById('flashsetoriginal').style.display='none'
        }).catch((err) => { console.log("error", err) })
    }
    if (value === "all") { fetchcreated_flashsets() }
  }
  // ----------------------------------------Filter saved flashsets----------------------------------------------------
  const [searchedFlashsets, setSearchedFlashsets] = useState([])
  const filtersavedflashsets = (value) => {
    if (value === "likes") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersSavedFlashsets/${user.user_id}/`, { 'like': true, 'datetime': false })
        .then((r) => {
          setSearchedFlashsets(r.data);
        }).catch((err) => { console.log("error", err) })
    }
    if (value === "date") {
      axiosInstance.post(`${ipaddress2}/FlashsetsFiltersSavedFlashsets/${user.user_id}/`, { 'like': false, 'datetime': true })
        .then((r) => {
          setSearchedFlashsets(r.data);
        }).catch((err) => { console.log("error", err) })
    }
  ...
  const handleTabClick = (tabName, fetchFunction) => {
    setActiveTab(tabName); // Update the active tab
    fetchFunction(); // Call the appropriate fetch function
  };
      ...
      {pageloading ? (<Preloader />) : (
        <div>
          <div className="d-flex">
            <div onClick={() => { setcourse_visible(false); setgroup_visible(false); setstudylist_visible(false); }} className="w-100 bg-light main-division d-flex flex-column align-items-center px-3 px-lg-0">
              <div onClick={() => { setnavbar_dropdown_visible(false) }} className="container">
                <div className='bg-white px-3 py-2 pt-2 shadow-sm'>
                  {/* <Link to={`/profile/${user_id}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" fill="black" className="bi bi-arrow-left" viewBox="0 0 16 16">
                      <path fillRule="evenodd" d="M15 8a.5.5 0 0 0-.5-.5H2.707l3.147-3.146a.5.5 0 1 0-.708-.708l-4 4a.5.5 0 0 0 0 .708l4 4a.5.5 0 0 0 .708-.708L2.707 8.5H14.5A.5.5 0 0 0 15 8" />
                    </svg>
                  </Link> */}
                  <ul className="nav nav-underline d-flex justify-content-start flex-lg-row flex-column" id="myTab" role="tablist">
                    <li className="nav-item" role="presentation">
                      <button className={`nav-link bg-white me-lg-4 ${activeTab === "uploaded_documents" ? "text-primary active" : "text-secondary"}`}
                        id="home-tab" aria-selected={activeTab === "uploaded_documents"} data-bs-toggle="tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="home-tab-pane"
                        onClick={() => handleTabClick("uploaded_documents", fetchuploaded_docs)}
                      >
                        Uploaded Documents ({uploadedDocuments.length})
                      </button>
                    </li>
                    <li className="nav-item" role="presentation">
                      <button className={`nav-link bg-white me-lg-4 ${activeTab === "followed_documents" ? "text-primary active" : "text-secondary"}`}
                        id="profile-tab" aria-selected={activeTab === "followed_documents"} data-bs-toggle="tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="profile-tab-pane"
                        onClick={() => handleTabClick("followed_documents", fetchfollowed_docs)}
                      >Liked Documents ({favouritedocs.length})</button>
                    </li>
                    {user_id === user.user_id && (
                      <>
                        <li className="nav-item" role="presentation">
                          <button onClick={() => handleTabClick("uploaded_flashcards", fetchcreated_flashsets)} aria-selected={activeTab === "uploaded_flashcards"}
                            className={`nav-link bg-white me-lg-4 ${activeTab === "uploaded_flashcards" ? "text-primary active" : "text-secondary"}`}
                            id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane"
                          >
                            Created Flashsets ({createdFlashsets.length})
                          </button>
                        </li>
                        <li className="nav-item" role="presentation">
                          <button onClick={() => handleTabClick("followed_flashcards", fetchsaved_flashsets)} aria-selected={activeTab === "followed_flashcards"}
                            className={`nav-link bg-white ${activeTab === "followed_flashcards" ? "text-primary active" : "text-secondary"}`}
                            id="disabled-tab" data-bs-toggle="tab" data-bs-target="#disabled-tab-pane" type="button" role="tab" aria-controls="disabled-tab-pane"
                          >
                            Saved Flashsets ({savedFlashsets.length})
                          </button>
                        </li>
                      </>
                    )}
                ...
                <div className="tab-content" id="myTabContent">
                  ...
                  {uploadedDocuments && (
                    <div className={`tab-pane fade bg-light mt-3 ${value === "uploaded_documents" ? 'show active' : ''}`} id="home-tab-pane" role="tabpanel" aria-labelledby="home-tab" tabIndex="0">
                      <div className='d-flex flex-column align-items-lg-center flex-lg-row bg-white gap-lg-2 justify-content-between shadow-sm py-3 px-3 mb-3'>
                        <p className='mb-0'>{uploadedDocuments.length} Documents</p>
                        <div className='d-flex'>
                          <div>
                          ...
                          <select name="" id="filter" className='border rounded ms-2' style={{ height: '35px' }} onChange={filterdocs}>
                      ...
                      <div className={`${filtereddocuments.length > 0 ? 'd-none' : ''}`}>
                        {uploadedDocuments.map((x, i) => {
                          return (
                            <div key={i} className='shadow-sm mb-3 py-3 px-2 px-lg-0 rounded bg-white'>
                              <div className="row m-0 align-items-center">
                                <div className="col-2 col-lg-2 d-flex align-items-center justify-content-center">
                                  <Document file={x.doc_url} >
                                ...
                                <div className="col-10 col-lg-10 ms-md-0 d-flex flex-column justify-content-center ">
                                  <div className="row m-0 border-bottom">
                                    <div className='d-flex justify-content-between'>
                                      <Link className='fw-bold d-none d-md-inline' style={{ color: '#2A3941', fontSize: '16px' }} to={`/showpdf/${x.document_id}`}>
                                        {x.title.length > 200 ? `${x.title.slice(0, 200)}...` : x.title}
                                      </Link>
                                      ...
                                      <div>
                                        {x.is_delete && <span style={{ fontSize: '14px', color: '#AAB0B0' }} className='me-2 bg-transparent border-0'><img className='text-start' src={deleteDoc_icon} alt="Delete Icon" width="20" height="20" /></span>}
                                        ...
                                        <button data-bs-toggle="modal" data-bs-target="#studylist_modal" className={`bg-transparent border-0 ${x.study_list_status ? 'd-none' : ''}`} style={{ color: '#8587EA' }} onClick={() => { setdocument_id(x.document_id) }}>
                                          ...
                                          <span className='ms-1 d-none d-md-inline'>Save</span>
                                        </button>
                                      </div>
                                    </div>
                                    <p className='mt-1 d-flex align-items-center' style={{ fontSize: '14px', color: '#5D5FE3' }}>
                                      ...
                                      {x.user_data.nickname}<span style={{ color: '#8E9696' }} className='ms-1'>{x.created_on}</span>
                                    </p>
                                  </div>
                                  <div className='d-lg-block d-none'>
                                    <div className="m-0 d-flex align-items-center mt-2">
                                      <span style={{ fontSize: '14px', color: '#AAB0B0' }} className='ms-3 d-flex align-items-center'>
                                        ...
                                        <span className='ms-1'>{x.download_count} <span className='d-lg-inline d-none'>Downloads</span></span>
                                      </span>
                      ...
                      <div className={`${filtereddocuments.length > 0 ? '' : 'd-none'}`}>
                        {filtereddocuments.map((x) => {
                          return (
                            <div className='shadow-sm mt-3 py-3 px-2 bg-white'>
                                ...
                                <div className="d-flex flex-column justify-content-center w-100">
                                  <div className="row m-0 border-bottom">
                                    <div className='d-flex justify-content-between'>
                                      ...
                                      <Link className='fw-bold d-inline d-md-none' style={{ color: '#2A3941', fontSize: '16px' }} to={`/showpdf/${x.document_id}`}>{x.title.slice(0, 16)}</Link>
                                      <button onClick={() => { unsave(x.document_id) }} className={`bg-transparent border-0 ${x.study_list_status ? '' : 'd-none'}`} style={{ color: '#8587EA' }}><svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                        <path d="M0.25 16.3128V1.84967C0.25 1.37025 0.41059 0.969944 0.731771 0.648763C1.05295 0.327582 1.45325 0.166992 1.93268 0.166992H11.0673C11.5467 0.166992 11.947 0.327582 12.2682 0.648763C12.5894 0.969944 12.75 1.37025 12.75 1.84967V16.3128L6.5 13.6285L0.25 16.3128Z" fill="#5D5FE3" />
                                  ...
                                  <div className='d-sm-block d-md-block d-lg-none'>
                                    <div className="m-0 d-flex align-items-center mt-2">
                                      <span data-bs-toggle="tooltip" data-bs-placement="bottom" className='ms-3 d-flex align-items-center' data-bs-custom-class="custom-tooltip"
                                        data-bs-title="Rating" style={{ fontSize: '14px', color: '#AAB0B0' }}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="19" viewBox="0 0 20 19" fill="none">
                                          <path d="M15.1891 18.8335H4.73237V7.37514L11.2628 0.924805L11.7236 1.38556C11.8184 1.48037 11.8982 1.6019 11.9629 1.75014C12.0277 1.89837 12.0601 2.03459 12.0601 2.15879V2.32306L10.9944 7.37514H18.234C18.6693 7.37514 19.0586 7.54675 19.4018 7.88996C19.7451 8.23317 19.9167 8.62246 19.9167 9.05783V10.3399C19.9167 10.4347 19.906 10.5382 19.8846 10.6504C19.8633 10.7626 19.8352 10.8661 19.8005 10.9609L16.9559 17.7037C16.813 18.0242 16.5727 18.2926 16.2348 18.509C15.8969 18.7253 15.5483 18.8335 15.1891 18.8335ZM5.77404 17.7918H15.1891C15.336 17.7918 15.4862 17.7518 15.6398 17.6716C15.7934 17.5915 15.9103 17.4579 15.9904 17.271L18.875 10.5001V9.05783C18.875 8.87086 18.8149 8.71729 18.6947 8.5971C18.5745 8.4769 18.4209 8.41681 18.234 8.41681H9.70031L10.9062 2.7277L5.77404 7.81986V17.7918ZM4.73237 7.37514V8.41681H1.16667V17.7918H4.73237V18.8335H0.125V7.37514H4.73237Z" fill="#8E9696" />
                  ...
                  <div className={`tab-pane fade bg-light mt-3 ${value === "followed_documents" ? 'show active' : ''}`} id="profile-tab-pane" role="tabpanel" aria-labelledby="profile-tab" tabIndex="0">
                    ...
                    <div>
                      <div className='d-flex flex-column flex-lg-row  justify-content-lg-between bg-white shadow-sm py-3 px-3 align-items-lg-center mb-3 gap-2 gap-lg-0'>
                        ...
                        <div className='d-flex gap-2'>
                          ...
                          <select name="" id="filter" className='border border py-1 px-2 px-md-3 rounded bg-light ms-2'
                            onChange={(e) => { setFiltertype(e.target.value); filterfolloweddocs(e.target.value) }}>
                                ...
                                <div className="d-flex flex-column justify-content-center w-100">
                                  <div className="row m-0 border-bottom">
                                    <div className='d-flex justify-content-between'>
                                      <Link className='fw-bold d-none d-md-inline' style={{ color: '#2A3941', fontSize: '16px' }} to={`/showpdf/${x.document_id}`}>{x.title}</Link>
                                      <Link className='fw-bold d-inline d-md-none' style={{ color: '#2A3941', fontSize: '16px' }} to={`/showpdf/${x.document_id}`}>{x.title.slice(0, 16)}</Link>
                                      <button onClick={() => { unsave1(x.document_id) }} className={`bg-transparent border-0 ${x.study_list_status ? '' : 'd-none'}`} style={{ color: '#8587EA' }}><svg xmlns="http://www.w3.org/2000/svg" width="13" height="17" viewBox="0 0 13 17" fill="none">
                                        <path d="M0.25 16.3128V1.84967C0.25 1.37025 0.41059 0.969944 0.731771 0.648763C1.05295 0.327582 1.45325 0.166992 1.93268 0.166992H11.0673C11.5467 0.166992 11.947 0.327582 12.2682 0.648763C12.5894 0.969944 12.75 1.37025 12.75 1.84967V16.3128L6.5 13.6285L0.25 16.3128Z" fill="#5D5FE3" />
                                      </svg> <span className='ms-2 d-none d-md-inline'>Saved</span></button>
                                      <button data-bs-toggle="modal" data-bs-target="#studylist_modal" className={`bg-transparent border-0 ${x.study_list_status ? 'd-none' : ''}`} style={{ color: '#8587EA' }} onClick={() => {
                                        setdocument_id(x.document_id)
                    ...
                    <div>
                      <div className='d-flex flex-column flex-lg-row  justify-content-lg-between bg-white shadow-sm py-3 px-3 align-items-lg-center mb-3 gap-2 gap-lg-0'>
                        ...
                        <div className='d-flex gap-2'>
                          ...
                          <select name="" id="filter" className='border border py-1 px-2 px-md-3 rounded bg-light' onChange={(e) => { filtercreatedflashsets(e.target.value) }}>
                      ...
                      <div className={`${createdFlashsets.length > 0 ? '' : 'd-none'}`}>
                        {createdFlashsets.map((x) => {
                          return (
                            <div className='shadow-sm mb-3 py-3 px-2 rounded bg-white'>
                              <div className="align-items-center d-flex gap-2">
                                <div className="d-flex align-items-center justify-content-center rounded" style={{ overflow: 'hidden', backgroundColor: '#CFF4D2', height: '100px', width: '100px', border: '0.5px solid #21B3A9' }}>
                                ...
                                <div className="d-flex flex-column justify-content-center w-100">
                                  <div className="row m-0 border-bottom">
                                    <div className='d-flex justify-content-between'>
                                      <Link to={`/viewflashcard/course/""/${x.flashset_id}`} onClick={()=>sessionStorage.setItem('selectedSubTab','created_flashcard')} className='fw-bold' style={{ color: '#2A3941' }}>{x.name}</Link>
                    ...
                    <div>
                      <div className='d-flex flex-column flex-lg-row  justify-content-lg-between bg-white shadow-sm py-3 px-3 align-items-lg-center mb-3 gap-2 gap-lg-0'>
                        ...
                        <div className='d-flex gap-2'>
                          ...
                          <select name="" id="filter" className='border border py-1 px-2 px-md-3 rounded bg-light ms-2' onChange={(e) => { filtersavedflashsets(e.target.value) }}>
                            <option value="all" className='mt-2'>All</option>
                            <option value="likes" className=''>Likes</option>
                            <option value="date" className=''>Date</option>
                          </select>
                            ...
                            <div className='shadow-sm mb-3 py-3 px-2 rounded bg-white'>
                              <div className="d-flex align-items-center gap-2">
                                ...
                                <div className="d-flex flex-column justify-content-center w-100">
                                  <div className="row m-0 border-bottom">
                                    <div className='d-flex justify-content-between'>
                                      {x.flashset_details && x.flashset_details.flashset_id != undefined ? (<Link to={`/viewflashcard/group/""/${x.flashset_details.flashset_id}`} onClick={()=>sessionStorage.setItem('selectedSubTab','saved_flashcard')} className='fw-bold' style={{ color: '#2A3941' }}>{x.flashset_details.name}</Link>) : (<></>)}
