...
const Profile = () => {
  ...
  const fetchAllUniversities = () => {
    axiosInstance.get(`${ipaddress}/universities/`)
      .then((r) => {
        ...
        // console.log("Organization data : ",r.data)
      })
  ...
  const handleSubmit = (event) => {
    ...
    axiosInstance.post(`${ipaddress}/UserStudyInfo/${user_id}/`, formData)
      .then((r) => {
        ...
        setCount(count + 1)
        resetForm();
        // setEncryptedData("additionaldetails",JSON.stringify(r.data))
      })
      .catch(() => {
  ...
  const addorganizationname = (organizationName, event, organizationId) => {
    event.preventDefault()
    // console.log("organization selected nameeeeeeeee",organizationName,organizationId)
                  ...
                  <div className="profile1 row m-0 px-lg-5 pb-4 align-items-center rounded" style={{ backgroundColor: '#F3F0FF', height: '200px' }}>
                    ...
                    {userdetails && (
                      <div className="col-lg-3 d-flex align-items-center" style={{ height: '100px' }}>
                        <img src={userdetails.profile_pic} width={100} height={100} className={`rounded ${userdetails.profile_pic != null ? '' : 'd-none'}`} alt="profile" />
                        ...
                        <div className="ms-2" style={{ height: '100%' }}>
                          ...
                          {joinedUniversity.map((x) => {
                            return (
                              <div>
                                <span style={{ fontSize: '13px' }}>{x.organization_id.organization_name}</span>
                              </div>
                            )
                          })}
                              ...
                              return (
                                <div className="row">
                                  <div className="col-12">
                                    <span className=''><i className="fa-solid fa-building-columns  me-2"></i>{x.organization_id.organization_name}</span>
                                  </div>
