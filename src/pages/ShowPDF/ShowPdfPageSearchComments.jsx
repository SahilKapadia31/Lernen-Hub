...
const Showpdfpage_search_comments = ({ pinnedcomments_status, id, count, setCount, discussionpagelayout, editPosts }) => {
  ...
  function handledislike(discussion_id) {
    axiosInstance.post(`${ipaddress}/doc_comments_dis_likes_view/${user.user_id}/${discussion_id}/`)
      .then((r) => { getpinnedcomments() })
      .catch(() => { console.log("User dislike error") })
  }
  // ----------------------------------------------------Reply Likes------------------------------------------------------
  ...
  function handlereplydislike(reply_id, discussion_id, index) {
    axiosInstance.post(`${ipaddress}/DocumentReplyDisLikeView/${user.user_id}/${reply_id}/`)
      ...
      .catch(() => { console.log("User reply dislike error") })
  }
  ...
  const deleteReply = (disc_reply_id, discid, index) => {
    axiosInstance.delete(`${ipaddress}/documentcommentreplydelete/${disc_reply_id}/`)
      .then((r) => {
        ...
        toastBootstrap.show();
        getreplies(discid, index)
      }).catch(() => { console.log("Document Reply Delete Error") })
  }
              ...
              <div className='col-12 bg-white px-3 pt-2 pb-3 rounded border'>
                <div className="row align-items-center">
                  <div className="col-1" onClick={() => { setindex1(-1) }}>
                    <img src={x.user_id.profile_pic} className={x.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="shw-pdf-ser" />
                  ...
                  <div className={`${discussionpagelayout ? 'col-9' : 'col-8 ms-lg-3'}`} onClick={() => { setindex1(-1) }}>
                    ...
                    <p className='ms-3 ms-sm-0 my-0' style={{ fontSize: '12px' }}>{x.document_post_id.created_on} <span className={`ms-2 edit ${x.edited ? '' : 'd-none'}`}>Edited</span></p>
                  </div>
                    ...
                    <div className="btn-group dropstart">
                      ...
                      <ul className={`bg-white border shadow-sm mt-0 p-0 ps-3 rounded ${index1 === index && dropdownstate ? '' : 'd-none'}`} style={{ width: '150px', position: 'absolute', left: '-160px' }}>
                        ...
                        <button className={`bg-transparent border-0 my-2 ${user.user_id === x.user_id.user_id ? 'd-flex align-items-center' : 'd-none'}`}
                          onClick={() => { deletePost(x.document_post_id.ddpid) }} style={{ height: '20px' }}>
                ...
                <div className='py-2' onClick={() => { setindex1(-1) }}>
                  ...
                  {x.document_post_id.images_attached.map((z) => {
                    return (<div><img src={z.image} width={300} className="mt-3 img-fluid" alt="shw-pdf-ser" /></div>)
                  })}
                </div>
                          ...
                          <div className="d-flex">
                            ...
                            {/* <div  className="ms-2 comment-section-img" style={{position:'relative'}}>
                              {x.liked_users.slice(0,5).map((a,index)=>{
                                return(
                                  <>
                                    <img src="http://localhost:3000/static/media/man.ba3e9327f074cac4e49e.png" width={24} style={{left:`${index*20}px`,position:"absolute",top:'-2px'}} alt="shw-pdf-ser" />
                                  </>
                                )
                              })}
                              </div> */}
                        ...
                        <div className='py-2 mt-3' onClick={() => { setindex1(-1) }}>
                          <div className="row m-0 w-100 align-items-center">
                            <div className="col-1 p-0 d-flex justify-content-end">
                              <img src={y.user_details.profile_pic} className={y.user_details.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="shw-pdf-ser" />
                            ...
                            <div className={`col-3 ${discussionpagelayout ? 'col-lg-2' : 'col-lg-3'} p-0 d-flex justify-content-between align-items-center`}>
                              ...
                              <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: y.disliked_status ? "#ff845d" : "gray" }}
                                onClick={() => { handlereplydislike(y.dprid, x.document_post_id.ddpid, index) }}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 30 30" fill="none">
                                  <path d="M8.17306 4.99983H20.7212V18.7498L12.8846 26.4902L12.3317 25.9373C12.2179 25.8236 12.1222 25.6777 12.0445 25.4998C11.9667 25.322 11.9279 25.1585 11.9279 25.0095V24.8123L13.2067 18.7498H4.51922C3.99678 18.7498 3.52963 18.5439 3.11778 18.132C2.70593 17.7202 2.5 17.253 2.5 16.7306V15.1921C2.5 15.0783 2.51282 14.9541 2.53847 14.8195C2.56409 14.6849 2.59774 14.5607 2.63941 14.447L6.05288 6.35558C6.22435 5.97097 6.51281 5.64886 6.91825 5.38923C7.32371 5.12963 7.74198 4.99983 8.17306 4.99983ZM19.4712 6.24983H8.17306C7.99679 6.24983 7.81651 6.2979 7.63222 6.39404C7.44793 6.49021 7.3077 6.65047 7.21153 6.87483L3.75 14.9998V16.7306C3.75 16.955 3.82211 17.1393 3.96634 17.2835C4.11057 17.4277 4.29486 17.4998 4.51922 17.4998H14.7596L13.3125 24.3268L19.4712 18.2162V6.24983ZM20.7212 18.7498V17.4998H25V6.24983H20.7212V4.99983H26.25V18.7498H20.7212Z" fill="currentColor" />
                                </svg>
                              </button>
                              <button className={user.first_name === y.user_details.first_name ? 'bg-transparent border-0 d-flex align-items-center' : 'd-none'}
                                onClick={() => { deleteReply(y.dprid, x.document_post_id.ddpid, index) }} style={{ height: '20px' }}>
                          ...
                          <div className='ps-0 ps-lg-5 mt-2 m-0'>
                            ...
                            {y.image.map((a) => {
                              return (<div><img src={a.image} style={{ width: '250px' }} alt="shw-pdf-ser" className='mt-3' /></div>)
                            })}
                          </div>
                                    ...
                                    <div className={`ps-0 ${discussionpagelayout ? 'ps-md-3' : 'ps-md-0'} py-2 mt-3 bg-white`} onClick={() => { setindex1(-1) }} >
                                      <div className="row w-100 align-items-center m-0">
                                        <div className="col-2 col-md-1 d-flex justify-content-center">
                                          <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="shw-pdf-ser" />
                                        ...
                                        {z.images_attached.map((b) => {
                                          return (
                                            <div className='d-flex justify-content-center'>
                                              <img src={b.dis_reply_reply_images} width={260} alt="shw-pdf-ser" className='mt-3' />
                                            </div>
                                          )
                                        })}
                                      </div>
                ...
                <div className='d-flex gap-2 mt-3 pt-3 border-secondary-subtle align-items-center' onClick={() => { setindex1(-1) }}>
                  <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="shw-pdf-ser" />
                  ...
                  <div className="input-group border rounded pe-3 bg-light">
                    ...
                    <div className='d-flex align-items-center bg-light'>
                      <input id="file2" type='file' name='file' accept="image/*" multiple onChange={handleReplyImage2} className="bg-light text-center p-3 btn" />
                      <label htmlFor="file2" className="custom-file-input bg-transparent border-0 px-4 py-2">
                      ...
                      <button className='h-100 bg-transparent border-0 ms-2' disabled={replies.length > 0 ? false : true} onClick={() => { postReplies(x.document_post_id.ddpid, index); }}>
                        <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
