...
const Dashboard = ({ language }) => {
  ...
  setEncryptedData("name", JSON.stringify('dashboard'), 180);
  setEncryptedData("path", JSON.stringify('/dashboard/page'), 180);
  ...
  useEffect(() => {
    ...
    axiosInstance.get(`${ipaddress}/docAndLikesCount/${user.user_id}/`)
      .then((r) => {
        setDatacount(r.data); // console.log("Documents and likes count",r.data)
      }).catch((err) => { console.log("docAndLikesCount Error") });
    axiosInstance.get(`${ipaddress}/CoursesView/${user.user_id}/`)
      .then((r) => {
        setall_joined_courses(r.data.joined_courses.reverse()); // console.log("Coursessss",r.data)
  ...
  const loadMoreDiscussions = () => { setVisibleDiscussions((prevVisibleDiscussions) => prevVisibleDiscussions + discussionsPerPage); };
  // -------------------------------------------------------LOAD MORE END----------------------------------------------------
  ...
  const postQuestion = async (e) => {
    ...
    const encoded_question = encodeURIComponent(question); // To encode the data
  ...
  const handleReplyImage1 = (event) => {
    const files = event.target.files;
    if (files && files.length > 0) {  // Ensure that 'files' is not null or undefined
  ...
  const postReplies = async (event, dis_id, index) => {
  ...
  const postreply_for_replies = async (event) => {
    event.preventDefault();
    const formData = new FormData();
  ...
  return (
    <>
      {loading ? (<Preloader />) : (
        <div className="d-flex dashboard position-relative">
          {/* ----------------------------Main Sidebar------------------------------- */}

          <div className="w-100">
            {/* ----------------------------Navbar------------------------------- */}

            <div className="container main-content" onClick={() => { setcourse_visible(false); setgroup_visible(false); setstudylist_visible(false); setnavbar_dropdown_visible(false) }}>

              {/* ----------------------------User Details------------------------------- */}
              <div className="d-flex justify-content-between px-2 px-lg-5 align-items-center bg-white shadow rounded mx-0 p-4 px-1">
                <div className="d-flex">
                  <img src={userdetails.profile_pic} width={50} className={userdetails.profile_pic == null ? 'd-none' : 'd-inline rounded'} style={{ objectFit: 'cover' }} alt="dashboard" />
                  {
                    ...
                    <p className={userdetails.profile_pic == null ? 'bg-success text-white p-2 rounded-circle m-auto' : 'd-none'} style={{ lineHeight: "1.25" }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>
                  }
                  <div className="ms-2">
                ...
                <div className="profile-details d-flex justify-content-between flex-column flex-lg-row">
                  <div className="subject justify-content-lg-center" onClick={() => navigate('/courses')} style={{ cursor: 'pointer' }}>
                    ...
                    <span className="ms-2" style={{ fontSize: '14px', letterSpacing: '3.5px', cursor: 'pointer' }}>
                      <span className="d-none d-lg-block">
                        {translate_value.dashboard.courses} : </span></span>
                    <span>{alljoined_courses.length}</span>

                  </div>

                  <div className="subject" style={{ cursor: 'pointer' }} onClick={() => { navigate(`/userfiles/${user.user_id}/${'uploaded_documents'}`) }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 35 35" fill="none">
                    ...
                    <span className="ms-2" style={{ fontSize: '14px', letterSpacing: '3.5px' }}>
                      <span className="d-none d-lg-block">{translate_value.dashboard.uploads} : </span>
                    </span>
                    <span className="">{datacount.documents_count}</span>
                  </div>

                  <div className="subject" style={{ cursor: 'pointer' }} onClick={() => { navigate(`/profile/${user.user_id}`) }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 35 35" fill="none">
                    ...
                    <span className="ms-2" style={{ fontSize: '14px', letterSpacing: '3.5px' }}>
                      <span className="d-none d-lg-block">{translate_value.dashboard.upvotes} : </span></span><span className="">{datacount.total_documents_likes}
                    </span>
                  </div>
                </div>
              ...
              <div className="row">
                <p className={`container py-3 m-0`} style={{ fontSize: '20px', fontWeight: 500, letterSpacing: '0.4px' }}>
                  {translate_value.dashboard.recommended_documents}
                </p>
                                ...
                                <div className="card p-1 border-0 view_pdf" style={{ height: '300px',boxShadow:'none' }}>
                                  <div className="card-body p-0" key={i}>
                                    ...
                                    <div className="dashboard-cards" style={{ height: '250px', width: '100%', position: 'absolute', top: 0, left: 0, cursor: 'pointer' }}>
                                      <div className="h-100 d-flex justify-content-between align-items-end border pe-none" style={{ pointerEvents: 'none' }}>
                                        <div>
              ...
              <div className="row mt-4">
                <div className="col-lg-12">
                  <div className="row" onClick={() => { setindex1(-1) }}>
                    <div className="col-lg-9">
                      <div className="d-flex">
                        <div onClick={() => { settablist(true) }} className="tabs" style={{ backgroundColor: tablist ? '#5D5FE3' : '#fff', color: tablist ? '#fff' : '#5D5FE3' }}>
                          {translate_value.dashboard.news_feed}
                        </div>
                        <div className="tabs" onClick={() => { setpincomment_status(true); settablist(false) }} style={{ backgroundColor: tablist ? '#fff' : '#5D5FE3', color: tablist ? '#5D5FE3' : '#fff', borderRadius: '0px 5px 5px 0px' }}>
                          {translate_value.dashboard.followed}
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="d-flex">
                        <div className={`input-group bg-light rounded border`}>
                          ...
                          <input type="text" disabled={!tablist} className={`form-control bg-transparent border-0 ps-0 sub-search`} placeholder={translate_value.common_words.search_discussion} value={searchcomment}
                            onChange={(e) => { setSearchcomment(e.target.value) }} aria-label="Username" aria-describedby="basic-addon1" style={{ fontSize: '14px' }} />
                        </div>
                        <span data-bs-toggle="tooltip" data-bs-placement="top" className='d-flex align-items-center' data-bs-custom-class="custom-tooltip" data-bs-title="My comments">
                          <span data-bs-toggle="modal" data-bs-target="#usercommentsmodal" onClick={() => { setusercomments_status(true) }} className="ms-2 p-2 rounded px-2 px-md-3 d-flex align-items-center justify-content-center border" style={{ color: '#5D5FE3', cursor: 'pointer' }}>
              ...
              <div className="row">
                <div className="col-lg-9 mt-4 comment-section">
                  {/* ----------------------------Comment Post Modal------------------------------- */}
                  <div className='w-100'>
                          ...
                          <div className="modal-body">
                            <div className='col-12 bg-white px-lg-3 pt-2 pb-3'>
                              ...
                              <div className='d-flex gap-1 gap-lg-3 align-items-center'>
                                <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="dashboard" />
                                ...
                                <form onSubmit={postQuestion} className="w-100">
                                  <div className="input-group bg-light border rounded pe-lg-3">
                                    <input type="text" name="question" value={question} onChange={questionData} className="form-control py-3 ps-3 bg-light border-0 shadow-none post-input" placeholder="Post your Comment....." style={{ position: 'relative' }} />
                                    <div className='d-flex align-items-center bg-light'>
                                      <input id="fileInput" type='file' name='file' accept="image/*" multiple onChange={handleImageChange} className="bg-light text-center p-3 btn" />
                                      <label data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Attach Image" htmlFor="fileInput" className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                                        <img src={require('../img/attachment.png')} width={22} height={22} alt="dashboard" />
                                      </label>
                                      <button type="submit" disabled={question.length > 0 ? false : true} data-bs-dismiss="modal" onClick={postQuestion} className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' >
                                        ...
                                        <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                          ...
                                          data-bs-title="Post" xmlns="http://www.w3.org/2000/svg" className={`${load ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 30 30" fill="none">
                                ...
                                {selectedImages.length > 0 &&
                                  selectedImages.map((image, index) => (
                                    <div key={index} className="image-preview bg-light p-2" style={{ position: 'relative' }}>
                                      ...
                                      <button style={{ position: 'absolute', top: '-10px', right: '-16px' }} className='btn btn-sm' onClick={() => removemainimage(index)}>
                  ...
                  <div className={`${tablist && searchcomment.length <= 0 ? '' : 'd-none '}`}>
                          ...
                          <div className='col-12 bg-white rounded shadow-sm border mb-3'>
                            <div className="d-flex ps-2 justify-content-between border-bottom py-2 m-0 align-items-center">
                              <div className="d-flex align-items-center gap-2">
                                <div className="px-1 px-lg-0 d-flex justify-content-center">
                                  {x.user_id.profile_pic && <img src={x.user_id.profile_pic} width={40} height={40} alt="dashboard" className="rounded-circle" />}
                                  {x.user_id.profile_pic == null &&
                                    <p className={'d-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto'} style={{ height: '40px', width: '40px' }}>
                                      <span>{x.user_id.nickname.slice(0, 1)}</span>
                                      <span>{x.user_id.nickname.slice(-1)}</span>
                                    </p>
                                  }
                                </div>

                                <div className={`p-0`} onClick={() => { setindex1(-1) }} >
                              ...
                              <div className={`justify-content-end gap-2 d-flex align-items-center p-0`} style={{ position: 'relative' }}>
                                <p className='ms-sm-0 my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA' }}>
                                  <span className={`ms-2 edit ${x.edited ? '' : 'd-none'}`}>Edited</span></p>
                                <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Unfollow")}>
                                  <button data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip"
                                    data-bs-title="Unfollow" onClick={() => { unpin(x.id) }} className={`p-1 btn btn-sm border px-2 ${x.pinned_status ? 'd-none d-md-inline' : 'd-none'}`} style={{ cursor: 'pointer' }}><span className=''>Followed</span></button>
                                </OverlayTrigger>

                                <span className="border-0" type="button" style={{ cursor: 'pointer' }} onClick={() => { (index1 == index) ? setindex1(-1) : setindex1(index) }}>
                                ...
                                <ul className={`bg-white shadow-sm border rounded mt-0 p-0 px-3 ${index1 == index ? '' : 'd-none'}`} style={{ width: '160px', position: 'absolute', right: '26px', top: '0px' }}>
                                  <button className={x.user_id.user_id != user.user_id || x.created_at.includes("day") || x.created_at.includes("week") || x.created_at.includes("year") ? 'd-none' : 'bg-transparent border-0 my-2 d-flex align-items-center'} data-bs-toggle="modal" data-bs-target="#editModal3"
                                    onClick={() => { editPosts(x.id) }} style={{ height: '20px' }}>
                                  ...
                                  <button className={`bg-transparent border-0 my-2 ${x.pinned_status ? 'd-none' : 'd-flex align-items-center'}`}
                                    onClick={() => { pincomment(x.id) }} style={{ height: '20px' }}>
                                    <span className="dropdownmenu">
                                  ...
                                  {user.user_id !== x.user_id.user_id &&
                                    <>
                                      ...
                                      <button className={`bg-transparent border-0 my-2 ${x.reported_status ? 'd-flex align-items-center' : 'd-none'}`} style={{ height: '20px', color: '#FF845D' }}>
                                        <span className="dropdownmenu">
                                          <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 35 35" fill="none">
                                            <path d="M17.4997 23.9505C17.754 23.9505 17.9671 23.8645 18.1391 23.6925C18.3111 23.5205 18.3971 23.3073 18.3971 23.0531C18.3971 22.7988 18.3111 22.5856 18.1391 22.4136C17.9671 22.2416 17.754 22.1556 17.4997 22.1556C17.2454 22.1556 17.0322 22.2416 16.8602 22.4136C16.6882 22.5856 16.6023 22.7988 16.6023 23.0531C16.6023 23.3073 16.6882 23.5205 16.8602 23.6925C17.0322 23.8645 17.2454 23.9505 17.4997 23.9505ZM16.7705 19.6316H18.2288V10.7694H16.7705V19.6316ZM12.6479 29.1668L5.83301 22.3651V12.6484L12.6347 5.8335H22.3514L29.1663 12.6352V22.3519L22.3646 29.1668H12.6479ZM13.2705 27.7085H21.7288L27.708 21.7293V13.271L21.7288 7.29183H13.2705L7.29134 13.271V21.7293L13.2705 27.7085Z" fill="#FF845D" />
                                          </svg>
                                  ...
                                  <button className={`bg-transparent border-0 my-2 ${user.user_id === x.user_id.user_id ? 'd-flex align-items-center' : 'd-none'}`}
                                    onClick={() => { deletePost(x.id) }} style={{ height: '20px' }}>
                                    <span className="dropdownmenu">
                                          ...
                                          <div className='d-flex gap-3'>
                                            ...
                                            <form onSubmit={(e) => sendEditedData(e, x.id)} className="w-100">
                                              <div className="input-group bg-light rounded border pe-3">
                                                <input type="text" name="question" onChange={(e) => editpostfunctionData(e.target.value)}
                                                  value={editedpost} className="form-control bg-transparent py-3 ps-3 shadow-none border-0" placeholder="Ask a question....."
                                                  style={{ position: 'relative' }} />
                                                <button data-bs-dismiss="modal" type="submit" disabled={editedpost.length > 0 ? false : true} className='text-secondary bg-transparent border-0'>
                            ...
                            <div onClick={() => { setindex1(-1) }} className='py-2 px-2'>
                              <p className='m-0 discussion_post' id={x.id}>{x.discussion}</p>
                              {x.images_attached && (
                                x.images_attached.map((z) => {
                                  return (
                                    <div className='d-flex justify-content-center'>
                                      <img src={z.image} width={300} alt="dashboard" className='mt-3' />
                                    </div>
                                  )
                                })
                              )}
                            ...
                            <div onClick={() => { setindex1(-1) }} className='d-flex justify-content-between border-bottom px-2 pb-2 mb-2'>
                              <div className="d-flex align-items-center gap-2">
                                <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: x.status ? "#ff845d" : "gray" }} onClick={() => { (x.status == true) ? handleLike1(x.id) : handleLike(x.id) }}>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                  ...
                                  <span style={{ fontSize: '14px', fontWeight: 500, lineHeight: '22px' }} className="ms-1">{x.likes}</span>
                                </button>

                                <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: x.dis_status ? "#ff845d" : "gray" }} onClick={() => { (x.dis_status == true) ? handledislike1(x.id) : handledislike(x.id) }}>
                                  <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                              ...
                              {x.replies_count > 0
                                && (
                                  <div className="d-flex gap-2">
                                    {(!replies_status) ?
                                      <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreply_status(!replies_status); setSelectedReply(index); getreplies(x.id) }}>
                                        {translate_value.dashboard.view_all} {x.replies_count} {translate_value.dashboard.replies}
                                      </button>
                                      :
                                      (selectedReply == index) ?
                                        <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreply_status(!replies_status); setSelectedReply(null); }}>
                                          {`View less`}
                                        </button> :
                                        <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreply_status(!replies_status); setSelectedReply(index); getreplies(x.id) }}>
                                          {translate_value.dashboard.view_all} {x.replies_count} {translate_value.dashboard.replies}
                                    ...
                                    <div className="comment-section-img d-flex">
                                      {/* VBM */}
                                      {x.replies && x.replies.map((a, index) => {
                                        return (
                                          <>{
                                            index < 3 ?
                                              a.user_id && a.user_id.profile_pic ?
                                                <img className="rounded-circle" style={{ objectFit: 'cover', backgroundColor: '#fff', position: 'relative', marginLeft: index > 0 ? '-11px' : '0px', border: '2px solid #fff' }} src={a.user_id.profile_pic} width={30} height={30} alt="dashboard" />
                                                :
                                                <p className={'text-white rounded-circle my-auto d-flex justify-content-center align-items-center'} style={{ fontSize: '14px', height: '30px', width: '30px', objectFit: 'cover', position: 'relative', marginLeft: index > 0 ? '-11px' : '0px', border: '2px solid #fff', backgroundColor: '#b13a1c' }}><span>{a.user_id.nickname.slice(0, 1)}</span><span>{a.user_id.nickname.slice(-1)}</span></p>
                                              :
                                              (index == 3) && <p className={'text-white rounded-circle my-auto d-flex justify-content-center align-items-center'} style={{ fontSize: '14px', backgroundColor: '#5d5fe3', height: '30px', width: '30px', objectFit: 'cover', position: 'relative', marginLeft: index > 0 ? '-11px' : '0px', border: '2px solid #fff' }}><span>{'+' + (x.replies.length - 3)}</span></p>
                                          }
                            ...
                            {fetchedreplies && fetchedreplies.length > 0 && (
                              <div className={fetchedreplies[0].udisid == x.id && replies_status ? 'd-block' : 'd-none'}>
                                {fetchedreplies.map((y, index2) => {
                                  return (
                                    <div key={index2} className='px-2 py-2 bg-white' onClick={() => { setindex1(-1) }}>
                                      <div className="d-flex w-100 justify-content-between align-items-center px-2">
                                        <div className="d-flex gap-2">
                                          <div className="d-flex justify-content-start">
                                            <img src={y.user_id.profile_pic} className={y.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="dashboard" />
                                        ...
                                        <div className="px-0 p-0 d-flex justify-content-end gap-3">
                                          <button className='bg-transparent border-0 d-flex align-items-center'
                                            style={{ height: '20px', color: y.status ? "#ff845d" : "gray" }}
                                            onClick={() => { (y.status == true) ? handleReplyLike1(y.id, x.id, index) : handleReplyLike(y.id, x.id, index) }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                              <path d="M21.8269 24.9999H9.27884V11.2499L17.1154 3.50952L17.6683 4.06243C17.7821 4.1762 17.8778 4.32203 17.9555 4.49993C18.0333 4.6778 18.0721 4.84126 18.0721 4.9903V5.18743L16.7933 11.2499H25.4808C26.0032 11.2499 26.4704 11.4559 26.8822 11.8677C27.2941 12.2796 27.5 12.7467 27.5 13.2691V14.8076C27.5 14.9214 27.4872 15.0456 27.4615 15.1802C27.4359 15.3148 27.4023 15.439 27.3606 15.5528L23.9471 23.6442C23.7756 24.0288 23.4872 24.3509 23.0818 24.6105C22.6763 24.8701 22.258 24.9999 21.8269 24.9999ZM10.5288 23.7499H21.8269C22.0032 23.7499 22.1835 23.7019 22.3678 23.6057C22.5521 23.5095 22.6923 23.3493 22.7885 23.1249L26.25 14.9999V13.2691C26.25 13.0448 26.1779 12.8605 26.0337 12.7163C25.8894 12.572 25.7051 12.4999 25.4808 12.4999H15.2404L16.6875 5.67299L10.5288 11.7836V23.7499ZM9.27884 11.2499V12.4999H5V23.7499H9.27884V24.9999H3.75V11.2499H9.27884Z" fill="currentColor" />
                                            </svg>
                                            <span className="ms-1">{y.likes_count}</span>
                                          </button>
                                          <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: y.dis_like_status ? "#ff845d" : "gray" }}
                                            onClick={() => { (y.dis_like_status == true) ? handlereplydislike1(y.id, x.id, index) : handlereplydislike(y.id, x.id, index) }}>
                                            <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                              <path d="M8.17306 5.00007H20.7212V18.7501L12.8846 26.4905L12.3317 25.9376C12.2179 25.8238 12.1222 25.678 12.0445 25.5001C11.9667 25.3222 11.9279 25.1587 11.9279 25.0097V24.8126L13.2067 18.7501H4.51922C3.99678 18.7501 3.52963 18.5441 3.11778 18.1323C2.70593 17.7204 2.5 17.2533 2.5 16.7309V15.1924C2.5 15.0786 2.51282 14.9544 2.53847 14.8198C2.56409 14.6852 2.59774 14.561 2.63941 14.4472L6.05288 6.35582C6.22435 5.97122 6.51281 5.6491 6.91825 5.38948C7.32371 5.12987 7.74198 5.00007 8.17306 5.00007ZM19.4712 6.25007H8.17306C7.99679 6.25007 7.81651 6.29814 7.63222 6.39429C7.44793 6.49046 7.3077 6.65072 7.21153 6.87507L3.75 15.0001V16.7309C3.75 16.9552 3.82211 17.1395 3.96634 17.2837C4.11057 17.428 4.29486 17.5001 4.51922 17.5001H14.7596L13.3125 24.327L19.4712 18.2164V6.25007ZM20.7212 18.7501V17.5001H25V6.25007H20.7212V5.00007H26.25V18.7501H20.7212Z" fill="currentColor" />
                                            </svg>
                                          </button>
                                          <button className={user.first_name === y.user_id.first_name ? 'bg-transparent border-0 d-flex align-items-center' : 'd-none'} onClick={() => { deleteReply(y.id, x.id) }} style={{ height: '20px' }}>
                                      ...
                                      <div className='ps-5 mt-2'>
                                        ...
                                        <div className="mt-2 ps-0">
                                          <span data-bs-toggle="modal" data-bs-target="#replyforreply_modal7" onClick={() => { setdiscuss_id(x.id); setreply_id(y.id) }} style={{ cursor: 'pointer' }}
                                          ...
                                          {fetchedreplies_for_reply && fetchedreplies_for_reply.length > 0 && (
                                            <div className={fetchedreplies_for_reply[0].reply == y.id && replies_for_reply_status ? 'd-block' : 'd-none'}>
                                              {fetchedreplies_for_reply.map((z) => {
                                                ...
                                                return (
                                                  <div className='px-2 py-2 bg-white' onClick={() => { setindex1(-1) }}>
                                                    <div className="d-flex w-100 justify-content-between">
                                                      <div className="d-flex gap-2">
                                                        <div className="">
                                                          <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="dashboard" />
                                                        ...
                                                        <div className="">
                                                          ...
                                                          <p className='ms-sm-0 my-0' style={{ fontSize: '13px' }}>{z.created_at}

                                                          </p>
                                                        </div>
                                                      </div>
                                                      <div className="d-flex align-items-center gap-3">
                                                        <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: z.liked_status ? "#ff845d" : "gray" }} onClick={() => { handleReplies_reply_like(z.id, y.id) }}>
                                                          <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                            <path d="M21.8269 24.9999H9.27884V11.2499L17.1154 3.50952L17.6683 4.06243C17.7821 4.1762 17.8778 4.32203 17.9555 4.49993C18.0333 4.6778 18.0721 4.84126 18.0721 4.9903V5.18743L16.7933 11.2499H25.4808C26.0032 11.2499 26.4704 11.4559 26.8822 11.8677C27.2941 12.2796 27.5 12.7467 27.5 13.2691V14.8076C27.5 14.9214 27.4872 15.0456 27.4615 15.1802C27.4359 15.3148 27.4023 15.439 27.3606 15.5528L23.9471 23.6442C23.7756 24.0288 23.4872 24.3509 23.0818 24.6105C22.6763 24.8701 22.258 24.9999 21.8269 24.9999ZM10.5288 23.7499H21.8269C22.0032 23.7499 22.1835 23.7019 22.3678 23.6057C22.5521 23.5095 22.6923 23.3493 22.7885 23.1249L26.25 14.9999V13.2691C26.25 13.0448 26.1779 12.8605 26.0337 12.7163C25.8894 12.572 25.7051 12.4999 25.4808 12.4999H15.2404L16.6875 5.67299L10.5288 11.7836V23.7499ZM9.27884 11.2499V12.4999H5V23.7499H9.27884V24.9999H3.75V11.2499H9.27884Z" fill="currentColor" />
                                                          </svg>
                                                          <span className="ms-1" style={{ fontSize: '14px' }}>{z.likes}</span>
                                                        </button>
                                                        <button className='bg-transparent border-0 d-flex align-items-center' style={{ height: '20px', color: z.dis_liked_status ? "#ff845d" : "gray" }} onClick={() => { handlereplies_replydislike(z.id, y.id) }}>
                                                          <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                                            <path d="M8.17306 5.00007H20.7212V18.7501L12.8846 26.4905L12.3317 25.9376C12.2179 25.8238 12.1222 25.678 12.0445 25.5001C11.9667 25.3222 11.9279 25.1587 11.9279 25.0097V24.8126L13.2067 18.7501H4.51922C3.99678 18.7501 3.52963 18.5441 3.11778 18.1323C2.70593 17.7204 2.5 17.2533 2.5 16.7309V15.1924C2.5 15.0786 2.51282 14.9544 2.53847 14.8198C2.56409 14.6852 2.59774 14.561 2.63941 14.4472L6.05288 6.35582C6.22435 5.97122 6.51281 5.6491 6.91825 5.38948C7.32371 5.12987 7.74198 5.00007 8.17306 5.00007ZM19.4712 6.25007H8.17306C7.99679 6.25007 7.81651 6.29814 7.63222 6.39429C7.44793 6.49046 7.3077 6.65072 7.21153 6.87507L3.75 15.0001V16.7309C3.75 16.9552 3.82211 17.1395 3.96634 17.2837C4.11057 17.428 4.29486 17.5001 4.51922 17.5001H14.7596L13.3125 24.327L19.4712 18.2164V6.25007ZM20.7212 18.7501V17.5001H25V6.25007H20.7212V5.00007H26.25V18.7501H20.7212Z" fill="currentColor" />
                                                          </svg>
                                                        </button>
                                                        <button className={user.first_name === z.user_id.first_name ? 'bg-transparent border-0 d-flex align-items-center' : 'd-none'} onClick={() => { deleteReply_for_reply(z.id, y.id, x.id) }} style={{ height: '20px' }}>
                            ...
                            <div onClick={() => { setindex1(-1) }} className='d-flex gap-2 px-2 border-secondary-subtle align-items-center mb-2'>
                              {userdetails.profile_pic != null && <img src={userdetails.profile_pic} className={'rounded-circle'} width={40} height={40} alt="dashboard" />}
                              {
                                userdetails.nickname != undefined ?
                                  (userdetails.profile_pic == null) &&
                                  <p className='d-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto'
                                    style={{ height: '40px', width: '40px' }}>
                                    <span>{userdetails.nickname.slice(0, 1)}</span>
                                    <span>{userdetails.nickname.slice(-1)}</span>
                                  </p> : ''
                              }
                              <form className="w-100" onSubmit={(event) => postReplies(event, x.id, index)}>
                                <div className="input-group border rounded pe-lg-3 bg-light">
                                  <input key={index} type="text" name={post} onChange={(e) => { repliesData(e); setSelectedPostForComment(x.id); }}
                                    className="form-control py-2 ps-3 shadow-none border-0 bg-light reply-input original-reply-input" placeholder={translate_value.dashboard.reply_here} style={{ position: 'relative' }} />

                                  <div className='d-flex align-items-center bg-light'>
                                    <input id="file1" type='file' name='file' accept="image/*" multiple onChange={handleReplyImage1}
                                      className="bg-light text-center p-3 btn" />
                                    <label data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip"
                                      data-bs-title="Attach Image" htmlFor="file1" className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                                      <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                                    ...
                                    <button type="submit" disabled={replies.length > 0 ? false : true} className='h-100 bg-transparent border-0 ms-2'>
                                      <div style={{ width: '30px' }} className={`${(load2 && selectedPostForComment == x.id) ? '' : 'd-none'}`}>
                                      ...
                                      <svg data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Post" xmlns="http://www.w3.org/2000/svg"
                                        className={`${(load2 && selectedPostForComment == x.id) ? 'd-none' : ''}`} width="25" height="25" viewBox="0 0 30 30" fill="none">
                                        <path d="M5 23.125V6.875L24.2789 15L5 23.125ZM6.25 21.25L21.0625 15L6.25 8.75V13.6058L12.3077 15L6.25 16.3942V21.25Z" fill="#8E9696" />
                                      </svg>
                                    </button>
                                  </div>
                            ...
                            {(repliesImage1 && repliesImage1.length > 0) &&
                              <div className='d-flex gap-3 mt-3'>
                                {
                    ...
                    {visibleDiscussions < discussions.length && (
                      <div className="py-3 col-sm-12 text-center">
                        <button onClick={loadMoreDiscussions} className="btn border rounded px-5" style={{ color: 'white', backgroundColor: '#5D5FE3' }}>
                          Load More
                        </button>
                      </div>
                    )}
                ...
                {/* ----------------------------Post Comment Button------------------------------- */}
                <div className="col-lg-3 mt-4 comment-post-section">
                  <button className='btn w-100 fw-medium d-lg-none' style={{ border: '1px solid #5D5FE3', color: '#5D5FE3', fontSize: '16px', fontWeight: 500, letterSpacing: '0.32px', lineHeight: '22px' }} data-bs-toggle="modal"
                    data-bs-target="#postmodal">{translate_value.dashboard.create_a_post}</button>

                  <div onClick={() => { setindex1(-1) }} className="rounded shadow-sm border d-none d-lg-block">
                    <div className="w-100 pt-2 px-2 bg-white shadow-sm rounded d-flex flex-column align-items-center" style={{ height: '400px' }}>
                      <img src={require('../img/images_icons/send-img1.png')} style={{ height: '60px', width: '60px' }} alt="dashboard" />
                      <img src={require('../img/images_icons/mascot1.png')} style={{ height: '250px', width: '118px' }} alt="dashboard" />
              ...
              <div className="modal-content">
                <div className="modal-body">
                  <div className='col-12 bg-white px-lg-3 pt-2 pb-3'>
                    ...
                    <form className="w-100" onSubmit={(event) => postreply_for_replies(event)}>
                      <div className='d-flex gap-lg-3 gap-1 align-items-center'>
                        <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="dashboard" />
                        {userdetails.nickname != undefined ? (<p className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>) : (<></>)}
                        <div className="input-group bg-light border rounded pe-lg-3">
                          <input type="text" name="question" value={reply_for_reply} onChange={(e) => { setReply_for_reply(e.target.value) }}
                            className="form-control py-3 ps-3 bg-light border-0 shadow-none post-input" placeholder="Post your reply....."
                            style={{ position: 'relative' }} />
                          <div className='d-flex align-items-center bg-light'>
                            <input id="fileInput2" type='file' name='file' accept="image/*" multiple onChange={handleReply_reply_Image} className="bg-light text-center p-3 btn" />
                            <label data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip" data-bs-title="Attach Image" htmlFor="fileInput2" className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                              <img src={require('../img/attachment.png')} width={22} height={22} alt="dashboard" />
                            </label>
                            <button type="submit" disabled={reply_for_reply.length > 0 ? false : true} data-bs-dismiss="modal" className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' >
                              <svg data-bs-toggle="tooltip" data-bs-placement="top"
                                data-bs-custom-class="custom-tooltip"
                    ...
                    <div className='d-flex gap-3 mt-3'>
                      {replies_reply_Image.length > 0 &&
                        replies_reply_Image.map((image, index) => (
                          <div key={index} className="image-preview bg-light p-2" style={{ position: 'relative' }}>
                            <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
                            <button style={{ position: 'absolute', top: '-10px', right: '-16px' }} className='btn btn-sm' onClick={() => removeImage3(index)}>
                              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
                                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                                <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708" />
                              </svg>
