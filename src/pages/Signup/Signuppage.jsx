...
const Signuppage = () => {
  ...
  const search_domain = (value) => { // To get the organization email id domains
  ...
  // const organizationData=(e)=>{ setUniversity(e.target.value)}
  ...
  const fetchOTP = async () => {
    ...
    try {
      const response = await apiClient.post(`${ipaddress}/userverification/`, organizationMail);
      if (response.data === 'This email is already registered with other account') {
  ...
  const fetchMatchedorganization = (city_name) => {
    fetched_organization_details.map((x) => {
      if (x.city === city_name) {
        ...
        console.log("Fetch the organization on the selected city!");
      }
    })
  }
  ...
  const sendorganizationdata = (e) => {
    ...
    apiClient.put(`${ipaddress}/UserUniversityAddition/`, formdata)
      .then((r) => {
        document.getElementById('signup').style.display = 'none'; // console.log("Organization ID sent successfully",r.data)
        setvalue(true)
      }).catch((err) => { console.log("Error", err) })
  }
  // ------------------------------------------------ADD Organization------------------------------------------------------
                ...
                <div className="col-lg-10 p-0 forms-div m-auto">
                      ...
                      <div className="pt-4">
                        <div className="">
                          <label htmlFor="formGroupExampleInput" className="form-label signup-labels d-flex align-items-center" style={{ fontSize: '16px', color: '#6c757d' }}>
                            ...
                            <OverlayTrigger placement="top" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Only Organization Emails are allowed")}>
                          ...
                          <OverlayTrigger placement="bottom" delay={{ show: 250, hide: 250 }} overlay={renderTooltip("Only Organization Emails are allowed")}>
                            <input type="text" className="form-control bg-light py-2" style={{height:'50px'}}  placeholder="Enter your Organization Email Id" value={organization} aria-label="Username" name="organization_mailid" onChange={(e) => {
                  ...
                  {signupform == 3 &&
                    <div className="bg-white shadow rounded pb-2 px-4 h-100 position-relative signupform3" onClick={() => { setShowOptions(false); setShowcity(false) }}>
                     <h3 className="page6-month text-center mb-0" style={{ fontSize: '35px', color: '#5D5FE3' }}>{translate_value.signup_page.signup}</h3>
                      {/* <h3 className="page6-month mb-0 text-center" style={{ fontSize: '35px', color: '#5D5FE3' }}>{translate_value.signup_page.signup}</h3> */}
                      <p className="fw-bold text-center mb-0" style={{ color: '#5d5fe3' }}>Kindly Add your Organization</p>
                      <form action="" className="p-2">
                          ...
                          <div className="col-12 mt-3">
                            ...
                            <div className={`px-3 py-2 bg-light border border-top-0 ${message === "Organization is not present" && city.length > 0 && cities.length > 0 ? '' : 'd-none'}`} style={{ maxHeight: '200px', overflowY: 'scroll' }}>
                              {cities.map((x) => {
                                return (
                                  <>
                                    <p onClick={() => { fetchMatchedorganization(x.name); setCity(x.name); setCities([]); }} className="m-0" style={{ cursor: 'pointer' }}>{x.name}</p>
                                  </>
                                )
                              })}
                            </div>
                        ...
                        <div className={`text-center ${message === "Organization is not present" ? '' : 'my-2'}`}>
                          <button disabled={message === "Organization is not present" ? true : false} className="btn w-100 signup-btn btn-md py-2 px-2 px-md-5 text-white fw-medium" type="submit" onClick={sendorganizationdata}>
          ...
          <div className="modal-content">
            <div className="modal-body px-2  px-lg-5 py-5 mb-4 mt-2">
              <div className='d-flex flex-column align-items-center'>
                ...
                <input type="text" name="" id="" className='form-control py-2' onChange={(e) => { setorganizationname(e.target.value) }} />
              </div>
              <div className='mt-4'>
