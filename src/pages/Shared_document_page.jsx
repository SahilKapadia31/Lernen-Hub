...
const Shared_document_page = () => {
  ...
  useEffect(() => {
    apiClient.get(`${ipaddress}/DisplayDocument/${token}/`)
      .then((response) => {
  ...
  const generate_token = (e) => {
    e.preventDefault();
    apiClient.post(`${ipaddress}/api/token/`, {
      email: userid,
      password: password
    })
      .then((r) => {
      ...
      <div data-bs-toggle="modal" data-bs-target="#loginmodal">
        <div className="w-100 p-1 p-md-4 pt-3 showpdf-div pb-2 mb-3">
          ...
          <div className="m-0 d-flex align-items-center mt-4 gap-lg-3" >
            ...
            <span style={{ fontSize: '14px', color: '#fff' }} className='ms-3 d-flex align-items-center'>
              ...
              <span className='ms-2'>
                {pdfurldata && pdfurldata.documnet_id && pdfurldata.documnet_id.created_on != undefined ? (
                  (moment(pdfurldata?.documnet_id?.create_date).format("YYYY-MM-DD") ||"")
                ) : (<></>)}</span>
            </span>
          </div>
          <div className="mt-4 d-flex align-items-center gap-3">
            ...
            {pdfurldata && (
              <span className="text-white ms-3 d-flex align-items-center" style={{ fontSize: '14px' }}>
                {/* {pdfurldata.user_id.profile_pic!=undefined ? (<img src={pdfurldata.user_id.profile_pic} className={`me-2 my-auto ${pdfurldata.user_id.profile_pic !=null ? '':'d-none'}`} width={30} height={30} alt="doc" />):(<></>)} */}
