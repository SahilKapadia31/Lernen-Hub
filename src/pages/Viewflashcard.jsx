...
const Viewflashcard = () => {
  ...
  const renderTooltip = (value) => (<Tooltip className="custom-tooltip">{value}</Tooltip>);
  ...
  useEffect(() => {
    axiosInstance.get(`${ipaddress2}/flashcardsList/${flashset_id}/${user.user_id}/`)
      .then((r) => { setFlashset(r.data) })
      .catch(() => {
        console.log("Particular Flashset details Fetching Error")
      })
  }, [count]);
  // -----------------------------------------------Set Path----------------------------------------------------------------
  setEncryptedData('name3', JSON.stringify("View Flashset"), 180);
  setEncryptedData('path3', JSON.stringify(`/viewflashcard/${type}/${id}/${flashset_id}`), 180);
  // ----------------------To like the particular flashset-------------------------------------
  const likeFlashset = () => {
    axiosInstance.post(`${ipaddress2}/flashsetlikes/${user.user_id}/${flashset_id}/`)
      .then((r) => { setCount(count + 1) })
      .catch(() => { console.log("Flashset like error") })
  }
  ...
  const likeFlashcard = (flashcardId) => {
    axiosInstance.post(`${ipaddress2}/BookmarkFlashcard/${user.user_id}/${flashcardId}/`)
      .then((r) => { setCount(count + 1) })
      .catch(() => { console.log("Flashcard like error") })
  }
  // -----------------------------------Functionality to Delete the Flashcard-------------------------------------------------
  const deleteFlashcard = (flashcardId) => {
    axiosInstance.post(`${ipaddress2}/Delete_flashcard/`, { 'user_id': user.user_id, 'flashcard_id': flashcardId })
      .then((r) => { setCount(count + 1) })
      .catch(() => { console.log("Flashcard delete error") })
  }
  ...
  const reportCourse = () => {
    ...
    axiosInstance.post(`${ipaddress2}/FlashsetReport/`, report)
      .then((r) => {
        toast.warn('Successfully reported', { autoClose: 2000 });
  ...
  const changeScope = () => {
    axiosInstance.post(`${ipaddress2}/Change_scope/`, { 'flashset_id': flashset_id, 'scope': scope })
      .then((r) => {
        setCount(count + 1);
        setscope_status(true);
        setscope("change");
        navigate('/dashboard/page');
      }).catch((err) => { console.log("Flashset scope changig error", err) })
  }
    ...
    <div>
      <div className='d-flex' style={{ position: 'relative' }}>
        <div onClick={() => {
          ...
          }} className="" style={{ backgroundColor: '#F3F0FF' }}>
            {/* <div className=''> */}

            <div className='d-flex justify-content-between py-3 px-3 px-lg-4'>
              <div>
                <h5 onClick={() => {
                  setreportdropdownstate(false)
                }} className='fw-bold d-flex align-items-center mb-2' style={{ color: '#2A3941' }}>
                ...
                <div onClick={() => {
                  setreportdropdownstate(false)
                }} className='d-flex gap-2 gap-lg-4 mb-2 align-items-center' style={{ color: '#2A3941' }}>
                  ...
                  </svg>&nbsp;{Flashset.upvote_count} <span className='ms-2 d-none d-lg-block' style={{ fontSize: '14px' }}> Upvotes</span></span>
                  ...
                  </svg>&nbsp;{Flashset.flashcards_data.length} <span className='ms-2 d-none d-lg-block' style={{ fontSize: '14px' }}> Flashcards</span></span>
                  )}
              ...
              <div className='d-flex gap-3'>
                ...
                <div className="d-inline">
                  ...
                  <span className={`ms-4 ps-2`} style={{ cursor: 'pointer', position: 'relative', color: '#5D60E1' }} onClick={() => {
                    ...
                    <ul className={`${reportdropdownstate ? '' : 'd-none'} rounded bg-white border-0 py-2 px-2 report-dropdown`} style={{ width: '180px', listStyleType: 'none', top: 0, right: '30px', position: 'absolute' }}>
                ...
                <span className=''><svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="#5D5FE3" className="bi bi-share" viewBox="0 0 16 16">
          ...
          <div onClick={() => {
            ...
            <div onClick={() => {
              setreportdropdownstate(false)
            }} className='p-0 p-lg-4 bg-light'>
              <div className='d-md-flex justify-content-end align-items-center pb-2 pt-2 pt-lg-0'>
                {/* <h5>{Flashset.name}</h5> */}
                <div className='d-flex gap-2 py-2 overflow-x-auto'>
                  {Flashset && Flashset.scope === "private" ? (
                    <OverlayTrigger placement="top" overlay={renderTooltip('Add Flashcard')}>
                      <Link to={`/add_additional_flashcard/${type}/${id}/${flashset_id}`} title="Add More Flashset" className='bg-transparent border-0 text-primary mt-1'>
                  ...
                  <OverlayTrigger placement="top" overlay={renderTooltip('Change Scope')}>
                    <select value={scope} className={`rounded px-2 shadow-none ${Flashset.scope === "private" && type !== "group" ? '' : 'd-none'}`} style={{ border: '1px solid #5d5fe3' }} name="" id="" onChange={(e) => { setscope(e.target.value); setscope_status(true) }}>
                      <option value="change">Change the scope</option>
                      <option value="public">Public</option>
                    </select>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Study Now')}>
                    <Link
                      to={Flashset?.flashcards_data?.length > 0 ? `/filterflashcard/${type}/${id}/${flashset_id}` : "#"}
                      className={`text-white fw-medium btn ${Flashset?.flashcards_data?.length === 0 ? 'disabled' : ''}`}
                      style={{
                        backgroundColor: '#5d5fe3',
                        fontSize: '14px',
                        pointerEvents: Flashset?.flashcards_data?.length === 0 ? 'none' : 'auto',
                        opacity: Flashset?.flashcards_data?.length === 0 ? 0.5 : 1
                      }}
                    >
                      ...
                      {/* <span className='ms-2 d-none d-lg-block d-none d-lg-block'>Study Now</span> */}
                    </Link>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Save')}>
                    <button data-bs-toggle="modal" data-bs-target="#flashcard_studylist_modal" className={`fw-medium btn ${Flashset.studylist_status ? 'd-none' : ''}`} style={{ border: '1px solid #5d5fe3', fontSize: '14px', color: '#5d5fe3', backgroundColor: '#fff' }}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-bookmark" viewBox="0 0 16 16">
                        <path d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v13.5a.5.5 0 0 1-.777.416L8 13.101l-5.223 2.815A.5.5 0 0 1 2 15.5zm2-1a1 1 0 0 0-1 1v12.566l4.723-2.482a.5.5 0 0 1 .554 0L13 14.566V2a1 1 0 0 0-1-1z" />
                      </svg>
                      {/* <span className='ms-2 d-none d-lg-block'>Save</span> */}
                    </button>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Saved')}>
                    <button onClick={() => { unsave() }} className={`fw-medium btn ${Flashset.studylist_status ? '' : 'd-none'}`} style={{ border: '1px solid #5d5fe3', fontSize: '14px', color: '#fff', backgroundColor: '#5d5fe3' }}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-bookmark" viewBox="0 0 16 16">
                        <path d="M2 2a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v13.5a.5.5 0 0 1-.777.416L8 13.101l-5.223 2.815A.5.5 0 0 1 2 15.5zm2-1a1 1 0 0 0-1 1v12.566l4.723-2.482a.5.5 0 0 1 .554 0L13 14.566V2a1 1 0 0 0-1-1z" />
                      </svg>
                      {/* <span className='ms-2 d-none d-lg-block'>Saved</span> */}
                    </button>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Likes')}>
                    <button className='fw-medium btn d-flex align-items-center' onClick={likeFlashset} style={{ border: '1px solid #5d5fe3', fontSize: '14px', color: Flashset.upvote ? '#ff845d' : '#5d5fe3' }}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
                      ...
                      <span className='ms-1'>{Flashset.upvote_count}</span>
                    </button>
                  </OverlayTrigger>
                  <OverlayTrigger placement="top" overlay={renderTooltip('Dislikes')}>
                    <button className='fw-medium btn' onClick={() => { dislikeFlashset() }} style={{ border: '1px solid #5d5fe3', fontSize: '14px', color: Flashset.disupvote ? '#FF845D' : '#5d5fe3' }}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
              ...
              {Flashset.flashcards_data ? (
                Flashset.flashcards_data.map((x, index) => {
                  return (
                    <div className="h-auto">
                      <div className='d-flex gap-2 mb-0 align-items-center'>
                        <div className='row w-100'>
                          <div className='col-md-6 col-sm-12 d-none d-lg-block'>{index == 0 ? <h6 style={{ color: '#5d5fe3' }}>Term</h6> : ''}</div>
                          <div className='col-md-6 col-sm-12 d-none d-lg-block'>{index == 0 ? <h6 style={{ color: '#5d5fe3' }}>Definition</h6> : ''}</div>

                          <div className="col-md-6 col-sm-12 pe-0 mb-lg-0 h-auto">
                            <div data-bs-toggle="tooltip" data-bs-placement="right"
                              data-bs-custom-class="custom-tooltip"
                              data-bs-title="Term" className='d-flex justify-content-between border p-2 h-100'>
                              <p style={{ fontSize: '14px' }} className=''>{x.term}</p>
                              {x.t_image_url !== null && (<img src={x.t_image_url} width={80} className='rounded object-fit-contain' alt="view-fcard" />)}
                            </div>
                          </div>
                          <div className="col-md-6 col-sm-12 pe-0 mb-lg-0 h-auto">
                            <div data-bs-toggle="tooltip" data-bs-placement="right"
                              data-bs-custom-class="custom-tooltip"
                              data-bs-title="Definition" className='d-flex justify-content-between border p-2 h-100' style={{ backgroundColor: '#E2E3E5' }}>
                              <span style={{ fontSize: '14px' }}>{x.definition}</span>
                              {x.d_image_url !== null && (<img src={x.d_image_url} width={80} className='rounded object-fit-contain' alt="view-fcard" />)}
                            </div>
                          </div>
                        </div>
                        <div className="ps-3 pe-2 pe-lg-0 d-flex justify-content-center align-items-center gap-1 gap-lg-2">
                          <div onClick={() => { likeFlashcard(x.flashcard_id) }} className='px-3 py-2 d-flex justify-content-center align-items-center rounded' style={{ height: '40px', width: '50px', border: '1px solid rgb(93, 95, 227)', cursor: 'pointer' }}>
                          ...
                          {Flashset && Flashset.scope === "private" ?
                            (<div className='px-3 py-2 d-flex justify-content-center align-items-center rounded' style={{ height: '40px', width: '50px', border: '1px solid rgb(93, 95, 227)', cursor: 'pointer' }}>
                              <button onClick={() => { deleteFlashcard(x.flashcard_id) }} className='bg-transparent border-0 d-flex'>
                      ...
                      {((Flashset.flashcards_data.length - 1) !== index) ? <hr className='my-2'></hr> : ''}
                    </div>
                  )
                })
              ) : (
