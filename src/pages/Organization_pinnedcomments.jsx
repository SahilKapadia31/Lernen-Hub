...
const Organizationpinnedcomments = ({ count, pincomment_status, setCount }) => {
  ...
  const getpinnedcomments = () => {
    axiosInstance.get(`${ipaddress}/DisplayPinnedUniversityComments/${user.user_id}/${user.organization_id}/`)
      .then((r) => {
  ...
  // ---------------------------------------------------UNPIN COMMENT-------------------------------------------------------
  const unpin = (discid) => {
    axiosInstance.delete(`${ipaddress}/UniversityPinnedCommentsView/${user.user_id}/${discid}/${user.organization_id}/`)
      .then((r) => {
  ...
  const clearInput = (index) => {
    const inputElement = document.getElementsByClassName('followed-reply-input');
    if (inputElement) {
  ...
  const postReplies = async (e, dis_id, index) => {
    ...
    try {
      ...
      const response = await fetch(`${ipaddress}/UniversityDiscussionReplyView/${user.user_id}/${user.organization_id}/${dis_id}/""/?post=${encoded_reply}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        // console.log('Organization Reply Sent successfully',formData);
        setcount1(count1 + 1)
        setCount((prev) => prev + 1)
        clearInput(index)
        getreplies(dis_id)
  ...
  const postreply_for_replies = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${ipaddress}/UniversityDiscussionRepliesRepliesView/${user.user_id}/${user.organization_id}/${reply_id}/""/?post=${encoded_reply_reply}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      ...
      if (response.ok) {
        console.log('Organization Reply for reply Sent successfully', response.data);
        setReply_for_reply("")
  ...
  function handledislike(discussion_id) {
    axiosInstance.post(`${ipaddress}/UniversityDiscussionDisLikesView/${discussion_id}/${user.user_id}/`)
      .then((r) => {
        // console.log("Organization Disliked the Post",r.data)
        setcount1(count1 + 1)
        setCount((prev) => prev + 1)
      })
      .catch(() => {
        // console.log("Organization dislike error")
      })
  }
  ...
  function handlereplydislike1(discussion_id, discid, index) {
    axiosInstance.delete(`${ipaddress}/UniversityDiscussionReplyDisLikesView/${user.user_id}/${discussion_id}/`)
      .then((r) => {
        //  console.log("Organization reply Removed the dislike",r.data)
  ...
  function handlereplydislike(discussion_id, discid, index) {
    axiosInstance.post(`${ipaddress}/UniversityDiscussionReplyDisLikesView/${user.user_id}/${discussion_id}/`)
      .then((r) => {
        // console.log("Organization Reply disliked",r.data)
        getreplies(discid, index)
      })
      .catch(() => {
        // console.log("Organization reply dislike error")
      })
  }
  ...
  const getreplies = (discussion_id, index) => {
    axiosInstance.get(`${ipaddress}/UniversityDiscussionReply/${user.user_id}/${user.organization_id}/${discussion_id}/`)
      .then((r) => {
        //console.log("Organization Replies fetched successfully", r.data)
  ...
  const getreplies_for_reply = (particular_reply_id) => {
    axiosInstance.get(`${ipaddress}/UniversityDiscussionRepliesRepliesView/${user.user_id}/${user.organization_id}/${particular_reply_id}/`)
      .then((r) => {
        //console.log("Organization Replies under reply fetched successfully", r.data)
  ...
  const deleteReply = (disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/UniversityDiscussionReply/${user.user_id}/${discid}/${disc_reply_id}/`)
      .then((r) => {
        // console.log("Organization Reply Successfully Deleted",r.data)
        setcount1(count1 + 1)
        setCount((prev) => prev + 1)
        const toastLiveExample = document.getElementById('liveToast')
  ...
  const deleteReply_for_reply = (reply_reply_id, disc_reply_id, discid) => {
    axiosInstance.delete(`${ipaddress}/UniversityDiscussionRepliesRepliesView/${user.user_id}/${user.organization_id}/${reply_reply_id}/`)
      .then((r) => {
  ...
  return (
    <div className={`${pincomment_status ? '' : 'd-none'}`}>
      <h6 className={`text-center ${pinnedComments.length > 0 ? 'd-none' : 'py-3'}`} style={{ color: '#5d5fe3', fontSize: '14px' }}>No Followed comments available ...💬</h6>
      {pinnedComments && (
        pinnedComments.map((x, index) => {
          return (
            <div key={index} style={{ zIndex: 4 }}>
              <div className='bg-white rounded shadow-sm border mb-3'>
                <div className="d-flex gap-2 ps-2 justify-content-between border-bottom py-2 m-0 align-items-center">
                  <div className="px-1 px-lg-0 d-flex justify-content-center">
                    <img src={x.user_id.profile_pic} className={x.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="uni-pin" />
                  ...
                  <div className='d-flex justify-content-between w-100'>
                    <div className="p-0" onClick={() => {
                      ...
                      <p className='ms-sm-0 my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA' }}>{user.program_name}</p>
                    </div>
                    <div className="d-flex gap-2 justify-content-end align-items-center p-0" style={{ position: 'relative' }}>
                      <p className='ms-sm-0 my-0 d-flex align-items-center' style={{ fontSize: '12px', color: '#8587EA' }}><span className={`ms-2 edit ${x.edited ? '' : 'd-none'}`}>Edited</span></p>
                      {/* --------------------------------------------Unfollow comment------------------------------------------------------ */}
                      <span data-bs-toggle="tooltip" data-bs-placement="top"
                        data-bs-custom-class="custom-tooltip"
                        data-bs-title="Unfollow" className="d-flex align-items-center justify-content-center" style={{ cursor: 'pointer', height: '35px', width: '35px' }} onClick={() => {
                          unpin(x.id)
                            ...
                            <div className="modal-body">
                              <div className=' bg-white px-3 pt-2 pb-3 rounded'>
                                ...
                                <div className='d-flex gap-3'>
                                  <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="uni-pin" />
                                  ...
                                  <form className='w-100' onSubmit={(e) => sendEditedData(e, x.id)}>
                                    <div className="input-group bg-light border rounded pe-3">
                                      ...
                                      <button data-bs-dismiss="modal" disabled={editedpost.length > 0 ? false : true} className='text-secondary bg-transparent border-0'><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 30 30" fill="none">
                ...
                <div onClick={() => {
                  setindex1(-1)
                }} className='d-flex justify-content-between border-bottom px-2 pb-2 mb-2'>
                    ...
                    && (
                      <div className="w-75 d-flex justify-content-end">
                        <div className="d-flex gap-2">
                          {(!replies_status) ?
                            <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_status(!replies_status); setSelectedReply(index); getreplies(x.id) }}>
                              {translate_value.dashboard.view_all} {x.replies_count} {translate_value.dashboard.replies}
                            </button>
                            :
                            (selectedReply == index) ?
                              <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_status(!replies_status); setSelectedReply(null); }}>
                                {`View less`}
                              </button> :
                              <button className='bg-transparent border-0 fw-bold' style={{ color: '#5D5FE3', fontSize: '14px' }} onClick={(e) => { setreplies_status(!replies_status); setSelectedReply(index); getreplies(x.id) }}>
                                {translate_value.dashboard.view_all} {x.replies_count} {translate_value.dashboard.replies}
                                ...
                                <>{
                                  index < 3 ?
                                    a.user_id && a.user_id.profile_pic ?
                                      <img className="rounded-circle" style={{ objectFit: 'cover', backgroundColor: '#fff', position: 'relative', marginLeft: index > 0 ? '-11px' : '0px', border: '2px solid #fff' }} src={a.user_id.profile_pic} width={30} alt="uni-pin" />
                                      :
                ...
                {fetchedreplies && fetchedreplies.length > 0 && (
                  <div className={fetchedreplies[0].udisid == x.id && replies_status ? 'd-block' : 'd-none'}>
                    {fetchedreplies.map((y, index2) => {
                      ...
                      return (
                        <div key={index2} className='px-2 py-2 bg-white' onClick={() => {
                          setindex1(-1)
                        }} >
                          <div className="d-flex gap-2 w-100 justify-content-between align-items-center px-2">
                            <div className="d-flex gap-2 justify-content-center">
                              <img src={y.user_id.profile_pic} className={y.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="uni-pin" />
                            ...
                            <div className='d-flex justify-content-between w-100'>
                              ...
                              <div className="p-0 d-flex gap-2 align-items-center">
                          ...
                          <div className='ps-5 mt-2 m-0'>
                            ...
                            {y.images_attached.map((a) => {
                              return (
                                <div className='d-flex justify-content-center'>
                                  <img src={a.organization_discussion_reply_images} width={300} alt="uni-pin" className='mt-3' />
                                </div>
                              )
                            })}
                                    ...
                                    return (
                                      <div className='px-2 py-2 bg-white' onClick={() => {
                                        ...
                                        <div className="d-flex gap-2 w-100 align-items-center">
                                          <div className="d-flex justify-content-center">
                                            <img src={z.user_id.profile_pic} className={z.user_id.profile_pic == null ? 'd-none' : 'rounded-circle'} width={30} height={30} alt="uni-pin" />
                                        ...
                                        <div className='ps-0 ps-lg-5 mt-2 m-0'>
                                          ...
                                          {z.images_attached.map((b) => {
                                            return (
                                              <div className='d-flex justify-content-center'>
                                                <img src={b.organization_discussion_reply_reply_images} width={300} alt="uni-pin" className='mt-3' />
                                              </div>
                                            )
                                          })}
                                        </div>
                ...
                <form onSubmit={(e) => { postReplies(e, x.id, index) }}>
                  <div onClick={() => {
                    setindex1(-1)
                  }} className='d-flex gap-2 px-2 border-secondary-subtle align-items-center mb-2'>
                    <img src={userdetails.profile_pic} className={userdetails.profile_pic == null ? 'd-none' : 'rounded-circle'} width={40} height={40} alt="uni-pin" />
                    {userdetails.nickname != undefined ? (<p className={userdetails.profile_pic == null ? 'd-flex justify-content-center align-items-center bg-warning text-white rounded-circle my-auto' : 'd-none'} style={{ height: '40px', width: '40px' }}><span>{userdetails.nickname.slice(0, 1)}</span><span>{userdetails.nickname.slice(-1)}</span></p>) : (<></>)}
                    <div className="input-group border rounded pe-lg-3 bg-light">
                      <input key={index}
                        ...
                        className="form-control py-2 ps-3 shadow-none border-0 bg-light followed-reply-input" placeholder={translate_value.dashboard.reply_here}
                        style={{ position: 'relative' }} />
      ...
      <div className="modal fade" id="replyforreply_modal6" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                    ...
                    <div className="input-group bg-light border rounded pe-lg-3">
                      ...
                      <div className='d-flex align-items-center bg-light'>
                        ...
                        <label data-bs-toggle="tooltip" data-bs-placement="top"
                          ...
                          htmlFor="fileInput3"
                          className="custom-file-input bg-transparent border-0 px-lg-4 py-2">
                          <img src={require('../img/attachment.png')} width={22} height={22} alt="uni-pin" />
                        </label>
                        <button data-bs-dismiss="modal" disabled={reply_for_reply.length > 0 ? false : true} type='submit' className='text-secondary h-100 bg-transparent border-0 ms-2 outline-0' >
                          <svg data-bs-toggle="tooltip" data-bs-placement="top"
                            data-bs-custom-class="custom-tooltip"
                ...
                <div className='d-flex gap-3 mt-3'>
                  {replies_reply_Image3.length > 0 &&
                    replies_reply_Image3.map((image, index) => (
                      <div key={index} className="image-preview bg-light p-2" style={{ position: 'relative' }}>
                        <img src={URL.createObjectURL(image)} width={50} alt={`Selected Image ${index + 1}`} />
                        <button style={{ position: 'absolute', top: '-10px', right: '-16px' }} className='btn btn-sm' onClick={() => removeImage2(index)}><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-x-circle" viewBox="0 0 16 16">
                          <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14m0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16" />
                          <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708" />
                        </svg></button>
                      </div>
...
export default Organizationpinnedcomments