...
const Admin_Loginpage = ({ setemailvalidation }) => {
  ...
  const useridData = (e) => { setUserid(e.target.value) }
  const passwordData = (e) => { setPassword(e.target.value) }
  ...
  const verifiedlogin = (e) => {
    ...
    apiClient.post(`${ipaddress}/admin_app/AdminLogin/`, values)
      .then((r) => {
      ...
      .catch(() => {
        ...
        setUserid("");
        setPassword("");
        toast.error('Invalid Email and Password', { autoClose: 3000 });
        setloading(false);
      })
  }
  ...
  const verifyEmail = (e) => {
    e.preventDefault()
    apiClient.post(`${ipaddress}/userverification/`)
      .then((r) => {
        setOtp(r.data.otp); // console.log("OTP FETCHED",r.data)
      })
  ...
  const submitOtp = (e) => {
    e.preventDefault()
    if (newotp === otp) {
      apiClient.put(`${ipaddress}/CheckEmailForThreeMonths/${userid}/`)
        .then((r) => {
  ...
  const forgotpassword = () => {
    ...
    apiClient.post(`${ipaddress}/ForgetPassword/`, formdata)
      .then((r) => {
        ...
        <div className="row mt-4 m-0 h-100 animate__animated animate__fadeIn">
          ...
          <div className="col-lg-6 d-flex align-items-center login-text mt-5 mt-lg-0">
            <div className='' style={{ position: 'relative' }}>
              ...
              <img className='login-img1' style={{ position: 'absolute', animation: 'moveAnimation 6s linear infinite', width: '115px', height: '127px' }} src={require('../img//images_icons/Group-removebg-preview.png')} alt="group" />
              <img className='login-img2' style={{ position: 'absolute', height: '135px', width: '31px' }} src={require('../img/images_icons/login-image.png')} alt="login" />
