...
const Admin_report = () => {
  ...
  const fetchdata = () => {
    apiClient.get(`${ipaddress}/admin_app/ReportedValues/${tab}/`)
      .then((r) => {
        setreport_details(r.data);
        setDisplayed_user(r.data.slice(0, ITEMS_PER_PAGE));
      })
      .catch((err) => { console.log("Reported data fetching error", err) })
  }

  //   Document Report
  const fetchdocumentdata = () => {
    apiClient.get(`${ipaddress}/admin_app/ReportedValues/documents/`)
      .then((r) => {
  ...
  const fetchcoursedata = () => {
    apiClient.get(`${ipaddress}/admin_app/ReportedValues/courses/`)
      .then((r) => {
  ...
  const fetchgroupdata = () => {
    apiClient.get(`${ipaddress}/admin_app/ReportedValues/groups/`)
      .then((r) => {
  ...
  const fetchflashsetdata = () => {
    apiClient.get(`${ipaddress}/admin_app/AdminFlashReport/`)
      .then((r) => {
