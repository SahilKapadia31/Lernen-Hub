...
const Comment_report = () => {
  ...
  useEffect(() => {
    apiClient.get(`${ipaddress}/admin_app/AdminCommentsReports/${value}/`)
      .then((r) => {
        setspecific_report(r.data); // console.log("Comment report data",r.data)
      })
      .catch((err) => {
  ...
  const deletecomment = (id) => {
    apiClient.delete(`${ipaddress}/admin_app/AdminCommentsReports/${value}/${id}/`)
      .then((r) => {
        setCount(count + 1);  // console.log("Comment deleted",r.data)
      }).catch((err) => { console.log("Comment deleting error", err) })
  }
