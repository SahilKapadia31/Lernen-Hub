...
export const Features = () => {
    ...
    const [schoolActiveFilter, setSchoolActiveFilter] = useState("All");
    const [tutorActiveFilter, setTutorActiveFilter] = useState("All");
    ...
    const getOrganizations = async () => {
        try {
            const response = await axiosInstance.get(`${ipaddress3}/public/homepage/organization/?organization_type=universities`);
            if (response?.data?.status) {
    ...
    const getSchoolsList = async () => {
        try {
            const response = await axiosInstance.get(`${ipaddress3}/public/homepage/organization/?organization_type=school`);
            if (response?.data?.status) {
                const schoolList = response.data.data;
                setSchoolList(schoolList);
            }
        } catch (err) {
            console.error('Error in getSchoolsList:', err);
        }
    }
    ...
    const getTutorList = async () => {
        try {
            const response = await axiosInstance.get(`${ipaddress3}/public/homepage/organization/?organization_type=tutor`);
            if (response?.data?.status) {
            ...
            <section className="university-listing">
                <Container>
                    ...
                    <div className="slider-container mt-4">
                        <Slider {...settings}>
                            {organizationList.map((university) => (
                                <div className="px-1" key={university?.university_id}>
                                    <Card className="slider-card border shadow-sm hover-shadow transition">
                                        <Card.Body className="d-flex flex-column position-relative">
                                            <div className="text-center mb-2">
                                                ...
                                                >
                                                    <span className="university-logo-text h3 mb-0 fw-bold" style={{ color: getRandomColor()?.textColor }}>{university?.logo || getUniversityInitials(university?.university_name)}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="university-info small text-muted mb-3">
                                                <div className="d-flex align-items-center mb-2">
                                                    ...
                                                    <span className="ms-2 text-truncate fw-semibold text-black text-capitalize">{university?.university_name}</span>
                                                </div>
                                                <div className="d-flex align-items-center mb-2">
                                                ...
                                                <div className="d-flex align-items-center">
                                                    ...
                                                    <span>{university?.programs_types?.map(item => item.name).join(', ') || 'Pending    '}</span>
                                                </div>
                                            </div>
                    ...
                    <div className="text-center mt-5">
                        <Button className="explore py-2 px-4" onClick={(e) => { window.location.href = '/learn-hub-demo/#/universities_info'; }}>
